...
| S1 | P1 | **全項目完了・清算済**(`18d307076` CAS封鎖 / `305b50c7f` JST正規化是正)。clock seam項目は前提誤りと判明し裁定で不採用。新規follow-up 2件(同日チェックのtransaction外・vaccination同型疑い)を起票 | なし（FE12-02と独立・所有path非重複） | 各項目の修正と並行/境界test証跡 |
...
- **【新規・follow-up】カルテ自動生成の同日チェックがtransaction外** — S1第2走で判明（本unitのscope外・既存構造）。同一pet/同日の存在チェックがcreateと同一transactionに入っておらず、対応するunique制約も無いため、異なる予約から同時にauto-createが走ると重複カルテが生じ得る。JST正規化の是正はこの競合を解消しない。判断者: medical-record contract owner。次の一手: 一意制約の付与かcheck-and-createの原子化のいずれを採るかを決める。
- **【新規・follow-up】`vaccination_repository.go:225`の同型ambient location依存疑い** — S1第2走のcensusが唯一の候補として検出。ドメインcontract（何を基準日とするか）が未確定のため本unitでは変更していない。判断者: vaccination contract owner。
