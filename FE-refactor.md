...
| examination parent/items部分成功 | 未解消 | POST/PATCH成功・PUT items失敗時にparentを残すか、combined writeか、再送単位が未定義 | 要件責任者=曽我、examination contract owner | `use-examination-form.ts:272-329`を起点に結果contractを定義し、parent成功/items失敗testを作る |
...
- **examination parent成功・items失敗** — 最初に開く: `frontend/src/features/examinations/hooks/use-examination-form.ts:272-329`、`api/create-examination.ts:8-27`、`api/update-examination-items.ts:8-31`。確認: POST/PATCH後に別PUTを行う2 operationで、items失敗時のparent残置、再送単位、表示、重複をどう扱うか。判断者: 要件責任者=曽我とexamination contract owner。手順: parent残置／combined write／再送contractを決め、既存`use-examination-form.test.ts`へparent成功・items失敗caseを作ってから変更する。
