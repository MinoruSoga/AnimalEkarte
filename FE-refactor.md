...
| S2 | P1 | **全項目完了・清算済**(`082be9961` reception danger / `da550a84d` master permission 45/45配線+fail-closed化 / `8a00a4794` auth独立chunk化+checkup resource整合 / `e8a4db982` StaffSettings test陳腐化)。USER再buildでINEFFECTIVE_DYNAMIC_IMPORT消滅・auth 13.90 kB独立chunk・main 109.05→96.72 kBを確認、type-check green。manual chunkは計測完了し対応不要と裁定。派生=staff検証契約drift 5件が仕様裁定待ち | なし（FE12-02と独立・所有path非重複） | 各項目の契約確定または修正と証跡 |
<!-- FE12-TASK-TABLE-END -->
...
- **【実装済・清算済】StaffSettings赤テスト4件** — 2026-07-27 S2第4走で解消（`e8a4db982`）。**production不具合ではない**: `99bac632e` が `useMasterSave.handleSave` を `Promise<boolean>` + `mutateAsync` へ変えた際、test側のmockが旧 `{ mutate }` 同期契約のまま取り残されて赤化していた。修正はtest契約のみ（`it` を async 化・mock を `mutateAsync` へ・保存を `await act(async () => ...)` で待機）。生成側の独立検証: `StaffSettings.tsx` は byte-identical、`expect` の削除ゼロ、skip/todo 導入ゼロ。**process教訓**: baseline相対ゲート（並行writer下でのspurious BLOCKED回避のため導入）は、baselineに元から赤があると3走連続でそれを素通りさせる。以後、baselineにFAILを含むプロンプトはtest名と一行の失敗理由の報告を必須とする（件数だけでは不可）。同走が staff 入力検証の契約 drift 5件（email truthy-onlyチェック・既存staffへの短いpassword・frontend/OpenAPIとbackendのアカウントなしstaff許容の食い違い・空名test命名・read-only時のEnter form action）を起票したが、**仕様裁定待ちのため実装unit化していない**。
- **【実装済・清算済】TreatmentPlan checkupのresource不整合** — S2第2走で判明、S2第3走で解消（`8a00a4794`・上記auth entry項目と同commit）。以下は当時の分析。checkupのsave経路はrequired化に伴い`ResourceCheckups`へ是正されたが、同ファイルのUI/reorder control（`TreatmentPlanMaster.tsx:313`の`resource={ResourceMasterMedical}`）は`ResourceMasterMedical`のままで、表示条件とmutation gateが別resourceを見ている。**どちらの向きもfail-safeであり脆弱性ではない**（checkups権限のみの利用者はcontrolが見えず、master-medical権限のみの利用者はcontrolが見えてmutationで拒否される）。判断者: frontend RBAC owner。次の一手: UI側resourceをsave経路と揃えるか、両者が意図的に別権限であることを明文化する。
...
- **【実装済・清算済】AuthProviderのfeature barrel経由eager import** — 2026-07-27 S2第3走で解消（`8a00a4794`）。`frontend/src/features/auth/provider.ts`（`export { AuthProvider } from "./hooks/use-auth";` の1行）を additive な public entry として追加し、`router.tsx:5` の参照をそこへ逃がした。deep-import lint は group patternから `regex: "^@/features/(?!auth/provider$)[^/]+/.+$"` へ変更し、end-anchored negative lookahead で `@/features/auth/provider` **だけ**を例外化（子・兄弟は従来どおり拒否）。**USER再buildで完了判定green**: `INEFFECTIVE_DYNAMIC_IMPORT` 警告が消滅し、`dist/assets/auth-OxZsOeao.js` = 13.90 kB（gzip 3.51 kB）が独立chunkとして出力、`main-*.js` は 109.05 kB → **96.72 kB**（-12.33 kB・auth chunk実体とほぼ一致）へ縮小した。`tsc --noEmit` もエラー0。**同走でTreatmentPlan checkup resource不整合も同時解消**: `activeResource`/`activeCanEdit` をタブ値で分岐させ、UI resource・reorder guard・sidePanel の canEdit/canDelete をすべて save 経路（`ResourceCheckups`）と揃えた。
- **【旧記述・裁定根拠】AuthProvider項目のbundler実測** — 2026-07-27 の USER build で **bundler 自身が欠陥を明示**した: `[INEFFECTIVE_DYNAMIC_IMPORT] src/features/auth/index.ts is dynamically imported by src/app/routes/app-routes.tsx but also statically imported by src/app/router.tsx, dynamic import will not move module into another chunk.` つまり `app-routes.tsx` の Login / ForgotPassword / ResetPassword の dynamic import は `router.tsx` の同期 import に打ち消されており、**認証 3 画面が独立 chunk にならず全利用者の初期 bundle に載っている**。第1走の推論が実測で裏付けられた。**裁定（2026-07-27）: provider 専用 public entry を採用する**（既存 root barrel を維持したまま additive に追加でき移行コストが最小。barrel 分割は root export 改変で影響が大きく、現状維持は警告の放置になる）。実装時の付随判断: 現行 deep-import lint が `@/features/auth/*` を禁止するため、provider entry のみの厳密例外か feature-level alias が要る。完了判定は再 build で当該 INEFFECTIVE_DYNAMIC_IMPORT が消え、認証 3 画面が独立 chunk として出力されること。
