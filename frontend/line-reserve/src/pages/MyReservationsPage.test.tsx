...
describe('MyReservationsPage（R-F25: ネイティブダイアログ→インラインUI置換）', () => {
  it('予約一覧を表示する', async () => {
    server.use(
      http.get('/api/liff/:clinicId/my-reservations', () => HttpResponse.json([confirmedReservation])),
    );
  ...
  it('確認UIで「はい」を選び失敗するとネイティブalertではなくインラインバナーを表示し、再試行可能にする', async () => {
    ...
    server.use(
      http.get('/api/liff/:clinicId/my-reservations', () => HttpResponse.json([confirmedReservation])),
      http.delete('/api/liff/:clinicId/my-reservations/:id', () => HttpResponse.json(null, { status: 500 })),
    );
  ...
  it('一覧取得API失敗(5xx)時はサーバーエラーメッセージと再試行ボタンを表示する（R-F22/R-F23）', async () => {
    server.use(
      http.get('/api/liff/:clinicId/my-reservations', () => HttpResponse.json(null, { status: 500 })),
    );
  ...
  it('一覧取得API失敗(401)時は再ログインメッセージを表示し、再試行ボタンは出さない（R-F22/R-F23）', async () => {
    server.use(
      http.get('/api/liff/:clinicId/my-reservations', () => HttpResponse.json(null, { status: 401 })),
    );
