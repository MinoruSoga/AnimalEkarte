...
export function useClinicMasterSettings() {
  ...
  // UI の非表示・disabled は操作補助にすぎない。mutation 境界では最新の権限値を
  // ref 経由で再検査し、fail-closed にする（frontend/CLAUDE.md 臨床安全境界②）。
  ...
  const [formState, formAction] = useActionState(
    async (_prevState: FormState, _formData: FormData): Promise<FormState> => {
      ...
      if (!fd.name) {
        return { success: false, timestamp: Date.now(), fieldErrors: { name: "院名は必須です" } };
      }
      const isUpdate = Boolean(selected?.id);
      if (isUpdate ? canEditRef.current !== true : canCreateRef.current !== true) {
      ...
      try {
        ...
        // mutation の onError で handleApiError 済み（api/clinics.ts）。ここでは状態遷移のみ。
        return { success: false, timestamp: Date.now() };
      }
    },
  ...
  const handleDeleteConfirm = useCallback(() => {
    if (pendingDeleteId === null || canDeleteRef.current !== true) return;
    startDeleteTransition(() => {
      // onError は mutation 側（api/clinics.ts の useDeleteClinic）で handleApiError 済み。
