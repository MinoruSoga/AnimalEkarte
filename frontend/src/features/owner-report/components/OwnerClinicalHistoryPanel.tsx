...
function examinationRowState(data: OwnerClinicalBriefingData): HistoryRowState {
  const accessCount =
    Number(data.permissions.examination.canView) +
    Number(data.permissions.checkup.canView);
  const state = rowState(accessCount > 0, {
    isLoading:
      (data.permissions.examination.canView &&
        data.examinationsQuery.isLoading) ||
      (data.permissions.checkup.canView && data.checkupsQuery.isLoading),
    isError:
      (data.permissions.examination.canView &&
        data.examinationsQuery.isError) ||
...
function historyRowStates(
  ...
  return {
    ...
    予防接種: rowState(
      data.permissions.vaccination.canView,
      data.vaccinationsQuery,
    ),
...
function historyMatrix(data: OwnerClinicalBriefingData) {
  return buildClinicalHistoryMatrix({
    ...
    examinations: data.permissions.examination.canView
      ...
      : [],
    checkups: data.permissions.checkup.canView
      ? (data.checkupsQuery.data ?? [])
      : [],
    treatments: data.treatmentsQuery.data?.items ?? [],
    vaccinations: data.permissions.vaccination.canView
      ? (data.vaccinationsQuery.data ?? [])
      : [],
    trimmings: data.permissions.trimming.canView
      ? (data.trimmingQuery.data?.items ?? [])
      : [],
  });
}
...
function historyIsTruncated(data: OwnerClinicalBriefingData): boolean {
  ...
  return [
    Boolean(
      medicalRecords && medicalRecords.total > medicalRecords.data.length,
    ),
