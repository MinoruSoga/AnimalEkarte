...
import {
  ...
  type BackendPetTreatmentHistory,
} from "./get-pet-treatment-history";
...
describe("transformHistoryItem", () => {
  it("medicine は medicine_name を表示名にする", () => {
    const out = transformHistoryItem(
      row({ item_type: "medicine", medicine_name: "アモキシシリン", content: "投薬メモ", admin_route: "経口", quantity: 2 }),
    );
  ...
  it("procedure は procedure_name と麻酔ラベルを表示する", () => {
    const out = transformHistoryItem(
      row({ item_type: "procedure", procedure_name: "避妊手術", anesthesia: "general", content: "" }),
    );
  ...
  it("名前も content も空なら '-' を表示する", () => {
    const out = transformHistoryItem(row({ content: "", medicine_name: undefined, procedure_name: undefined }));
    expect(out.name).toBe("-");
  });

  it("未知の麻酔値は原文を保持する", () => {
    const out = transformHistoryItem(row({ item_type: "procedure", procedure_name: "X", anesthesia: "spinal" }));
    expect(out.anesthesia).toBe("spinal");
  });

  it("is_surgery=true は isSurgery=true にマップされる", () => {
    const out = transformHistoryItem(
      row({ item_type: "procedure", procedure_name: "開腹手術", anesthesia: "general", is_surgery: true }),
    );
    expect(out.isSurgery).toBe(true);
  });

  it("is_surgery が undefined のとき isSurgery は undefined", () => {
    const out = transformHistoryItem(row({ item_type: "medicine", medicine_name: "アモキシシリン" }));
