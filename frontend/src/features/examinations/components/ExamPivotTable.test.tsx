...
function makeExamination(id: string, date: string, testTypeId = "10"): ExaminationRecord {
  return {
    id,
    date,
    ownerName: "飼主",
...
function makeItem(overrides: Partial<ExamItemFixture> = {}): ExamItemFixture {
  return {
    id: 1,
    exam_id: 1,
...
function renderPivot(
  ...
  return render(<ExamPivotTable examinations={examinations} sortOrder="desc" />, {
...
describe("ExamPivotTable", () => {
  it("同日検査を別列に保ち、field ID行を転置して欠測セルを「-」で表示する", async () => {
    renderPivot([makeExamination("2", "2026-07-20"), makeExamination("1", "2026-07-20")], {
      "2": [
        makeItem({
          id: 2,
          exam_id: 2,
    ...
    expect(screen.getByRole("columnheader", { name: "2026-07-20 2件目" })).toBeInTheDocument();

    const gluRow = screen.getByRole("row", { name: /GLU.*120.*60/ });
    expect(within(gluRow).getByText("120").closest("td")).toHaveClass(C.bgDanger8);
    expect(within(gluRow).getByText("60").closest("td")).toHaveClass(C.bgStatusBlueLight);
  ...
  it("未評価のnormal cellに「未判定」cueを表示する", async () => {
    renderPivot([makeExamination("1", "2026-07-20")], {
    ...
    expect(within(row).getByText("（基準値未設定のため判定していない）")).toHaveClass("sr-only");
  });
  ...
  it("同名でも異なるfield IDは別行にし、inspection_valueだけを値として表示する", async () => {
    ...
    expect(screen.queryByText("result-field-is-not-the-source")).not.toBeInTheDocument();
  });
  ...
  it("直近10検査だけを取得・表示する", async () => {
    ...
    expect(await screen.findByRole("columnheader", { name: "2026-07-11" })).toBeInTheDocument();
    await waitFor(() => expect(requestedIds).toHaveLength(10));
    expect(screen.queryByRole("columnheader", { name: "2026-07-01" })).not.toBeInTheDocument();
  ...
  it("保存済みreference_valueを優先し、無ければ保存済みref_min/ref_maxを表示する", async () => {
    ...
    expect(within(screen.getByRole("row", { name: /BUN/ })).getByText("-")).toBeInTheDocument();
    expect(screen.getByText("0-")).toBeInTheDocument();
  });
  ...
  it("検査履歴が無い場合は空状態を表示する", () => {
    render(<ExamPivotTable examinations={[]} sortOrder="desc" />, { wrapper: createTestWrapper() });
  ...
  it("項目取得失敗を欠測表示にせず明示する", async () => {
    server.use(
      http.get("/api/v1/examinations/:id/items", () => new HttpResponse(null, { status: 500 })),
    );

    render(
      <ExamPivotTable examinations={[makeExamination("1", "2026-07-20")]} sortOrder="desc" />,
      { wrapper: createTestWrapper() },
    );
