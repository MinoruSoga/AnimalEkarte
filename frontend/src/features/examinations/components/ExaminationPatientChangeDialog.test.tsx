...
vi.mock("@/components/shared/ReservationFormModal/PatientSelectionTable", () => ({
  PatientSelectionTable: (props: { includeDeceased?: boolean; onSelect: (pet: Pet) => void }) => {
    ...
    return (
      <>
        ...
        <button type="button" onClick={() => props.onSelect({ ...pet, status: "不明" })}>
          状態不明患者を選択
        </button>
      </>
    );
...
describe("ExaminationPatientChangeDialog", () => {
  it("死亡sentinelを含む検索を要求し、生存患者の選択後に閉じる", async () => {
    ...
    render(<ExaminationPatientChangeDialog selectedPet={undefined} onSelect={onSelect} />);
    ...
    expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ id: "84", status: "生存" }));
    expect(screen.queryByRole("button", { name: "テスト患者を選択" })).not.toBeInTheDocument();
  });
  ...
  it("状態不明の患者を選択せずdialogを開いたままにする", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "患者を変更" }));
    await user.click(screen.getByRole("button", { name: "状態不明患者を選択" }));

    expect(onSelect).not.toHaveBeenCalled();
    expect(screen.getByRole("button", { name: "状態不明患者を選択" })).toBeInTheDocument();
  });
});
