...
export function validateReferenceRangeDrafts(
  ...
  for (const draft of drafts) {
    ...
    const hasNumeric = draft.mode === "numeric" &&
      (draft.min.trim() !== "" || draft.max.trim() !== "");
    const rawQualitativeMin = draft.mode === "qualitative"
      ? draft.min
      : draft.qualitativeMin ?? "";
    const rawQualitativeMax = draft.mode === "qualitative"
      ? draft.max
      : draft.qualitativeMax ?? "";
    const hasQualitative =
      rawQualitativeMin.trim() !== "" || rawQualitativeMax.trim() !== "";
    ...
    if (hasQualitative) {
      const minIndex = rawQualitativeMin.trim() === ""
        ? undefined
        : qualitativeIndex(rawQualitativeMin);
      const maxIndex = rawQualitativeMax.trim() === ""
        ? undefined
        : qualitativeIndex(rawQualitativeMax);
...
export function buildReferenceRangeRequest(
  ...
  return drafts.map((draft) => {
    ...
    return {
      ...
      ...(draft.min.trim() === ""
        ? {}
        : { qualitative_min: normalizeQualitativeValue(draft.min) }),
      ...(draft.max.trim() === ""
        ? {}
        : { qualitative_max: normalizeQualitativeValue(draft.max) }),
    };
  });
}

export function toReferenceRangeDraft(
  range: ExamReferenceRange,
): ReferenceRangeDraft {
  const qualitative = range.qualitativeMin !== undefined ||
    range.qualitativeMax !== undefined;
  return {
