...
describe("CategorySidePanel", () => {
  it("leaf type: ReservationTypeAvailableSlotsSection が render される", async () => {
    server.use(
      http.get("/api/v1/masters/reservation-types/1/available-slots", () => HttpResponse.json([])),
      http.get("/api/v1/masters/reservation-types/1/unavailable-times", () =>
        HttpResponse.json([]),
      ),
      http.get("/api/v1/masters/reservation-types/1/occupations", () => HttpResponse.json([])),
    );

    const leafType = makeType({ id: "1", isLeaf: true });

    render(<CategorySidePanel item={leafType} onClose={noop} onSave={noop} groups={[]} />, {
    ...
    // 案内文は表示されない
    expect(screen.queryByText("子予約区分ごとに予約枠を設定してください")).not.toBeInTheDocument();
  });
  ...
  it("parent type: セクションが render されず、案内文が表示される", async () => {
    ...
    render(<CategorySidePanel item={parentType} onClose={noop} onSave={noop} groups={[]} />, {
      wrapper: createWrapper(),
    });

    expect(await screen.findByText("子予約区分ごとに予約枠を設定してください")).toBeInTheDocument();
