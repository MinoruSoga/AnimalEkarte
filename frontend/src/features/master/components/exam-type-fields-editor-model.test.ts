...
describe("exam type reference range validation", () => {
  it("rejects numeric and qualitative bounds coexisting", () => {
    expect(validateReferenceRangeDrafts([baseDraft({ min: "1", qualitativeMin: "(-)" })])).toBe(
      "数値範囲と定性範囲は同時に指定できません",
    );
  });
  ...
  it("rejects unsupported and reversed qualitative bounds", () => {
    ...
    expect(
      validateReferenceRangeDrafts([baseDraft({ mode: "qualitative", min: "(++)", max: "(+)" })]),
    ).toBe("定性範囲の下限は上限以下にしてください");
  });

  it("rejects non-finite and reversed numeric bounds", () => {
    expect(validateReferenceRangeDrafts([baseDraft({ min: "Infinity" })])).toBe(
      "数値範囲には有限の数値を入力してください",
    );
    expect(validateReferenceRangeDrafts([baseDraft({ min: "10", max: "5" })])).toBe(
      "数値範囲の下限は上限以下にしてください",
    );
  });
