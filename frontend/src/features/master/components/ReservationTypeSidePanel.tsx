    ...
    >
      ...
      <PropertyRow label="グループ">
        <Select
          ...
          onValueChange={(v) =>
            setFormDataDirty((prev) => ({ ...prev, groupId: v === "none" ? undefined : v }))
          }
        >
      ...
      <div className={`mt-4 pt-4 ${STYLE.sectionDivider}`}>
        ...
        <PropertyRow label="予約ページに表示">
          <Switch
            ...
            onCheckedChange={(v) =>
              setFormDataDirty((prev) => ({ ...prev, reservationVisible: v }))
            }
          />
        </PropertyRow>
        ...
        <PropertyRow label="予約可能曜日">
          <Select
            ...
            onValueChange={(v) =>
              setFormDataDirty((prev) => ({ ...prev, reservationDayOption: v }))
            }
          >
            <SelectTrigger className={STYLE.selectCompact}>
              <SelectValue />
      ...
      {item !== null ? (
        <>
          {item.isLeaf ? (
            ...
            <div className={`mt-4 pt-4 ${STYLE.sectionDivider}`}>
              <p className={`text-xs ${C.text40}`}>子予約区分ごとに予約枠を設定してください</p>
            </div>
          )}
          <ReservationTypeOccupationsSection clinicId={item.clinicId} reservationTypeId={item.id} />
        </>
      ) : null}
    </MasterSidePanel>
  );
