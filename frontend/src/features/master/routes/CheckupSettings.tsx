...
const COLUMNS = [
  ...
  { header: "健診種別名" },
  { header: "単価", className: "w-[120px]", align: "right" as const },
...
// ─────────────────────────────────────────────────
// フォームデータ型（price: number — 0 = 未設定/無料）
// ─────────────────────────────────────────────────
...
function CheckupSidePanel({
  item,
  onClose,
  onSave,
  onDeleteRequest,
}: {
  item: CheckupTypeItem | null;
  onClose: () => void;
  onSave: (data: CheckupFormData) => void;
  onDeleteRequest: (item: CheckupTypeItem) => void;
}) {
        ...
        <div className="px-16 pb-8">
          <div className="pt-4 pb-2">
            <div className={STYLE.pageIcon}>
              <ClipboardCheck className={LAYOUT.pageIcon.innerIcon} />
            </div>
          </div>
            ...
            <PropertyRow label="単価（税込）">
              <div className="flex items-center gap-1.5 w-full">
                <input
                  ...
                  placeholder="0"
                />
                <span className={`text-sm ${C.text70} shrink-0`}>円</span>
              </div>
            </PropertyRow>
            <PropertyRow label="健診間隔">
              <PropInput
                value={formData.interval}
                onChange={(v) => setFormData((prev) => ({ ...prev, interval: v }))}
                placeholder="例: 年1回"
              />
            </PropertyRow>
            <PropertyRow label="対象年齢">
              <PropInput
                value={formData.targetAge}
                onChange={(v) => setFormData((prev) => ({ ...prev, targetAge: v }))}
                placeholder="例: シニア"
              />
            </PropertyRow>
...
function SortableCheckupRow({
  item,
  onEdit,
}: {
  item: CheckupTypeItem;
  onEdit: () => void;
}) {
...
// ─────────────────────────────────────────────────
// CheckupSettings (main page)
// ─────────────────────────────────────────────────

export function CheckupSettings() {
  const navigate = useNavigate();

  const [selectedItem, setSelectedItem] = useState<CheckupTypeItem | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [pendingDelete, setPendingDelete] = useState<CheckupTypeItem | null>(null);

  const { data: rawData } = useGetAllCheckupTypes();
  ...
  const handleEdit = useCallback((item: CheckupTypeItem) => {
  ...
  const handleSave = useCallback(
    (data: CheckupFormData) => {
      ...
      if (selectedItem) {
        ...
        const req: CreateCheckupTypeRequest = {
          name: data.name,
          price: data.price > 0 ? data.price : undefined,
          is_active: data.isActive,
          interval: data.interval || undefined,
  ...
  const handleDeleteRequest = useCallback((item: CheckupTypeItem) => {
  ...
  return (
    ...
    >
      <div className="flex h-full">
        ...
        {isEditing ? (
          <CheckupSidePanel
            key={selectedItem ? String(selectedItem.id) : "new-checkup"}
            item={selectedItem}
            onClose={handleClose}
            onSave={handleSave}
