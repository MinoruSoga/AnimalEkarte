...
import { ChiefComplaintSettings } from "./ChiefComplaintSettings";

const mocks = vi.hoisted(() => ({
  queryResult: {
    data: [] as Array<{ id: string; name: string; description?: string; isActive: boolean }>,
    isPending: false,
    isError: false,
...
vi.mock("../components/ChiefComplaintSidePanel", () => ({
  ChiefComplaintSidePanel: () => null,
}));
...
vi.mock("../api/chief-complaint-types", () => ({
  useGetChiefComplaintTypes: () => mocks.queryResult,
  useCreateChiefComplaintType: mutationStub,
  useUpdateChiefComplaintType: mutationStub,
  useDeleteChiefComplaintType: mutationStub,
}));
...
describe("ChiefComplaintSettings", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setQueryResult({});
    setPermissions();
  ...
  it("取得成功時に一覧データを CRUD hook へ渡し、作成/編集権限を保存 hook へ伝える", () => {
    setQueryResult({
      data: [{ id: "1", name: "咳", description: "呼吸器", isActive: true }],
    });
    ...
    expect(mocks.crudData).toEqual([
      { id: "1", name: "咳", description: "呼吸器", isActive: true },
    ]);
  ...
  it("view-only 権限では create/edit/delete を閉じる", () => {
    ...
    setQueryResult({
      data: [{ id: "1", name: "咳", isActive: true }],
    });
  ...
  it("新規追加操作が handleNew を呼ぶ", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "主訴マスタを追加" }));
