...
describe("examination type field master API", () => {
  ...
  it("preserves items and reference ranges from the examination type response", () => {
    ...
    expect(result.items).toEqual([expect.objectContaining({
      ...
      name: "白血球",
      unit: "/μL",
      referenceRanges: [expect.objectContaining({
        id: "41",
        animalSpeciesId: "2",
        refMin: 5,
        refMax: 10,
  ...
  it("uses the fixed CRUD, reorder, replace, and explicit clear endpoints", async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(
      "/v1/masters/examination-types/3/fields",
      { name: "白血球", sort_order: 1 },
    );
    expect(axios.patch).toHaveBeenCalledWith(
      "/v1/masters/examination-types/3/fields/31",
      { unit: "/μL" },
    );
    ...
    expect(axios.patch).toHaveBeenCalledWith(
      "/v1/masters/examination-types/3/fields/reorder",
      { ids: [31, 32] },
    );
