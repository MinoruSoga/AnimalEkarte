...
type CarePlanItemRequestInput<T extends CreateCarePlanItemInput | UpdateCarePlanItemInput> = Omit<
  T,
  keyof CarePlanItemRequestReferenceIds
> &
  CarePlanItemRequestReferenceIds;
...
function toCarePlanItemRequestInput<T extends CreateCarePlanItemInput | UpdateCarePlanItemInput>(
  input: T,
): CarePlanItemRequestInput<T> {
  const { medicine_id, procedure_id, hospitalization_plan_id, ...requestInput } = input;
...
const deleteCarePlanItem = async (hospitalizationId: string, itemId: string): Promise<void> => {
...
export function useCreateCarePlanItem(hospitalizationId: string) {
  ...
  return useMutation({
    mutationFn: (input: CreateCarePlanItemInput) => createCarePlanItem(hospitalizationId, input),
    onSuccess: () => {
