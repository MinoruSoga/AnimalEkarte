...
describe("summarizeCategoryTotals DEC-40 未分類・要確認件数", () => {
  ...
  it("件数 0・金額 0 の新 snapshot は行を出さない", () => {
    expect(summarizeCategoryTotals({ categories: {}, unclassified_other_count: 0 })).toEqual([]);
  });

  it("件数のみ（金額 0）の新 snapshot は行を出す", () => {
    expect(summarizeCategoryTotals({ categories: {}, unclassified_other_count: 1 })).toEqual([
      { label: "未分類・要確認", total: 0, count: 1 },
    ]);
  });
});
