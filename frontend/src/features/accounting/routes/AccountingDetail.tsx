...
export function AccountingDetail() {
  ...
  const updateMutation = useUpdateAccounting();
  ...
  // null = baseAccounting の items をそのまま使う
  ...
  // 保険設定: API から取得した payment があれば初期値として使う
  const initialInsurance = (baseAccounting?.payment?.insuranceAmount ?? 0) < 0;
  const initialInsuranceRatio =
    baseAccounting?.payment?.insuranceRatio?.toString() ?? "0.5";
  const initialReceived =
    baseAccounting?.payment?.receivedAmount?.toString() ?? "";
  const initialMethod: string = baseAccounting?.payment?.method ?? "cash";

  const [useInsurance, setUseInsurance] = useState(initialInsurance);
  const [insuranceRatio, setInsuranceRatio] = useState<string>(initialInsuranceRatio);
  const [receivedAmount, setReceivedAmount] = useState<string>(initialReceived);
      ...
      updateMutation.mutate(
        {
          ...
          req: {
            ...
            insurance_ratio: useInsurance ? parseFloat(insuranceRatio) : null,
            insurance_amount:
            ...
            <Card className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="p-4 bg-gray-50 border-t flex justify-end gap-6 text-sm">
                ...
                <span className="font-bold text-lg">
                  合計: ¥{calculation.totalAmount.toLocaleString()}
                </span>
              </div>
            </Card>
          </div>
            ...
            <Card className="flex-1">
              ...
              <CardContent className="p-6 space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 justify-end">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setReceivedAmount(calculation.billingAmount.toString())}
                      >
                        丁度
                      </Button>
                      <Button
                        ...
                        onClick={() =>
                          setReceivedAmount(
                            (Math.ceil(calculation.billingAmount / 1000) * 1000).toString(),
                          )
                        }
                      >
                        千円単位
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setReceivedAmount(
                            (Math.ceil(calculation.billingAmount / 10000) * 10000).toString(),
                          )
                        }
                      >
                        一万単位
                ...
                <div className="bg-gray-100 p-4 rounded-lg flex justify-between items-center">
                  ...
                  <span
                    className={`text-2xl font-bold ${calculation.changeAmount < 0 ? "text-red-500" : "text-gray-900"}`}
                  >
