...
export function useMedicalRecordForm(recordId?: string) {
  ...
  const { formState, formAction, isSaving } = useMedicalRecordSaveAction({
    ...
    chiefComplaintBaseline: read.existingRecord?.chiefComplaint || DEFAULT_CHIEF_COMPLAINT,
    ...
    treatmentPolicyBaseline: read.existingRecord?.notes || DEFAULT_TREATMENT_POLICY,
