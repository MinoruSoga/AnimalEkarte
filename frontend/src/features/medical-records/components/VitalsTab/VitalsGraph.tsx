...
const METRICS: MetricDef[] = [
  {
    ...
    label: "体温",
    unit: "℃",
    color: PALETTE.chartTemperature,
    yAxisId: "left",
    domain: [30, 45],
  },
  {
  ...
  },
  {
    ...
    label: "呼吸数",
    unit: "/min",
    color: PALETTE.chartRespiratory,
    yAxisId: "right",
    domain: [0, "auto"],
  },
  {
...
const CustomTooltip = memo(function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div
      className={`${C.bgWhite} border ${C.borderMedium} rounded-xs shadow-level1 px-3 py-2 text-sm`}
    >
      ...
      {payload.map((item) => {
        ...
        return (
          <p key={item.dataKey} style={{ color: item.color }} className="font-medium">
            {def?.label ?? item.name}: {item.value}
            {def?.unit ?? ""}
          </p>
        );
      })}
