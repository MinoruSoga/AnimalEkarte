...
describe("billing confirmation mutation authority", () => {
  it("does not forward a caller-supplied confirmed_by", async () => {
    ...
    server.use(
      http.post(
        "*/v1/medical-records/:id/billing-confirmation/confirm",
        async ({ request }) => {
    ...
    const { result } = renderHook(
      () => useCreateBillingConfirmation("42"),
      { wrapper: createTestWrapper() },
    );
    const legacyMutate = result.current.mutateAsync as unknown as (
      input: { confirmed_by: number; memo?: string },
    ) => Promise<unknown>;
  ...
  it("does not derive returned_by from a stale caller argument", async () => {
    ...
    server.use(
      http.post(
        "*/v1/medical-records/:id/billing-confirmation/return",
        async ({ request }) => {
          receivedBody = await request.json();
          return HttpResponse.json({ id: 1 });
        },
