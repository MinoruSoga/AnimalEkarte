...
export function usePetSubOwnersSection(petId: string, canEdit: boolean) {
  ...
  const [invalidRelationshipOwnerId, setInvalidRelationshipOwnerId] = useState<number | null>(null);
  const debouncedCandidateSearch = useDebouncedValue(candidateSearch, SUB_OWNER_SEARCH_DEBOUNCE_MS);
  const normalizedCandidateSearch = candidateSearch.trim();
  const normalizedDebouncedSearch = debouncedCandidateSearch.trim();
  const isCandidateSearchPending = normalizedCandidateSearch !== normalizedDebouncedSearch;
  const candidatesQuery = useGetSubOwnerCandidates(normalizedDebouncedSearch, canEdit);
  const rows = useMemo(
    () => draftRows ?? toEditableSubOwners(subOwnersQuery.data?.sub_owners ?? []),
  ...
  const candidateOptions = useMemo<SearchableSelectOption[]>(() => {
    ...
    return (candidatesQuery.data ?? [])
      .filter(
        (candidate) => candidate.ownerId !== primaryOwnerId && !selectedIds.has(candidate.ownerId),
      )
      .map((candidate) => ({
  ...
  const handleAddSubOwner = (value: string) => {
    ...
    const candidate = candidatesQuery.data?.find((item) => item.ownerId === ownerId);
  ...
  const handleRemoveSubOwner = (ownerId: number) => {
    setDraftRows((currentRows) => (currentRows ?? rows).filter((row) => row.ownerId !== ownerId));
    setInvalidRelationshipOwnerId(null);
    document.getElementById("pet-sub-owner-search")?.focus();
  };

  const [saveState, saveAction, isSavePending] = useActionState<SaveState, FormData>(async () => {
    if (subOwnersQuery.data === undefined || subOwnersQuery.error !== null) {
      return {
        kind: "error",
        message: "副飼主情報を取得できないため保存できません。再読み込みしてください。",
      };
    }
    ...
    if (metadataQuery.data === undefined) {
      return {
        kind: "error",
        message: "ペットの更新情報を取得できませんでした。再読み込みしてください。",
      };
    }
  ...
  const loadError = subOwnersQuery.error ?? metadataQuery.error ?? candidatesQuery.error;
