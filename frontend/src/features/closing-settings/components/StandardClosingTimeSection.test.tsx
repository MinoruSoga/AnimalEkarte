...
describe("StandardClosingTimeSection (#151 時間帯プレビュー)", () => {
  it("初期設定から平日・日曜の AM/PM/EMG レンジを表示する", () => {
    render(<StandardClosingTimeSection settings={makeSettings()} />);

    // AM は境界のみに依存し平日・日曜で共通 → 同一文字列が 2 セル
    expect(screen.getAllByText("00:00:00～11:59:59")).toHaveLength(2);
    ...
    expect(within(emgRow).getByText("18:30:00～23:59:59")).toBeInTheDocument();
    expect(within(emgRow).getByText("17:30:00～23:59:59")).toBeInTheDocument();
  });

  it("旧来の曖昧表示（区切り時刻のみ・レンジ無し）ではなく、明示的な開始〜終了レンジを表示する", () => {
    render(<StandardClosingTimeSection settings={makeSettings()} />);

    // 単一の時刻ではなく "HH:MM:SS～HH:MM:SS" 形式のレンジが存在することを固定（回帰防止）。
    // AM(2) + PM(2) + EMG(2) = 6 セル。
    const ranges = screen.getAllByText(/^\d{2}:\d{2}:\d{2}～\d{2}:\d{2}:\d{2}$/);
  ...
  it("境界時刻の編集でレンジが即時に再計算される（ライブプレビュー）", () => {
    ...
    fireEvent.change(boundaryInput, { target: { value: "09:00" } });

    // AM 終了が即時に 08:59:59 へ更新され、旧レンジは消える
    expect(screen.getAllByText("00:00:00～08:59:59")).toHaveLength(2);
    expect(screen.queryByText("00:00:00～11:59:59")).not.toBeInTheDocument();

    // PM 開始も 09:00:00 に追従
    const pmRow = screen.getByText("PM").closest("tr")!;
    expect(within(pmRow).getByText("09:00:00～18:29:59")).toBeInTheDocument();
  });
