      ...
      <div className={`${C.bgWhite} p-6 rounded-lg border ${C.borderLight} space-y-6`}>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <LotInput label="LOT 4" value={lot4} onChange={onLot4Change} onMarkDirty={onMarkDirty} />
          </div>
        </div>
...
function LotInput({ label, value, onChange, onMarkDirty }: LotInputProps) {
  return (
    <div className="space-y-1">
      <Label className={`text-xs ${C.text60}`}>{label}</Label>
      <Input
        value={value}
        onChange={(event) => {
          onMarkDirty();
