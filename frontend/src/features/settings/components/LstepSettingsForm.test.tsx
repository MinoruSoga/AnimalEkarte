...
function setupGetHandler(data: LstepSettingsResponse) {
  server.use(
    http.get(`/api/v1/clinics/${CLINIC_ID}/lstep-settings`, () => HttpResponse.json(data)),
  );
}
...
describe("LstepSettingsForm — A: Switch 表示・バッジ (FEAT-376)", () => {
  ...
  it('設定済み: "OFFにすると連携処理を一時停止します" の説明コピーが表示される', async () => {
    await renderAndWait(configuredSyncOn);
    expect(screen.getByText(/OFFにすると連携処理を一時停止します/)).toBeInTheDocument();
  });
});
...
describe("LstepSettingsForm — D: 同期無効化確認ダイアログ (FEAT-376)", () => {
  ...
  it("確認ダイアログの説明文が正しい", async () => {
    ...
    await user.click(screen.getByRole("switch", { name: "同期を有効にする" }));
    expect(
      screen.getByText("同期を無効にすると Lステップへのタグ付与が停止します。よろしいですか？"),
    ).toBeInTheDocument();
  });
...
describe("LstepSettingsForm — G: CPM V2 来院数閾値", () => {
  it("各閾値 input の defaultValue がサーバー値で表示される", async () => {
    ...
    expect(
      (screen.getByLabelText("CPM V2 来院数閾値 (出会い→良好)") as HTMLInputElement).value,
    ).toBe("3");
    expect(
      (screen.getByLabelText("CPM V2 来院数閾値 (良好→ファミリー)") as HTMLInputElement).value,
    ).toBe("6");
    expect(
      (screen.getByLabelText("CPM V2 来院数閾値 (ファミリー→NOAH)") as HTMLInputElement).value,
    ).toBe("10");
    expect((screen.getByLabelText("CPM V2 来院数閾値 (NOAH)") as HTMLInputElement).value).toBe(
      "15",
    );
  });
...
describe("LstepSettingsForm — H: CPM V1 判定閾値", () => {
  it("各閾値 input の defaultValue がサーバー値で表示される", async () => {
    ...
    expect(
      (screen.getByLabelText("CPM V1 スポット 単回最大金額 (円)") as HTMLInputElement).value,
    ).toBe("30000");
    expect(
      (screen.getByLabelText("CPM V1 グローイング 在籍最大日数") as HTMLInputElement).value,
    ).toBe("90");
  });
...
describe("LstepSettingsForm — I: 健診・予防タグ判定閾値", () => {
  it("各閾値 input の defaultValue がサーバー値で表示される", async () => {
    ...
    expect(
      (screen.getByLabelText("健診・来院履歴ルックバック日数") as HTMLInputElement).value,
    ).toBe("365");
