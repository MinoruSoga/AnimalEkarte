...
export function usePatientSelectionTable({
  ...
  const selectedPetIds = useMemo(() => new Set(selectedPets.map((p) => p.id)), [selectedPets]);
  ...
  return {
    ...
    selectedPetIds,
    selectionText: resolvePatientSelectionText(selectedPets.length, offPageSelectedCount),
    handlePageChange,
