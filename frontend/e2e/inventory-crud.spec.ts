...
test.describe('在庫管理 E2E', () => {
  ...
  test('/inventory/new — 在庫登録フォームが表示される', async () => {
    ...
    try {
      ...
      await expect(page.getByRole('heading', { name: '在庫登録' })).toBeVisible({ timeout: 15000 });
  ...
  test('/inventory — 検索フィルタが機能する', async () => {
    ...
    try {
      ...
      const searchInput = page.getByPlaceholder('品名、保管場所、仕入先...');
  ...
  test('/inventory — 在庫一覧から編集画面に遷移する', async () => {
    ...
    try {
      ...
      await expect(page.getByRole('heading', { name: '在庫編集' })).toBeVisible({ timeout: 15000 });
