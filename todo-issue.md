...
| [UAT-R2-EXCLUSIVE-LOCK](#uat-r2-exclusive-lock) | mock回帰追加済み / 子レコード4種別のstale更新CASはbackend実装済み / FE・handler配線は残る | 開発: [実施記録](docs/work/todo-ledger-20260922/UAT-R2-EXCLUSIVE-LOCK.md) のとおりversion送付配線（handler/DTO/OpenAPI/FE・codegen） | 実DB/2セッションはDB適用証拠と専用環境が必要。全面占有ロックは未採用 |
...
- **9月23日のコード照合で更新（午後に再更新）:** 子レコード4種別（治療/バイタル/処方/接種）のstale更新CASは **main 着地済み**（`95cfe63f6`・[実施記録](docs/work/todo-ledger-20260922/UAT-R2-EXCLUSIVE-LOCK.md)）。[migration 005](backend/migrations/005_child_records_version.sql)（4テーブルへ `version INTEGER NOT NULL DEFAULT 1`）と006はローカルDBへ適用済み（ユーザー `make migrate` 済み・STG/本番は未適用）。model 4種別へ `Version`（`gorm:"default:1"`）、[repository](backend/internal/medicalrecord/treatment_repository.go#L270) の optional CAS（expectedVersion不一致→Conflict正規化・成功時 `version+1`）、[service input](backend/internal/medicalrecord/prescription_service.go#L26) へ `Version *int`（nil=照合スキップの後方互換）、vaccination Update に確定済みカルテへのdraftガードを追加。生成 [models.ts](frontend/src/types/generated/models.ts) は version 対応済み（`841c44424`）。ただし request DTO・handler・OpenAPI には version がなく（本日 grep 実測・0件）、**HTTP経路ではCASが常にスキップ**される（明示設計「nil=スキップ」の後方互換）。Delete と [一括並べ替え](backend/internal/medicalrecord/treatment_repository.go#L328) はversion対象外。
...
- **次のローカル作業:** 上記のAPIごとの対応づけは実装で完了。残る配線は request/response DTO・handler・OpenAPI・FEへの `version` 送付（読取レスポンスの `version` を保存要求へ必須で載せる fail-closed、use-medical-record-save-action.ts と同方針）。生成types同期（`make codegen`）は済み（`841c44424`）。Delete・一括並べ替えのversion対象可否は別unitで検討。追加済みの合計/重複/遅延明細mock回帰は再作成しない。DB/2セッションの実行は [適用証拠](todo-operations.md#billing-schema-readiness) と専用環境待ち。
...
- **9月23日のコード照合で更新:** M4のカルテ表示回帰は `a95835cb9` で着地済み（[手入力FIXTURE-STRIPの結果値・単位・基準の表示と空machineの欠落扱い](frontend/src/features/medical-records/components/MedicalRecordExamination.test.tsx#L197)、[pet/record切替で直前行を残さない](frontend/src/features/medical-records/components/MedicalRecordExamination.test.tsx#L229)、[空machineを欠落・目視と混同しない](frontend/src/features/medical-records/components/ExaminationGroup.test.tsx#L80)）。実行票 [S28](docs/ops/testing/scenarios/S28-manual-urine-examination.md) も追加済み。残は医院承認の項目/凡例/単位/基準・origin表示の裁定と実機/DB受入。
- **source / 状態:** MANUAL-URINE。出典212–294の9月9日返信で敷島/猫は尿試験紙を目視、城東/八王子は機器測定と明示。手入力の受入設計票作成済み、検証は残件。
- **現行根拠・次の作業:** M4の回帰は `a95835cb9` で着地済み（再作成しない）。M1–M3/M5–M8の既存検証も再作成しない。originはFE変換でjob_idが落ちる範囲を明示して採用する表示を臨床POと決め、医院承認の項目/凡例/単位/基準・実機/DBの受入は [検証](todo-verification.md#ready8-followup-20260922) へ分ける。
- **完了 / PO・停止:** 各院の実際の項目/定性表現/単位/基準値について医院が承認した期待表と保存・表示の一致。基準値や陽性/陰性の臨床的意味は推定しない。不明項目は PO/検査担当へ確認し、機器測定結果と手入力結果の混同・上書きを防ぐ。

### SLACK-COMPLAINT

- **対応状況（2026-09-23 更新）:** **null再読込・カルテ切替の修正/回帰と、C0の実UI解除（clearable）まで実装済み・実機保存・再読込は未実行。** `cd2feaa14` の [hydrate](frontend/src/features/medical-records/hooks/use-apply-medical-record.ts#L47) は `chiefComplaintTypeId ?? null` をsetterへ渡す。空欄payload/本文保持・空値callback・再読込/記録切替の [回帰](frontend/src/features/medical-records/hooks/use-apply-medical-record.test.ts) を追加済み。`a95835cb9` で [SearchableSelectにclearable](frontend/src/components/ui/searchable-select.tsx#L51) を追加し、[主訴区分セレクトへ配線](frontend/src/features/medical-records/components/InterviewChiefComplaint.tsx#L99) 済み。[UIテスト](frontend/src/features/medical-records/components/InterviewChiefComplaint.test.tsx#L61) は実clear affordanceで空値→null setter を検証。[調査票](docs/work/todo-campaign-20260919-ready17/SLACK-COMPLAINT.md) の状態は「C0 clearable 実装済み／C3 hydrate 先行修正済み／N・C・reload 回帰 PASS／実機保存・再読込 未実行」。修正済みC3を再実装せず、課題全体は閉じない。
...
| ID | 状態・次の一手 | 担当・ローカル成果物 | 本体の開始条件 |
|---|---|---|---|
| [UAT-Q3-GENDER-MAP](#uat-q3-gender-map) | 対応表・STG訂正ドラフト作成済み / hachioji 1院は統合revisionと一致、3院は修正前mapping世代でbundle再生成対象 | 運用: [訂正ドラフト](docs/work/todo-ledger-20260922/UAT-Q3-GENDER-MAP-CORRECTION-DRAFT.md) のoperator承認と3院bundle再生成 | 再統合は不要。bundle/STG は [運用計画](todo-operations.md#uat-q3-gender-map) |
...
**9月23日のコード照合で更新:** [対応表](docs/work/todo-ledger-20260922/UAT-Q3-GENDER-MAP.md) を作成済み（読取専用照合）。old_db main HEAD `b941a9c80`。統合済みrevision（`5fbc3b2`／merge `a2cea37`）と現行4院bundleの対応は **hachioji 1院のみ一致、jouto/shikishima/hakobuneco の3院は修正前mapping世代**。後続の [STG限定訂正票ドラフト](docs/work/todo-ledger-20260922/UAT-Q3-GENDER-MAP-CORRECTION-DRAFT.md) も完成（operator承認待ち・ドラフトの作成はSTG/DB変更の承認ではない）。3院の再取り込みに備えたcross-clinic staff統合は `0c91f5dc9`、ローカルrehearsal経路のCSV契約digest不一致は `d34e09df7` で解消済み（[契約差分票](docs/work/todo-ledger-20260922/BUG-LOCAL-HANDOFF-CSV-CONTRACT.md)。formal F6はREHEARSAL_ONLYでBLOCKEDのまま）。PostgreSQL検証receiptの取得前提だった [stg-uat-staff-attach のRLS bypass](backend/cmd/stg-uat-staff-attach/main.go#L183) は PR #431・`6063f58a0`（timeout flag追加）で main 着地済み。
