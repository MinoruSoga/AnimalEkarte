...
- **9月23日のコード照合で更新:** 子レコード4種別（治療/バイタル/処方/接種）のstale更新CASを実装済み・**未コミット**（[実施記録](docs/work/todo-ledger-20260922/UAT-R2-EXCLUSIVE-LOCK.md)）。[migration 005](backend/migrations/005_child_records_version.sql) は4テーブルへ `version INTEGER NOT NULL DEFAULT 1` を追加（**未適用**・適用はユーザー `make migrate`）。model 4種別へ `Version`（`gorm:"default:1"`）、[repository](backend/internal/medicalrecord/treatment_repository.go#L270) の optional CAS（expectedVersion不一致→Conflict正規化・成功時 `version+1`）、[service input](backend/internal/medicalrecord/prescription_service.go#L26) へ `Version *int`（nil=照合スキップの後方互換）、vaccination Update に確定済みカルテへのdraftガードを追加。scoped test GREEN（9/22、`-run "OptimisticLock|ExpectedVersion|ConcurrentUpdatesSerialize|FinalizedMedicalRecordRejected"` は PASS の記録）。ただし request DTO・handler・OpenAPI・FE生成typesにversionはなく、**HTTP経路ではCASが常にスキップ**される（明示設計「nil=スキップ」の後方互換）。Delete と [一括並べ替え](backend/internal/medicalrecord/treatment_repository.go#L328) はversion対象外。
...
- **次のローカル作業:** 上記のAPIごとの対応づけは実装で完了。残る配線は request/response DTO・handler・OpenAPI・FEへの `version` 送付（読取レスポンスの `version` を保存要求へ必須で載せる fail-closed、use-medical-record-save-action.ts と同方針）と、`make codegen` による生成types同期。Delete・一括並べ替えのversion対象可否は別unitで検討。追加済みの合計/重複/遅延明細mock回帰は再作成しない。DB/2セッションの実行は [適用証拠](todo-operations.md#billing-schema-readiness) と専用環境待ち。
