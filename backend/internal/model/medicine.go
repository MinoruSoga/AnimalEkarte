package model

import (
	"time"

	"gorm.io/gorm"
)

type DosageForm string

const (
	DosageFormTablet    DosageForm = "tablet"
	DosageFormLiquid    DosageForm = "liquid"
	DosageFormInjection DosageForm = "injection"
	DosageFormTopical   DosageForm = "topical"
	DosageFormPowder    DosageForm = "powder"
)

type MedicineUnit string

const (
	MedicineUnitPerTablet MedicineUnit = "per_tablet"
	MedicineUnitPerML     MedicineUnit = "per_ml"
	MedicineUnitPerDose   MedicineUnit = "per_dose"
	MedicineUnitPerGram   MedicineUnit = "per_gram"
)

type Medicine struct {
	ID              uint64        `gorm:"primaryKey;autoIncrement"                       json:"id"`
	ClinicID        uint64        `gorm:"not null"                                       json:"clinic_id"`
	Name            string        `gorm:"not null"                                       json:"name"`
	ParentID        *uint64       `gorm:"column:parent_id"                               json:"parent_id,omitempty"`
	Price           *int64        `gorm:"type:bigint"                                    json:"price,omitempty"`
	IsActive        bool          `gorm:"default:true"                                   json:"is_active"`
	Description     string        `gorm:"default:''"                                     json:"description"`
	DosageForm      *DosageForm   `gorm:"type:dosage_form"                               json:"dosage_form,omitempty"`
	MedicineUnit    *MedicineUnit `gorm:"type:medicine_unit"                             json:"medicine_unit,omitempty"`
	InventoryID     *uint64       `                                                      json:"inventory_id,omitempty"`
	DefaultQuantity float64       `gorm:"type:numeric(10,2);default:1"                   json:"default_quantity"`
	TaxType         TaxType       `gorm:"type:tax_type;not null;default:excluded"        json:"tax_type"`
	TaxRate         float64       `gorm:"type:numeric;not null;default:0.10"             json:"tax_rate"`
	SortOrder       int           `gorm:"type:integer;default:0"                         json:"sort_order"`
	IsNonInsurance  bool          `gorm:"not null;default:false"                         json:"is_non_insurance"`

	// #201 投与量自動計算（製品軸パラメータ）。calculation_type=none（既定）で従来通り手動。
	CalculationType     MedicineCalculationType `gorm:"type:medicine_calculation_type;not null;default:none" json:"calculation_type"`
	Strength            *float64                `gorm:"type:numeric(10,4)"                                   json:"strength,omitempty"`              // 製品含量 mg/単位。分母は medicine_unit で解釈
	FrequencyPerDay     *int                    `gorm:"type:integer"                                         json:"frequency_per_day,omitempty"`     // 1日投与回数（per_day 按分）
	DefaultDurationDays *int                    `gorm:"type:integer"                                         json:"default_duration_days,omitempty"` // 既定投与日数

	CreatedAt time.Time      `gorm:"autoCreateTime" json:"created_at"`
	UpdatedAt time.Time      `gorm:"autoUpdateTime" json:"updated_at"`
	DeletedAt gorm.DeletedAt `                      json:"-"`

	// Relations
	Inventory  *InventoryItem      `gorm:"foreignKey:InventoryID" json:"inventory,omitempty"`
	DoseParams []MedicineDoseParam `gorm:"foreignKey:MedicineID"  json:"dose_params,omitempty"` // #201 種別計算パラメータ
}

func (Medicine) TableName() string { return "medicines" }
