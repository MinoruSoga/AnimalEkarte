...
func TestAuth_StaffValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:              "notifies exactly once and continues for bad database connection",
		...
		{
			name:              "continues when temporary failure notification fails",
			staffErr:          temporaryStorageErr,
		...
		{
			name:              "notifies and continues for PostgreSQL cannot-connect-now",
