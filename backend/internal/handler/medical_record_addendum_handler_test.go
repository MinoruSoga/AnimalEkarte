...
type mockMedicalRecordAddendumService struct {
	createFn                func(ctx context.Context, clinicID uint64, input service.CreateMedicalRecordAddendumInput) (*model.MedicalRecordAddendum, error)
	findByMedicalRecordIDFn func(ctx context.Context, clinicID, medicalRecordID uint64) ([]*model.MedicalRecordAddendum, error)
}

func (m *mockMedicalRecordAddendumService) Create(ctx context.Context, clinicID uint64, input service.CreateMedicalRecordAddendumInput) (*model.MedicalRecordAddendum, error) {
	if m.createFn != nil {
...
func newHandlerWithAddendumSvc(svc service.MedicalRecordAddendumService) *Handler {
	return &Handler{
		svc: &service.Services{
			MedicalRecordAddendum: svc,
		},
	}
}
...
func TestCreateMedicalRecordAddendum(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			body: validBody,
			svc: &mockMedicalRecordAddendumService{
				createFn: func(_ context.Context, clinicID uint64, input service.CreateMedicalRecordAddendumInput) (*model.MedicalRecordAddendum, error) {
					assert.Equal(t, uint64(1), clinicID)
		...
		},
		{
			...
			body:     validBody,
			svc: &mockMedicalRecordAddendumService{
				createFn: func(_ context.Context, _ uint64, _ service.CreateMedicalRecordAddendumInput) (*model.MedicalRecordAddendum, error) {
