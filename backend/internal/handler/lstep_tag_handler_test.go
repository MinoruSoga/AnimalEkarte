...
func (m *mockLstepTagService) GetOwnerTags(ctx context.Context, clinicID, ownerID uint64) (*service.OwnerTagsResult, error) {
	...
	return &service.OwnerTagsResult{Tags: []string{}}, nil
}
...
func newHandlerWithLstepTagSvc(svc service.LstepTagService) *Handler {
	return &Handler{svc: &service.Services{LstepTag: svc}}
}

func newGetOwnerLstepTagsRouter(svc service.LstepTagService, withClinicID bool) *gin.Engine {
	r := gin.New()
...
func newPostOwnerLstepTagRouter(svc service.LstepTagService, withClinicID bool) *gin.Engine {
	r := gin.New()
	h := newHandlerWithLstepTagSvc(svc)
	if withClinicID {
...
func newDeleteOwnerLstepTagRouter(svc service.LstepTagService, withClinicID bool) *gin.Engine {
	r := gin.New()
	h := newHandlerWithLstepTagSvc(svc)
	if withClinicID {
...
func TestGetOwnerLstepTags(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			ownerID: "1",
			svc: &mockLstepTagService{
				getOwnerTagsFn: func(_ context.Context, _, _ uint64) (*service.OwnerTagsResult, error) {
					return &service.OwnerTagsResult{IsLinked: false, Tags: []string{}}, nil
				},
			},
			wantStatus: http.StatusOK,
		},
