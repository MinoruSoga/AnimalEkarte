...
// pet一覧 (toPetListResponse)・owner ネスト (toPetInOwnerResponse) の全経路で
// byte 一致のシリアライズを返すことを保証する (SC1)。
//
...
func TestPetDateFieldsSerializeIdenticallyAcrossEndpoints(t *testing.T) {
	...
	// owner ネストを基準にせず全 3 経路を独立に同一の期待値と照合するため、
	// 仮に toPetInOwnerResponse が将来退行しても本テストが検出する。
	...
	for _, p := range []struct {
		...
		{"ownerネスト(toPetInOwnerResponse)", nested.BirthDate, nested.NeuteredDate, nested.LastVisit},
	} {
...
func TestPetBirthDateCalendarDateNeverRolls(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NotNil(t, nested.BirthDate)
			...
			assert.Equal(t, tc.wantDate, nested.BirthDate.Format("2006-01-02"),
				"owner ネストのカレンダー日付がずれてはならない")
		})
	}
}
