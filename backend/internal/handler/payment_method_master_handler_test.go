...
func newHandlerWithPaymentMethodSvc(svc service.PaymentMethodMasterService) *Handler {
	return &Handler{
		svc: &service.Services{PaymentMethodMaster: svc},
	}
}
...
func TestPaymentMethodMasterHandler_Delete(t *testing.T) {
	...
	tests := []struct {
		name        string
		paramID     string
		setupRouter func(svc service.PaymentMethodMasterService) *gin.Engine
		svc         *mockPaymentMethodMasterService
		wantStatus  int
	}{
		{
...
func newReorderPaymentMethodRouter(svc service.PaymentMethodMasterService) *gin.Engine {
	r := gin.New()
