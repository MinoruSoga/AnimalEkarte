...
func TestToExaminationTypeResponse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !got.CreatedAt.Equal(localTime(tt.et.CreatedAt)) {
				t.Errorf("CreatedAt = %v, want %v", got.CreatedAt, localTime(tt.et.CreatedAt))
			}
			if !got.UpdatedAt.Equal(localTime(tt.et.UpdatedAt)) {
				t.Errorf("UpdatedAt = %v, want %v", got.UpdatedAt, localTime(tt.et.UpdatedAt))
			}
		})
	}
}
