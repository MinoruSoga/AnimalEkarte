...
//  3. クエリパラメータ → service.ListOwnerAggregationInput への変換
...
func newHandlerWithAggregationSvc(svc service.AggregationService) *Handler {
	return &Handler{svc: &service.Services{Aggregation: svc}}
}
...
func TestListOwnerAggregation_ResponseShape(t *testing.T) {
	...
	svc := &mockAggregationService{
		listFn: func(_ context.Context, _ uint64, _ *service.ListOwnerAggregationInput) (*service.ListOwnerAggregationResult, error) {
			return &service.ListOwnerAggregationResult{
				Total:   1,
				Page:    1,
				PerPage: 50,
				Items: []service.OwnerAggregationItem{
					{
						OwnerID:            1,
						OwnerName:          "山田 太郎",
						TotalAmount:        156000,
...
// service.ListOwnerAggregationInput に正しくマップされることを検証する。
func TestListOwnerAggregation_QueryToInput(t *testing.T) {
	gin.SetMode(gin.TestMode)

	t.Run("all parameters mapped", func(t *testing.T) {
		var captured *service.ListOwnerAggregationInput
		svc := &mockAggregationService{
			listFn: func(_ context.Context, clinicID uint64, in *service.ListOwnerAggregationInput) (*service.ListOwnerAggregationResult, error) {
	...
	t.Run("default values when params omitted", func(t *testing.T) {
		var captured *service.ListOwnerAggregationInput
		svc := &mockAggregationService{
			listFn: func(_ context.Context, _ uint64, in *service.ListOwnerAggregationInput) (*service.ListOwnerAggregationResult, error) {
				captured = in
				return &service.ListOwnerAggregationResult{}, nil
			},
		}
...
func TestListOwnerAggregation_EmptyResult(t *testing.T) {
	...
	svc := &mockAggregationService{
		listFn: func(_ context.Context, _ uint64, _ *service.ListOwnerAggregationInput) (*service.ListOwnerAggregationResult, error) {
			return &service.ListOwnerAggregationResult{Total: 0, Page: 1, PerPage: 50, Items: nil}, nil
		},
	}
	h := newHandlerWithAggregationSvc(svc)
	w := httptest.NewRecorder()
