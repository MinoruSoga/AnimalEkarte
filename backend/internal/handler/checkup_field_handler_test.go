...
type mockCheckupFieldResultService struct {
	...
	replaceForCheckupFn func(ctx context.Context, clinicID, medicalRecordID, checkupID uint64, actorID *uint64, inputs []service.UpsertCheckupFieldResultInput) ([]model.CheckupFieldResult, error)
}
...
func (m *mockCheckupFieldResultService) ReplaceForCheckup(ctx context.Context, clinicID, medicalRecordID, checkupID uint64, actorID *uint64, inputs []service.UpsertCheckupFieldResultInput) ([]model.CheckupFieldResult, error) {
	return m.replaceForCheckupFn(ctx, clinicID, medicalRecordID, checkupID, actorID, inputs)
}

func newHandlerWithCheckupFieldResultSvc(svc service.CheckupFieldResultService) *Handler {
	return &Handler{svc: &service.Services{CheckupFieldResult: svc}}
}
...
func TestReplaceCheckupFieldResults(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupCtx: func(c *gin.Context) { setClinicID(c) },
			svc: &mockCheckupFieldResultService{
				replaceForCheckupFn: func(_ context.Context, clinicID, medicalRecordID, checkupID uint64, actorID *uint64, inputs []service.UpsertCheckupFieldResultInput) ([]model.CheckupFieldResult, error) {
					assert.Equal(t, uint64(1), clinicID)
					assert.Equal(t, uint64(1), medicalRecordID)
					assert.Equal(t, uint64(2), checkupID)
		...
		},
		{
			...
			setupCtx:  func(c *gin.Context) { setClinicID(c) },
			svc: &mockCheckupFieldResultService{
				replaceForCheckupFn: func(_ context.Context, _, _, _ uint64, _ *uint64, _ []service.UpsertCheckupFieldResultInput) ([]model.CheckupFieldResult, error) {
					return nil, fmt.Errorf("db failure")
				},
			},
			wantStatus: http.StatusInternalServerError,
