...
type mockLstepTriggerPriorityService struct {
	...
	updatePrioritiesFn func(ctx context.Context, clinicID uint64, input service.UpdateTriggerPrioritiesInput) error
...
func (m *mockLstepTriggerPriorityService) UpdatePriorities(ctx context.Context, clinicID uint64, input service.UpdateTriggerPrioritiesInput) error {
...
func newHandlerWithTriggerPrioritySvc(svc service.LstepTriggerPriorityService) *Handler {
	return &Handler{svc: &service.Services{LstepTriggerPriority: svc}}
}

func newGetTriggerPrioritiesRouter(svc service.LstepTriggerPriorityService, withClinicID bool) *gin.Engine {
	r := gin.New()
...
func newPatchTriggerPrioritiesRouter(svc service.LstepTriggerPriorityService, withClinicID bool) *gin.Engine {
	r := gin.New()
	h := newHandlerWithTriggerPrioritySvc(svc)
	if withClinicID {
...
func TestUpdateLstepTriggerPriorities(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			svc: &mockLstepTriggerPriorityService{
				updatePrioritiesFn: func(_ context.Context, _ uint64, _ service.UpdateTriggerPrioritiesInput) error {
