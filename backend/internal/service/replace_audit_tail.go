...
// 一箇所に集約する（BE-refactor.md E-6）。汎用 replace エンジンは対象モデルが異なるため作らず、
// 監査テール（actor-type 導出 → LogEntryTx → 失敗時ロールバック）のみを共有する。
// deletedCount<=0（純粋な新規挿入のみで削除を伴わない置換）の場合は何もせず nil を返す。
// logMsg/wrapMsg/idFieldName は各呼び出し元の既存文言・ログキーをそのまま渡す
...
func logReplaceDeletionTx(txCtx context.Context, auditTx AuditTxLogger, clinicID uint64, actorID *uint64,
	...
	if err := auditTx.LogEntryTx(txCtx, &AuditLogInput{
		ClinicID:   &clinicID,
		ActorID:    actorID,
		ActorType:  actorType,
		Action:     action,
