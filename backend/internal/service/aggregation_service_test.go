...
func (m *mockLtvRepository) FindOwnerLTV(_ context.Context, _ *repository.FindOwnerLTVParams) ([]repository.OwnerLTVRow, error) {
	if m.err != nil {
		return nil, m.err
	}
	out := make([]repository.OwnerLTVRow, len(m.rows))
	copy(out, m.rows)
	return out, nil
}

// intPtr / strPtr は *int / *string ヘルパ。
...
func TestListOwnerAggregation_Pagination(t *testing.T) {
	...
	rows := make([]repository.OwnerLTVRow, 5)
	for i := range rows {
		v := int64(100 + i)
...
func TestListOwnerAggregation_InputForwarding(t *testing.T) {
	var captured *repository.FindOwnerLTVParams
	repoMock := &mockLtvRepositoryCapture{
		fn: func(_ context.Context, p *repository.FindOwnerLTVParams) ([]repository.OwnerLTVRow, error) {
			captured = p
			return nil, nil
		},
	}
...
type mockLtvRepositoryCapture struct {
	fn func(ctx context.Context, p *repository.FindOwnerLTVParams) ([]repository.OwnerLTVRow, error)
}

func (m *mockLtvRepositoryCapture) FindOwnerLTV(ctx context.Context, p *repository.FindOwnerLTVParams) ([]repository.OwnerLTVRow, error) {
	if m.fn != nil {
		return m.fn(ctx, p)
	}
	return nil, nil
