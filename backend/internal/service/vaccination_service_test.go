...
type mockVaccinationRepository struct {
	findAllFn  func(ctx context.Context, petID *uint64, ownerID *uint64, page, limit int) ([]model.Vaccination, int64, error)
	findByIDFn func(ctx context.Context, id uint64) (*model.Vaccination, error)
	createFn   func(ctx context.Context, vaccination *model.Vaccination) error
	updateFn   func(ctx context.Context, vaccination *model.Vaccination) error
	deleteFn   func(ctx context.Context, id uint64) error
}

func (m *mockVaccinationRepository) FindAll(ctx context.Context, petID *uint64, ownerID *uint64, page, limit int) ([]model.Vaccination, int64, error) {
	return m.findAllFn(ctx, petID, ownerID, page, limit)
}

func (m *mockVaccinationRepository) FindByID(ctx context.Context, id uint64) (*model.Vaccination, error) {
...
func (m *mockVaccinationRepository) Update(ctx context.Context, vaccination *model.Vaccination) error {
	return m.updateFn(ctx, vaccination)
}

func (m *mockVaccinationRepository) Delete(ctx context.Context, id uint64) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			repo := &mockVaccinationRepository{
				findAllFn: func(_ context.Context, petID *uint64, ownerID *uint64, _, _ int) ([]model.Vaccination, int64, error) {
					capturedPetID = petID
			...
			vaccinations, total, err := svc.List(context.Background(), tt.petID, tt.ownerID, tt.page, tt.limit)
...
func TestVaccinationService_GetByID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := &mockVaccinationRepository{
				findByIDFn: func(_ context.Context, _ uint64) (*model.Vaccination, error) {
					return tt.repoVaccination, tt.repoErr
				},
			}
			svc := NewVaccinationService(repo)

			vaccination, err := svc.GetByID(context.Background(), tt.id)
...
func TestVaccinationService_GetByID_NotFound(t *testing.T) {
	...
	vaccination, err := svc.GetByID(context.Background(), 999)
...
func TestVaccinationService_Update(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := &mockVaccinationRepository{
				updateFn: func(_ context.Context, _ *model.Vaccination) error {
					return tt.repoErr
				},
			}
			svc := NewVaccinationService(repo)

			err := svc.Update(context.Background(), tt.vaccination)
