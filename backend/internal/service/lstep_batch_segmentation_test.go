...
func newSegBatchService(
	...
	return NewLstepBatchService(&batchMockReservationRepo{}, tagSvc, clinicRepo, medRepo, auditSvc, settingsSvc, nil)
}
...
func TestRunVisitDormantSyncAllClinics_Success(t *testing.T) {
	medRepo := &batchMockMedRecordRepo{
		findDormantFn: func(_ context.Context, _ uint64, minDays int) ([]repository.DormantOwnerEntry, error) {
			assert.Equal(t, 120, minDays, "VISIT_* バッチは120日閾値を使う")
			return []repository.DormantOwnerEntry{
...
func TestRunVisitDormantSyncAllClinics_PartialSyncErrorsStillSucceeds(t *testing.T) {
	medRepo := &batchMockMedRecordRepo{
		findDormantFn: func(_ context.Context, _ uint64, _ int) ([]repository.DormantOwnerEntry, error) {
			return []repository.DormantOwnerEntry{{OwnerID: 1, DaysSince: 200}, {OwnerID: 2, DaysSince: 250}}, nil
		},
	}
	tagSvc := newSegTagSyncSvc()
	tagSvc.syncVisitDormantTagsFn = func(_ context.Context, _, ownerID uint64, _ int) error {
