...
type mockDeliveryMonitorTriggerLogRepo struct {
	...
	suppressedErr        error
	findByDateRangeFn    func(ctx context.Context, clinicID uint64, from, to time.Time, triggerType, status string, limit, offset int) ([]repository.DeliveryTriggerLogRow, int64, error)
}
...
func (m *mockDeliveryMonitorTriggerLogRepo) FindByDateRangeWithFilters(ctx context.Context, clinicID uint64, from, to time.Time, triggerType, status string, limit, offset int) ([]repository.DeliveryTriggerLogRow, int64, error) {
...
func TestLstepDeliveryMonitorService_GetLogs(t *testing.T) {
	...
	t.Run("applies default page/perPage and maps rows", func(t *testing.T) {
		...
		repo := &mockDeliveryMonitorTriggerLogRepo{
			findByDateRangeFn: func(_ context.Context, _ uint64, _, _ time.Time, _, _ string, limit, offset int) ([]repository.DeliveryTriggerLogRow, int64, error) {
				capturedLimit = limit
				capturedOffset = offset
				return []repository.DeliveryTriggerLogRow{
					{
	...
	t.Run("clamps perPage above 100 down to 100", func(t *testing.T) {
		...
		repo := &mockDeliveryMonitorTriggerLogRepo{
			findByDateRangeFn: func(_ context.Context, _ uint64, _, _ time.Time, _, _ string, limit, _ int) ([]repository.DeliveryTriggerLogRow, int64, error) {
				capturedLimit = limit
				return nil, 0, nil
			},
		}
	...
	t.Run("computes offset for page > 1", func(t *testing.T) {
		...
		repo := &mockDeliveryMonitorTriggerLogRepo{
			findByDateRangeFn: func(_ context.Context, _ uint64, _, _ time.Time, _, _ string, _, offset int) ([]repository.DeliveryTriggerLogRow, int64, error) {
				capturedOffset = offset
				return nil, 0, nil
			},
		}
	...
	t.Run("propagates repo error wrapped", func(t *testing.T) {
		repo := &mockDeliveryMonitorTriggerLogRepo{
			findByDateRangeFn: func(_ context.Context, _ uint64, _, _ time.Time, _, _ string, _, _ int) ([]repository.DeliveryTriggerLogRow, int64, error) {
