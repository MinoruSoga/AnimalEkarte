...
func TestImportFriendAttributesCSV_Integration_HappyPath(t *testing.T) {
	...
	svc := NewLstepCsvImportService(
		db,
		repository.NewLstepCsvImportRepository(db),
		repository.NewLstepFriendAttributeSnapshotRepository(db),
...
func getLstepCsvImportTestDatabaseConnection(t *testing.T) *gorm.DB {
	...
	testDBName := fmt.Sprintf("%s_lstep_csv_import_%s_%d", dbName, sanitizedTestName, time.Now().UnixNano())
