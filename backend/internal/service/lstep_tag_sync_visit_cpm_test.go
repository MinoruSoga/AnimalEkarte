...
type cpmAccountingRepositoryWrapper struct {
	*mockAccountingRepository
	...
	findOwnersByAnnualRevenueFn   func(ctx context.Context, clinicID uint64) ([]repository.OwnerAnnualRevenue, error)
}
...
func (m *cpmAccountingRepositoryWrapper) FindOwnersByAnnualRevenue(ctx context.Context, clinicID uint64) ([]repository.OwnerAnnualRevenue, error) {
...
func TestSyncCPMStageTagV2(t *testing.T) {
	...
	t.Run("removes old V2 stage tags and adds the newly-calculated one", func(t *testing.T) {
		...
		svc := &lstepTagSyncService{
			...
			medRecordRepo: &mockMedicalRecordRepository{
				findOwnerVisitSummaryFn: func(_ context.Context, _, _ uint64) (*repository.OwnerVisitSummary, error) {
					return &repository.OwnerVisitSummary{TotalCount: 0}, nil
				},
			},
			tagCacheRepo:  &mockLstepTagCacheRepository{},
			buildClientFn: func(_ context.Context, _ uint64) (lstep.Client, error) { return client, nil },
