...
func (r *ownerRepository) FindByID(ctx context.Context, clinicID, id uint64) (*model.Owner, error) {
	...
	err := r.db.WithContext(ctx).Preload("Pets", "deleted_at IS NULL").Preload("Pets.AnimalSpecies").Preload("Pets.Insurance", "deleted_at IS NULL").Scopes(clinicScope(clinicID)).Where("id = ?", id).First(&owner).Error
	if err != nil {
