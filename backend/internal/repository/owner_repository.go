...
type OwnerRepository interface {
	FindAll(ctx context.Context, page, limit int, search string) ([]model.Owner, int64, error)
...
func (r *ownerRepository) FindAll(ctx context.Context, page, limit int, search string) ([]model.Owner, int64, error) {
	var owners []model.Owner
	var total int64

	q := r.db.WithContext(ctx).Model(&model.Owner{})
	if search != "" {
...
func (r *ownerRepository) Delete(ctx context.Context, id uuid.UUID) error {
	if err := r.db.WithContext(ctx).Delete(&model.Owner{}, "id = ?", id).Error; err != nil {
		return apperrors.Wrap(err, "delete owner")
	}
	return nil
}
