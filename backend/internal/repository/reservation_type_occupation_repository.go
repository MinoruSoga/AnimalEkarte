...
type ReservationTypeOccupationRepository interface {
	...
	// CountByStaffID は指定日に対応職種のスタッフが何人出勤しているかを返す（LIFF 専用）
	// shift_type が 'off' / 'paid_leave' 以外のスタッフのみカウント
	CountByStaffID(ctx context.Context, clinicID, reservationTypeID uint64, date time.Time) (int64, error)
}
...
func (r *reservationTypeOccupationRepository) CountByStaffID(
