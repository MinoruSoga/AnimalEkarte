...
//
// 移動元: diagnosis_repository_test.go の TestDiagnosisNameRepository_* 群（BE8-4 batch28）。
// makeDiagnosisTypeMaster/makeDiagnosisNameRec はフラット package
// (master_preload_clinic_isolation_test.go) の同名ヘルパーの複製（diagnosistype/repository_test.go
// と同じ複製・BE8-4: import cycle を避けるための最小限の重複、移動時の型リネームはしない方針の対象外）。
// makeSpeciesAndPet/makeHistoryMedicalRecord は consultation/checkup 等と同様の局所複製。
//
// 保護する不変条件:
