...
func TestLstepTagCacheRepository_BulkReplaceOwnerTags(t *testing.T) {
	db := setupLstepTagCacheTestDB(t)
	repo := NewLstepTagCacheRepository(db)
	ctx := context.Background()

	t.Run("既存タグを削除し新しいタグ一覧で置き換える", func(t *testing.T) {
		require.NoError(t, repo.UpsertTag(ctx, 1, 800, "old-tag-1", "auto", ""))
		require.NoError(t, repo.UpsertTag(ctx, 1, 800, "old-tag-2", "auto", ""))

		require.NoError(t, repo.BulkReplaceOwnerTags(ctx, 1, 800, []TagEntry{
			{TagName: "new-tag-1", Category: "manual", Reason: "reason-1"},
			{TagName: "new-tag-2", Category: "manual", Reason: ""},
		}))

		records, err := repo.FindByOwner(ctx, 1, 800)
		require.NoError(t, err)
		require.Len(t, records, 2)
		...
		assert.ElementsMatch(t, []string{"new-tag-1", "new-tag-2"}, names)
	})

	t.Run("空スライスを渡すと全タグが削除されるのみ", func(t *testing.T) {
		require.NoError(t, repo.UpsertTag(ctx, 1, 801, "tag-to-clear", "auto", ""))
		require.NoError(t, repo.BulkReplaceOwnerTags(ctx, 1, 801, []TagEntry{}))

		records, err := repo.FindByOwner(ctx, 1, 801)
		require.NoError(t, err)
		assert.Empty(t, records)
	})

	t.Run("他の飼い主のタグには影響しない", func(t *testing.T) {
		require.NoError(t, repo.UpsertTag(ctx, 1, 802, "untouched-tag", "auto", ""))
		require.NoError(t, repo.BulkReplaceOwnerTags(ctx, 1, 803, []TagEntry{{TagName: "new", Category: "manual"}}))

		records, err := repo.FindByOwner(ctx, 1, 802)
		require.NoError(t, err)
		require.Len(t, records, 1)
		assert.Equal(t, "untouched-tag", records[0].TagName)
	})
}
