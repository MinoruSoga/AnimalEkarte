...
//
// 移動元: diagnosis_repository_test.go の TestDiagnosisTypeRepository_* 群（BE8-4 batch27）。
// makeDiagnosisTypeMaster/makeDiagnosisNameRec はフラット package
// (master_preload_clinic_isolation_test.go) の同名ヘルパーの複製（BE8-4: import cycle を
// 避けるための最小限の重複、移動時の型リネームはしない方針の対象外）。
//
// TestDiagnosisTypeRepository_CountChildrenByParentID はフラット側に意図的に残置する
// （BE8-4 batch27 accepted deviation）: そのテストのソフトデリート検証サブテストが
// repository.NewDiagnosisNameRepository(db) を直接呼ぶため、ここに移すと
// diagnosistype → repository への逆依存が発生し、repository → diagnosistype
// （facade）との import cycle になる。DiagnosisName 側は batch28 まで repository
// パッケージに残るため、この 1 テストのみ move-not-modify の例外としてフラット側に残す
// （facade 経由で同一実装をテストしており、カバレッジの欠落はない）。
//
// 保護する不変条件:
