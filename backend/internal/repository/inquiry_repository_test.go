...
// medical_records は setupTestDB のコア AutoMigrate で既に用意済みのため再定義しない。
...
func TestInquiryRepository_CountByChiefComplaintTypeID(t *testing.T) {
	db := setupInquiryTestDB(t)
	repo := NewInquiryRepository(db)
	ctx := context.Background()
	const clinicA, clinicB = uint64(1), uint64(2)

	// chief_complaint_type_id は fk_inquiries_chief_complaint_type で実在の chief_complaint_types.id を
	// 要求するため、存在しないハードコードIDではなく実際に作成した行のIDを使う。
	category := makeChiefComplaintType(t, db, clinicA, "テストカテゴリ")
	categoryID := category.ID

	mrA := makeInquiryMedicalRecord(t, db, clinicA, "MR-A-001")
	mrB := makeInquiryMedicalRecord(t, db, clinicB, "MR-B-001")

	t.Run("参照が無ければ 0", func(t *testing.T) {
		count, err := repo.CountByChiefComplaintTypeID(ctx, clinicA, categoryID)
		require.NoError(t, err)
		assert.Equal(t, int64(0), count)
	})

	catID := categoryID
	_, err := repo.SaveByMedicalRecordID(ctx, clinicA, &model.Inquiry{MedicalRecordID: mrA.ID, ChiefComplaintTypeID: &catID})
	require.NoError(t, err)
	_, err = repo.SaveByMedicalRecordID(ctx, clinicB, &model.Inquiry{MedicalRecordID: mrB.ID, ChiefComplaintTypeID: &catID})
	require.NoError(t, err)

	t.Run("clinic_id で隔離してカウントする（別クリニックの参照は含まない）", func(t *testing.T) {
		count, err := repo.CountByChiefComplaintTypeID(ctx, clinicA, categoryID)
		require.NoError(t, err)
		assert.Equal(t, int64(1), count)
	})

	t.Run("別カテゴリIDでは 0", func(t *testing.T) {
		count, err := repo.CountByChiefComplaintTypeID(ctx, clinicA, 999999)
		require.NoError(t, err)
		assert.Equal(t, int64(0), count)
	})
}
