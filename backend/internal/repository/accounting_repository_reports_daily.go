...
func (r *accountingRepository) GetDailySummary(ctx context.Context, clinicID uint64, date time.Time) (*DailySummaryResult, error) {
	...
	// Convert JST date range to UTC for query (completed_at is stored in UTC)
	jstDateStart := time.Date(date.Year(), date.Month(), date.Day(), 0, 0, 0, 0, jst)
	jstDateEnd := jstDateStart.AddDate(0, 0, 1)
	utcStart := jstDateStart.UTC()
	utcEnd := jstDateEnd.UTC()
	...
	if err := r.db.WithContext(ctx).
		...
		Where("billings.completed_at >= ? AND billings.completed_at < ?", utcStart, utcEnd).
