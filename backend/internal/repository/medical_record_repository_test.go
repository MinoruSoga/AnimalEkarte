...
func TestMedicalRecordRepository_Update(t *testing.T) {
	...
	t.Run("draft カルテは更新できる", func(t *testing.T) {
		...
		updated, err := repo.Update(ctx, clinicA, rec.ID, map[string]any{"record_no": "UP-DRAFT-EDITED"})
		require.NoError(t, err)
	...
	t.Run("finalized カルテは Conflict で更新できない", func(t *testing.T) {
		...
		_, err := repo.Update(ctx, clinicA, rec.ID, map[string]any{"record_no": "UP-FIN-EDITED"})
		require.Error(t, err)
	...
	t.Run("他院からの更新は対象0件で Conflict（越境更新の拒否）", func(t *testing.T) {
		...
		_, err := repo.Update(ctx, clinicB, rec.ID, map[string]any{"record_no": "HACKED"})
		require.Error(t, err)
