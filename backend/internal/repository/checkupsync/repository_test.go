...
// setupTestDB/ensureAutoMigrated/makeTestOwner はフラット側と同様 repotest.* に直接委譲する。
...
func TestCheckupSyncRepository_FindCheckupSyncPreview_AggregationClinicIsolationAndSpecies(t *testing.T) {
	...
	ownerA := repotest.MakeTestOwner(t, db, clinicA, "同期飼主A")
	deceasedAt := time.Now()
	...
	// 別クリニックの飼主（混入しないこと）
	ownerB := repotest.MakeTestOwner(t, db, clinicB, "同期飼主B")
...
func TestCheckupSyncRepository_FindCheckupSyncPreview_LastVisitDateRangeFilters(t *testing.T) {
	...
	dogSpecies := makeSyncSpecies(t, db, "犬")
	ownerEarly := repotest.MakeTestOwner(t, db, clinicA, "早期飼主")
	...
	ownerRecent := repotest.MakeTestOwner(t, db, clinicA, "最近飼主")
...
func TestCheckupSyncRepository_FindCheckupSyncPreview_ChronicConditionFilter(t *testing.T) {
	...
	dogSpecies := makeSyncSpecies(t, db, "犬")
	ownerSick := repotest.MakeTestOwner(t, db, clinicA, "慢性疾患飼主")
	...
	ownerHealthy := repotest.MakeTestOwner(t, db, clinicA, "健康飼主")
...
func TestCheckupSyncRepository_FindCheckupSyncPreview_TotalAmountAndAnnualVisitFilters(t *testing.T) {
	...
	dogSpecies := makeSyncSpecies(t, db, "犬")
	ownerHigh := repotest.MakeTestOwner(t, db, clinicA, "高額飼主")
	...
	ownerLow := repotest.MakeTestOwner(t, db, clinicA, "低額飼主")
	...
	ownerFrequent := repotest.MakeTestOwner(t, db, clinicA, "頻回飼主")
	...
	ownerRare := repotest.MakeTestOwner(t, db, clinicA, "希少飼主")
...
func TestCheckupSyncRepository_FindCheckupSyncPreview_AgeAndLastCheckupFilters(t *testing.T) {
	...
	ownerYoung := repotest.MakeTestOwner(t, db, clinicA, "若齢飼主")
	...
	ownerOld := repotest.MakeTestOwner(t, db, clinicA, "高齢飼主")
