...
// manual_article_repository_test.go — ManualArticleRepository 統合テスト。
//
// 保護する不変条件:
...
// setupManualArticleTestDB は manual_articles / manual_article_versions を用意する。
...
func TestManualArticleRepository_FindAll(t *testing.T) {
	db := setupManualArticleTestDB(t)
	repo := NewManualArticleRepository(db)
...
func TestManualArticleRepository_FindByCategoryAndSlug(t *testing.T) {
	db := setupManualArticleTestDB(t)
	repo := NewManualArticleRepository(db)
	ctx := context.Background()
...
func TestManualArticleRepository_Upsert(t *testing.T) {
	db := setupManualArticleTestDB(t)
	repo := NewManualArticleRepository(db)
	ctx := context.Background()
...
func TestManualArticleRepository_Delete(t *testing.T) {
	db := setupManualArticleTestDB(t)
	repo := NewManualArticleRepository(db)
	ctx := context.Background()
...
func TestManualArticleRepository_FindVersionsByArticleID(t *testing.T) {
	db := setupManualArticleTestDB(t)
	repo := NewManualArticleRepository(db)
	ctx := context.Background()
