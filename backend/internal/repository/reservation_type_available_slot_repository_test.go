...
// reservation_type_available_slot_repository_test.go
// ReservationTypeAvailableSlotRepository の CRUD・clinic_id 隔離・NotFound を実 DB で検証する。
...
// setupReservationTypeAvailableSlotTestDB は予約可能開始時刻テスト用に DB を整備する。
// reservation_types CASCADE で reservation_type_available_slots も連鎖クリアされる（FK ON DELETE CASCADE）。
func setupReservationTypeAvailableSlotTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	db := setupTestDB(t)
	require.NoError(t, ensureAutoMigrated(db, &model.ReservationType{}, &model.ReservationTypeAvailableSlot{}))
...
func TestReservationTypeAvailableSlotRepository_FindAll(t *testing.T) {
	db := setupReservationTypeAvailableSlotTestDB(t)
