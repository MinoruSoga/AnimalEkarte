...
// TestFindPeriodUnpaidCarryover_LatestScheduled は
// 飼主+ペットグループの未納会計の MAX(scheduled_date) が latest_scheduled として
// YYYY-MM-DD で返ることを検証する。EMR-189
func TestFindPeriodUnpaidCarryover_LatestScheduled(t *testing.T) {
	db := setupUnpaidCarryoverTestDB(t)
	repo := NewAccountingRepository(db)
	ctx := context.Background()
	clinicID := uint64(1)
	...
	// 同一 owner+pet グループに期間前・期間内をまたぐ複数 scheduled_date の未納会計を投入し、
	...
	items, total, _, err := repo.FindPeriodUnpaidCarryover(ctx, clinicID, firstDay2026June, lastDay2026June, 1, 100)
	require.NoError(t, err)
	assert.Equal(t, int64(1), total)
	require.Len(t, items, 1)
