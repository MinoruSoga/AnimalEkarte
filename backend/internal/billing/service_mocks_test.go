...
func (m *mockAccountingRepository) FindAll(ctx context.Context, clinicID uint64, filters AccountingListFilters, page, limit int) ([]model.Billing, int64, error) {
	if m.findAllFn != nil {
		return m.findAllFn(ctx, clinicID, filters, page, limit)
	}
	return nil, 0, nil
}

func (m *mockAccountingRepository) FindAllForClinics(_ context.Context, _ []uint64, _ AccountingListFilters, _, _ int) ([]model.Billing, int64, error) {
	return nil, 0, nil
}
