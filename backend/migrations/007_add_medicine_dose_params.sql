-- 007_add_medicine_dose_params.sql
--
-- 由来: 旧 009_add_medicine_dose_params.sql（2026-07-04 に 001_init.sql §7 へ畳み込み済み）。
...
ALTER TABLE medicines
  ADD COLUMN IF NOT EXISTS calculation_type      medicine_calculation_type NOT NULL DEFAULT 'none', -- default-deny
  ADD COLUMN IF NOT EXISTS strength              numeric(10,4),   -- 製品含量。medicine_unit で分母解釈（per_tablet=mg/錠, per_ml=mg/mL, per_gram=mg/g）
  ADD COLUMN IF NOT EXISTS frequency_per_day     integer,         -- 1日投与回数（dose_basis=per_day の按分に使用）
  ADD COLUMN IF NOT EXISTS default_duration_days integer;         -- 既定投与日数（プリフィル補助）
ALTER TABLE medicines
...
-- ------------------------------------
CREATE TABLE IF NOT EXISTS medicine_dose_params (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_dose_params_med_species
    ON medicine_dose_params (medicine_id, species)
    WHERE deleted_at IS NULL;

-- clinicScope 主クエリ（clinic_id, medicine_id）。
CREATE INDEX IF NOT EXISTS idx_dose_params_clinic_medicine
...
ALTER TABLE treatments
  ADD COLUMN IF NOT EXISTS dose_weight_kg      numeric(6,2),     -- 使用体重スナップショット（kg 正規化後）
  ADD COLUMN IF NOT EXISTS dose_weight_source  varchar(255),      -- 体重の出典（vital_records.id / 時刻 pin 等）
  ADD COLUMN IF NOT EXISTS dose_amount_mg      numeric(12,6),    -- 実効用量(mg)。安全域判定(C1)はこの丸め後の値
  ADD COLUMN IF NOT EXISTS dose_amount_unit    text,             -- 'mg' | 'ug'
  ADD COLUMN IF NOT EXISTS dose_param_snapshot jsonb;            -- 適用 species/dose_per_kg/strength/丸め設定/計算式版を値で固定
