-- 008_add_checkup_packages.sql
--
-- 由来: 旧 010_add_checkup_packages.sql（2026-07-04 に 001_init.sql §7 へ畳み込み済み）。
...
-- CASCADE: checkup_type_fields.checkup_type_id / checkup_field_results.checkup_id（純粋従属子・レビュー済み）。
...
-- ------------------------------------
CREATE TABLE IF NOT EXISTS checkup_type_fields (
...
-- ------------------------------------
CREATE TABLE IF NOT EXISTS checkup_field_results (
    id                    BIGSERIAL          PRIMARY KEY,
    clinic_id             bigint             NOT NULL REFERENCES clinics(id)   ON DELETE RESTRICT,
...
-- ------------------------------------
CREATE INDEX IF NOT EXISTS idx_checkup_type_fields_clinic_id        ON checkup_type_fields(clinic_id);
CREATE INDEX IF NOT EXISTS idx_checkup_type_fields_checkup_type_id  ON checkup_type_fields(checkup_type_id);
CREATE INDEX IF NOT EXISTS idx_checkup_type_fields_clinic_type_sort ON checkup_type_fields(clinic_id, checkup_type_id, sort_order) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_checkup_field_results_clinic_checkup ON checkup_field_results(clinic_id, checkup_id);
-- checkup_id 単独は FindByPetID の JOIN（checkups.id = checkup_field_results.checkup_id）用に保持する。
CREATE INDEX IF NOT EXISTS idx_checkup_field_results_checkup_id      ON checkup_field_results(checkup_id);
-- checkup_type_field_id は ON DELETE SET NULL で NULL 行が増えるため部分インデックス。
CREATE INDEX IF NOT EXISTS idx_checkup_field_results_field_id        ON checkup_field_results(checkup_type_field_id) WHERE checkup_type_field_id IS NOT NULL;
