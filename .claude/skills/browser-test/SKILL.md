...
1. **テスト結果レポートを出力**する
   ...
   - NG 項目はリポジトリ直下 `bug.md`（受入テストバグ台帳）の末尾へ `## BUG-XXX:` 節として起票（ローカル連番 最大+1。症状・再現手順・調査の起点を記載）。バグではない改善事項は `todo.md` の `## 個別タスク詳細` へ `### TASK-XXX:` 節として起票（旧 `3-session-agent.html#ledger` は 2026-07-31 廃止）
