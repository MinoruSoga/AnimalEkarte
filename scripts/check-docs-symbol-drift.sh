...
# 2026-07-16 再編（現行: architecture|spec|ops|delivery|work ＋ README/product-philosophy）を機械強制する。
...
for p in "$ROOT"/docs/*; do
  ...
  if [[ -d "$p" ]]; then
    ...
    [[ "$ok" == true ]] || fail "docs/ 直下に allowlist 外のフォルダ \`docs/$base/\`（正: architecture|spec|ops|delivery|work。docs/README.md の規律参照）"
  elif [[ -f "$p" ]]; then
    ok=false
