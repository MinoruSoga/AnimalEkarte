#!/usr/bin/env bash
# Activate migrated staffs via the authoritative old_db staff-activity map
# (old_db sensitive-local/staff-activity-map artifact).
#
#   scripts/staff-activity-map-apply.sh <local|stg> [--dry-run]
#
# Env:
#   OLD_DB_STAFF_ACTIVITY_MAP_CSV path to staff-activity-map.csv (required;
...
MAP_CSV="${OLD_DB_STAFF_ACTIVITY_MAP_CSV:-}"
if [[ -z "$MAP_CSV" || ! -f "$MAP_CSV" || -L "$MAP_CSV" ]]; then
  echo "ERROR: OLD_DB_STAFF_ACTIVITY_MAP_CSV must point to a regular map CSV file" >&2
  exit 1
fi
...
expected="$(grep -E ' staff-activity-map\.csv$' "$ARTIFACT_DIR/SHA256SUMS" | awk '{print $1}' | head -1)"
...
if m.get("schemaVersion") != "staff-activity-map-manifest-v1":
...
build_stream() {
  printf 'BEGIN;\n'
  printf 'CREATE TEMP TABLE tmp_staff_activity_map (\n'
  printf '  csv_staff_id bigint NOT NULL, clinic_code text NOT NULL,\n'
  printf '  classification text NOT NULL, basis text NOT NULL,\n'
  printf '  last_activity_bucket text\n'
  printf ') ON COMMIT DROP;\n'
  printf 'COPY tmp_staff_activity_map FROM STDIN WITH (FORMAT csv, HEADER true);\n'
  cat "$MAP_CSV"
  printf '\\.\n'
  cat "$ROOT/scripts/sql/apply-old-db-staff-activity-map.sql"
  if [[ "$DRY_RUN" = "1" ]]; then
    printf 'ROLLBACK;\n'
  else
    printf 'COMMIT;\n'
...
case "$MODE" in
  local)
    ...
    echo "INFO  applying staff activity map (local db: $DB_NAME_VAL, dry_run=$DRY_RUN)"
  ...
  stg)
    ...
    echo "INFO  applying staff activity map (stg db: $PGDATABASE, dry_run=$DRY_RUN)"
...
if [[ "$DRY_RUN" = "1" ]]; then
  ...
  echo "INFO  staff activity map apply committed"
fi
