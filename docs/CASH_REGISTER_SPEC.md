...
```
通常営業日: 月〜土（祝日含む・通常時間で営業）
短縮営業日: 日曜日（原則として短縮時間）
臨時短縮日: 特定日付を手動で指定（年末年始・院長不在日など）
```

> **補足**: 祝日も通常通り営業するため「祝日＝特別スケジュール」とはならない。  
> 短縮営業日の区分は「日曜」を基本とし、臨時対応が必要な日は個別に設定する。

### 2-3. 時間帯変更の仕様

- 変更単位：**営業日区分ごと**に「AM/PM切替時刻」「閉院時刻」を設定
- 臨時変更：特定日付を指定して当日限りのオーバーライドが可能（年末年始・短縮営業日など）
- 適用優先順位：`特定日付設定 > 日曜設定 > 通常設定`
...
| # | 質問 | 回答・現在の仮定 |
|---|------|---------------|
| 1 | 日祝カレンダーはシステム自動判定か手動入力か？ | **解決**: 祝日も通常営業のため祝日判定は不要。短縮営業は臨時設定で手動対応。 |
...
| 2026-04-19 | 祝日も通常営業のため「日祝」区分を廃止。短縮営業日は臨時設定で対応する方式に変更。未決事項 #1 解決。 |
