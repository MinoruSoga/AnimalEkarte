# AnimalEkarte テスト環境 インフラ構成（AWS / us-east-1）

本ドキュメントは **AnimalEkarte** の **テスト環境**（単一事業者想定）における AWS インフラ構成を、再現可能な粒度でまとめたものです。  
...
- Security Groups（SG）
  ...
  - RDS SG: `sg-053d44ac9fab4e71a`
    ...
    - egress: all
- CloudWatch Logs
  - LogGroup: `/ecs/animalekarte-test`
  - retention: 30日
- SSM Parameter Store
  - `/animalekarte/test/db/user`（String）
  - `/animalekarte/test/db/password`（SecureString）
  - `/animalekarte/test/db/name`（String）
...
- DB Engine: PostgreSQL 16（テスト用途）
- Single-AZ（コスト優先）
- Storage: gp3 20GB（暗号化あり）
- PubliclyAccessible: false（Private Subnet）
- Backup retention: 1日（テスト用途）
- Deletion protection: false（テスト用途）
- Endpoint:
  - `animalekarte-test-db.cqbe28s44fta.us-east-1.rds.amazonaws.com:5432`
- DB Name:
  - `ekarte_db`
...
- ECS Cluster
  - name: `animalekarte-test-cluster`
  - Container Insights: enabled
- ECS Service
  - name: `animalekarte-test-service`
  - desired: 1（テスト用途）
...
```mermaid
flowchart LR
  subgraph VPC[10.0.0.0/16  animalekarte-test-vpc]
...
- `task_execution_role_arn`  
  `arn:aws:iam::698109622668:role/animalekarte-test-ecs-task-execution-role`
...
出力:
- `task_role_arn`  
  `arn:aws:iam::698109622668:role/animalekarte-test-ecs-task-role`
...
```bash
aws ecs update-service   --cluster animalekarte-test-cluster   --service animalekarte-test-service   --force-new-deployment   --region us-east-1   --profile AnimalEkarte
```
...
```bash
aws logs tail /ecs/animalekarte-test --follow --since 10m --region us-east-1 --profile AnimalEkarte
```

### 7.2 ECS Service 状態

```bash
aws ecs describe-services   --cluster animalekarte-test-cluster   --services animalekarte-test-service   --region us-east-1   --profile AnimalEkarte
```

### 7.3 ALB Target Health

```bash
TG_ARN=$(aws elbv2 describe-target-groups   --names animalekarte-test-tg   --region us-east-1   --profile AnimalEkarte | jq -r '.TargetGroups[0].TargetGroupArn')
...
- `ecs_service_name`: `animalekarte-test-service`
- `rds_endpoint`: `animalekarte-test-db.cqbe28s44fta.us-east-1.rds.amazonaws.com:5432`
