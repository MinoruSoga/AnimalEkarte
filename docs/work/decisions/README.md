# decisions/ — 採択済み決裁メモ

> **目的**: PO が採択した方針の短いポインタ。
> **正本**: Linear（hub [BRT-4](https://linear.app/baritechllc/issue/BRT-4)）。repo 補助入口は [`todo-po.md`](../../../todo-po.md)。[`todo.md`](../../../todo.md) は repo に結び付く未完了作業の入口（受入残・USER ゲート・STG・deferred）。競合時は Linear を正とする。

| 文書 | 内容 |
|------|------|
| [fable-po-recommendation.md](./fable-po-recommendation.md) | 2026-08-06 採択 pack の残 HOLD 索引。全文は CorpVault |

## 規律

- dated 調査レポートは置かない（git 履歴 / CorpVault evidence）
- 新しい未解決タスクは Linear に起票する
- 未採択の議論メモは残さない
