...
範囲: 医院からの質問原文、送付用回答、および各問に対して実施すべき修正タスク。第1報は Q1–Q4（§0–§6）。第2報は #1–#4（§7 以降）。  
...
残差: Q3 gender map、ワクチン種調査、未納 STG 集計、処置移行。第2報は §7 以降。

---

## 7. 第2報 — 医院からの質問原文

受領した文面を改変せずに置く。写真は本ファイル作成時点で未添付。送付用は §8。
...
追加のご指摘ありがとうございます。検証環境の実装に照らして回答します。写真は未受領のため、②は文面の説明だけを根拠にしています。

**#1 ① マスタを入れても金額が出ない／会計画面に出ない**
...
| UAT-R2-RESERVATION-HOWTO | #4 | — | 手順回答のみ | §8 に記載。機能追加なし |
...
手順は §8。コード変更なし。操作マニュアルへ同じ手順を足すのは任意。
