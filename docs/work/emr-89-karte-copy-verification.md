...
```mermaid
flowchart TB
    Q["スプシ No.9<br>カルテ複製時に金額の注意喚起は出るか"]
    Q --> C{"カルテ複製機能の有無"}
    C -->|行アクションは編集・削除のみ<br>API・service に複製なし| N["警告の付け先なし"]
    V["唯一の複製＝ワクチン履歴メタ<br>金額を含まない"] -.-> N
    P["明細金額は常に選択時点の現在マスタ価格<br>旧価格の自動引継ぎなし"] --> N
    N --> E["結論：実装対象なし<br>システム経路で過不足は発生しない"]
```
