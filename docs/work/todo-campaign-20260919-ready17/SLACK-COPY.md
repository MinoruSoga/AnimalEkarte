...
```mermaid
flowchart LR
    A["同一 clinic・同一 pet の過去カルテをユーザーが選択"] --> B["Preview: 取込項目をチェック・数量/単価/薬剤を再確認"]
    B -->|"取消"| C["overlay を閉じる（現入力・元記録は無変更）"]
    B -->|"適用"| D["選択行ごと新規 POST（旧 id・inventory_id は載せない）"]
    D --> E["新規 treatment.id だけが未請求抽出へ載る"]
```
