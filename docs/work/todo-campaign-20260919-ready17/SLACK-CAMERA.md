...
本票は [ImageGalleryFilter](../../../frontend/src/features/medical-records/components/ImageGalleryFilter.tsx) の file input（**`capture` 属性なし**）と [MedicalRecordImage](../../../frontend/src/features/medical-records/components/MedicalRecordImage.tsx) の既存 upload をトレースする。製品コード・テストは変更しない。
...
| 対象 iPad / スマートフォン / PC | file input は `type="file"` + `accept` + `multiple` のみ。`capture` 無し（後述） | **UNKNOWN**。対象機種・OS・ブラウザは PO 確定前 |
| 現行 picker に「撮影」が出るか | HTML はカメラ優先を要求していない。OS がギャラリー/撮影/ファイルを出すかは UA 依存 | **UNKNOWN**。実機確認前に「出ない」とも「出る」ともしない |
...
| --- | --- | --- | --- |
| P — 既存 file picker | [ImageGalleryFilter](../../../frontend/src/features/medical-records/components/ImageGalleryFilter.tsx) L105–112。`type="file"` `accept="image/jpeg,image/png,image/gif,application/pdf"` `multiple`。**`capture` 属性は無い** | 「capture が無い」=「端末で撮影できない」と読まない。OS picker の撮影項目は **UNKNOWN** |
| U — 既存 upload | [MedicalRecordImage](../../../frontend/src/features/medical-records/components/MedicalRecordImage.tsx) L58–65 → [uploadMedicalRecordImages](../../../frontend/src/features/medical-records/api/medical-record-images.ts) L71–78（1 ファイルは L24–37 `uploadImage`）`POST /v1/medical-records/:id/images/upload` | 別エンドポイントや JSON create（同 handler の POST `/images`）を撮影導線に足さない |
| C — 独自カメラ UI | `frontend` に `capture=` 0 件。医療画像経路に `getUserMedia` なし | picker 未確認のまま MediaStream / 全画面カメラを「最小導線」と呼ない |
...
3. **picker:** [ImageGalleryFilter.tsx](../../../frontend/src/features/medical-records/components/ImageGalleryFilter.tsx) L65–69 ボタンが hidden input を `click()`。L105–112 に `capture` **なし**。ラベルは「画像アップロード」（L121）。`canUpload=false` なら input/ボタンごと出さない（L103–124）。
4. **選択ゲート（SEC-CS-F08）:** 同ファイル L71–99。件数 >10、合計 >50MiB、1 件 >10MiB は toast して **バッチ全体拒否**（部分 upload しない）。通った `File[]` だけ `onFilesSelected`。
5. **即 upload:** MedicalRecordImage L61–65。`canUpload` かつ `resolvedId` があるときだけ `useCreateMedicalRecordImages.mutate(files)`。アプリ内の確認/取消ステップは無い。
...
9. **BE route:** [routes_records.go](../../../backend/internal/medicalrecord/routes_records.go) L43–45。list=`view`、JSON create と **upload=`create`**、delete=`delete`。
...
| 観点 | 現行 | capture を足す場合（実装しない。比較用） |
| --- | --- | --- |
| HTML | L105–112: `type="file"` `accept=…` `multiple`。`capture` トークンなし。`rg capture=` は frontend 0 件 | `capture="environment"` または `user` は **ヒント**。ブラウザが無視してよい |
| 撮影選択 | コードはカメラ優先を要求しない | 一部モバイル UA はカメラを直接開く。iPad/Safari の実挙動は **UNKNOWN** |
| ギャラリー複数枚 | `multiple` あり | `capture` 付きだと単発カメラになり `multiple` と衝突する UA がある（実機 **UNKNOWN**） |
| PDF | `accept` に `application/pdf` | カメラ専用にすると PDF 添付導線が消えるリスク。撮影とファイルを同じ input に混ぜるかは PO |
| 独自カメラ | 無し | picker に撮影が**無い**と実機で出たときだけ検討。getUserMedia は権限・プレビュー・向きを全部自前で持つので ② 違反になりやすい |

**結論（本票）:** 現行 input は capture を欠く。upload 経路は既にある。カスタムカメラ UI は **必須と推定しない**。次の実装があるとしても、実機で「撮影項目が無い」が取れてから `capture` ヒントを試すのが上限。
...
| ID | ケース | 現行の期待 | 検証 | 停止 |
| --- | --- | --- | --- | --- |
| CAM-PERM-UI | `medical-records:create` なし | アップロードボタン非表示（canUpload）。BE は route `create` で 403 | FE は死亡ケースのテストあり。`canCreate=false` 単独のボタン非表示テストは薄い | 権限モデルを撮影のために緩めない |
...
| CAM-NEW | 未保存カルテ | `resolvedId` なし → 選択しても mutate しない。ボタンは canUpload なら出る | コード L38–65。専用テストなし | 未保存への撮影を「できる」と案内しない |
...
あるもの（現行 file-picker upload）:

- FE: [ImageGalleryFilter.test.tsx](../../../frontend/src/features/medical-records/components/ImageGalleryFilter.test.tsx) 件数/合計/単体サイズ/canUpload。
- FE: [MedicalRecordImage.test.tsx](../../../frontend/src/features/medical-records/components/MedicalRecordImage.test.tsx) 検索・死亡・見出し。
...
実機で「撮影」が出る → **製品変更ゼロ**で案内する（削除が先）。

実機で「撮影」が無い（対象端末の証拠付き）→ 独自カメラではなく:

1. 既存 hidden file input に `capture` ヒントを足すか、撮影用に **別の** input を 1 つ足す（PDF `multiple` と衝突する場合）
2. `onFilesSelected` 以降は **今の** `uploadMedicalRecordImages` だけを使う
3. 死亡/確定/create 権限/clinic を変えない
4. HEIC・向きは PO が用途を決めるまで変換パイプラインを足さない

getUserMedia のカスタムカメラは、上記でも撮影不能と分かったあとのみ再検討。
...
2. ダミー画像（実患者でない jpeg）だけで、現行「画像アップロード」に撮影選択が出るか採取する
3. 出る → 操作案内で閉じる。出ない → `capture` ヒントの最小パッチを別 unit にする
