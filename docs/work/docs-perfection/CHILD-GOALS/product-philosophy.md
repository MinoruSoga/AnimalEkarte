---
id: "DOCS-PERFECT-PRODUCT-PHILOSOPHY"
objective: "既存の意思決定原則と臨床安全の優先関係を保ち、例の強すぎる断定と索引の正本参照を整える。"
status: "COMPLETE"
scope: "docs本文保守の一カテゴリ。全対象MarkdownはINVENTORYの該当カテゴリ行から再列挙する。coordinator成果物は除外。"
allowlist: ["docs/product-philosophy.md", "docs/README.md"]
...
verification: ["開始時に git status --short と git branch --list claim/DOCS-PERFECT-PRODUCT-PHILOSOPHY を確認。claimがあれば編集せずBLOCKED。なければ git branch claim/DOCS-PERFECT-PRODUCT-PHILOSOPHY を取得。claim削除はUSERのみ。", "bash scripts/check-docs-symbol-drift.sh", "git diff --check -- docs/product-philosophy.md docs/README.md", "git diff --name-only と git status --short を開始時baselineに比較し、tracked/staged/untracked全ての新規差分がallowlist内であることを検査する。", "docs/product-philosophy.md の各節を .claude/CLAUDE.md と docs/spec/specification.md §2.1 に照合。説明例の修正が既存要件の変更になっていないことをレビューする。", "rg -n '確認ダイアログ|明示同意|臨床の安全' docs/product-philosophy.md .claude/CLAUDE.md docs/spec/specification.md", "docs/README.md の全ローカルMarkdownリンクが存在し、カテゴリ集合が既存5ディレクトリとproduct-philosophyのままであることを検査する。"]
...
priority: "P1"
depends_on: ["DOCS-PERFECT-COORDINATOR"]
repair_queue_refs: ["RQ-010"]
...
# Child goal: DOCS-PERFECT-PRODUCT-PHILOSOPHY
...
flowchart LR
    ...
    B -->|なし| C["claim取得"]
    C --> D["allowlist内で修復<br>docs/product-philosophy.md と docs/README.md（RQ-010）"]
    D --> E["検証群を実行"]
