...
状態: **競合ケース設計 READY／unit・mock 防御の再照合済み（2026-09-21）／DB apply 証明と 2 セッション実機は残件**。2026-09-19依頼者回答の「両方」により、目的はカルテの上書きと同じ会計の二重確定の防止に確定した。旧システムの画面占有を複製する要件ではない（[要件・状態の正本](../../../todo-issue.md#uat-r2-exclusive-lock)、[元報告](../stg-uat-clinic-feedback-q1-q4.md#uat-r2-exclusive-lock)）。画面占有ロックは未採用。
...
3. 非 findings タブ（治療/バイタル/処方/接種）への `expectedVersion` 拡張は別裁定。
