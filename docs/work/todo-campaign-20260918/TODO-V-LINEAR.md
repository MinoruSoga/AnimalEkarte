...
```mermaid
flowchart TB
    A["ローカル ID"] --> B{"既存 Issue の本文・コメントに full ID の直接一致があるか"}
    X["親子関係・同じ語・検索ヒット"] -.->|直接一致の根拠にしない| B
    B -->|一致を特定| C["既存 Issue URL・現状差分・更新文面を追記"]
    B -->|一致なし・UNKNOWN| D["更新案なし"]
    C --> E["対象 URL と文面の明示承認・更新直前/直後の再読取"]
    D --> F["残る既存 Issue の照合を継続"]
```
