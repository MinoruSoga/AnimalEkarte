# 会計一覧 仕様書 (Accounting List)

## 概要
- **画面の目的**: 院内で行われた全診療・サービスの請求レコードを一覧表示・管理し、未精算の把握や過去の会計参照を行う。
- **URLパターン**: `/accounting`
- **アクセス権限**: 認証済ユーザー全員（操作権限は `usePermission` で制御）

---

## 画面構成

### 1. 検索・フィルタリング (`NotionFilter`)
- **診療日指定**: 会計が発生した日付範囲（From-To）によるサーバーサイド抽出。
- **ステータスフィルタ**: 「会計待ち」「会計済」「キャンセル」での絞り込み。
- **支払方法フィルタ**: 現金、クレジットカード、電子マネー等による抽出。
- **全文検索**: 飼主名、ペット名によるキーワード検索。

### 2. データテーブル (`DataTable`)
請求額と現在の精算状況を一目で把握できるよう設計されています。

| カラム | 説明 |
|:---|:---|
| **日時** | 会計予定日（YYYY-MM-DD）。 |
| **患者・飼主** | 飼主名およびペット名。 |
| **請求金額** | 税込の合計請求額。 |
| **支払方法** | 精算済の場合、使用された決済手段を表示。 |
| **ステータス** | **「会計待ち」**: 未精算または精算中。 |
| | **「会計済」**: 支払完了。 |
| | **「キャンセル」**: 請求取り消し。 |
| **特殊バッジ** | **「返金あり」**: 部分返金または全額返金が発生している場合に表示。 |
| **関連リンク** | 診療内容（カルテ）への直接リンクボタン（存在する場合のみ）。 |

---

## 主要なユーザーアクション

- **詳細・精算**: テーブルの行をクリック、または操作ボタンから会計精算画面 (`/accounting/:id`) へ遷移し、支払い処理や詳細確認を行います。
- **未納管理**: ヘッダーの「未納者一覧」ボタンより、売掛金が発生している顧客を抽出する専用ビューへ遷移します。
- **新規作成**: 診療外の物販等で個別に会計を作成する場合、ヘッダーの「新規会計登録」から実施します。

---

## 技術仕様

### パフォーマンスと体験
- **視覚的フィードバック**: フィルタ処理やページ切り替え中、`FilteringIndicator` により計算中であることを通知し、UI のフリーズ感を排除します。
- **URL同期**: ページ番号や検索条件は URL パラメータと同期され、ページ再訪時も状態が維持されます。

### API連携
| メソッド | エンドポイント | 用途 |
|:---|:---|:---|
| GET | `/api/v1/accountings` | 条件に応じた会計一覧の取得。 |
| DELETE | `/api/v1/accountings/:id` | 特定の会計レコードの論理削除（キャンセル処理）。 |

---
