...
### 概要
Notion 風の操作感を持つ、高度な動的フィルタリングおよびソートコンポーネント。

### 主な機能
- **キーワード検索**: インクリメンタルな全文検索。
- **動的フィルタ追加**: プロパティの種類（テキスト、選択、日付等）に応じた条件（「を含む」「と一致する」「以前」等）で動的にフィルタを追加可能。
- **マルチソート**: 複数のソート条件を組み合わせてデータの並び順を制御。
- **件数表示**: 現在のフィルタ条件に合致する件数をリアルタイム表示。

---

## 14. データテーブル (`DataTable`)

### 概要
システム全体で共通のレイアウトを持つデータ一覧表示コンポーネント。

### 特徴
- **Notionスタイル**: 罫線が少なく、余白を活かしたクリーンなデザイン。
- **ステータスバッジ統合**: `StatusBadge` や `NotionStatusPill` と組み合わせた視覚的な一覧表示。
- **レスポンシブ**: モバイル時はスクロールまたはカラムの省略に対応。
...
### 概要
大量のデータを分割して表示するためのページ切り替えコンポーネント。

### 機能
- **URL同期**: 現在のページ番号（`?page=N`）を URL パラメータと同期。
- **ナビゲーション**: 「前へ」「次へ」および数値指定によるページ移動。
- **表示範囲表示**: 「1〜20 / 100件」のような現在の表示範囲を明示。
