# 在庫管理一覧 仕様書 (Inventory List)

## 概要
- **画面 of Purpose**: 院内で使用・販売する医薬品、フード、備品等の在庫状況の監視と、適切な発注タイミングの把握。
- **URLパターン**: `/inventory`
- **アクセス権限**: 認証済ユーザー全員（`ResourceInventory` 権限が必要）

---

## 1. 画面構成

### 1.1 在庫アラートパネル
在庫切れ、または最低在庫数を下回った（残少）品目がある場合、画面上部に警告サマリーが表示されます。
- **視覚的強調**: `AlertTriangle` アイコンと共に、緊急対応が必要な件数を一目で確認可能。

### 1.2 高度な検索・フィルタ (`NotionFilter`)
- **状態フィルタ**: 「在庫切れ」「残少」「十分」による状態別の絞り込み。
- **カテゴリフィルタ**: 医薬品、療法食、備品等のマスタカテゴリによる抽出。
- **全文検索**: 品名、保管場所、仕入先名による部分一致検索。

### 1.3 在庫状況テーブル (`DataTable`)
| カラム | 説明 |
|:---|:---|
| **品名** | 商品名または薬品名。 |
| **カテゴリ** | マスタ定義された分類。 |
| **在庫数 / 単位** | 現在の理論在庫数と単位。 |
| **最低在庫数** | 発注の閾値（この数値を下回るとアラート対象）。 |
| **有効期限** | 医薬品等の使用期限管理（期限間近の把握に利用）。 |
| **保管場所** | 院内のどの棚・冷蔵庫にあるかの記録。 |

---

## 2. 主要な運営機能

- **一括棚卸し（計画中）**: 実在庫との差異を調整するためのバルク入力機能。
- **発注リスト作成**: 「残少」および「在庫切れ」品目を抽出した CSV の出力。

---

## 3. 技術仕様

### 3.1 描画の最適化
- **遅延フィルタリング**: `useDeferredValue` により、数千点の在庫品目から目的の品を特定する際も UI がフリーズしません。

### API連携
| メソッド | エンドポイント | 用途 |
|:---|:---|:---|
| GET | `/api/v1/inventory` | 条件に応じた在庫一覧の取得。 |
| DELETE | `/api/v1/inventory/:id` | 在庫品目の論理削除。 |

---
