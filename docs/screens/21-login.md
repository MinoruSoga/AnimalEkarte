...
## 概要
- **画面 of Purpose**: システムを利用するスタッフの本人確認（認証）を行い、セキュアなセッションを開始する。
- **URLパターン**: `/login`
- **アクセス権限**: 全ユーザー（未ログイン時）
...
2.  **メール送信**: 有効期限 1 時間の署名付きワンタイムトークンが送信されます。
...
- **セッション維持**: `httpOnly`, `Secure`, `SameSite=Strict` Cookie により、XSS 攻撃からの防護を最大化しています。

### 3.2 パスワードポリシー
- **構成**: 最小 8 文字、英字および数字の混在を推奨。
