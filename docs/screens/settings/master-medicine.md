...
- **処方設定**:
    ...
    - **保険対象外**: 対象/対象外の切り替え。
- **在庫連携**:
    - バックエンドの `Medicine` モデルは在庫アイテム（`inventory_id`）と投与量自動計算パラメータ（`dose_params`）を保持するが、本設定画面のサイドパネルに在庫アイテム紐付けUIは実装されていない（#201 投与量自動計算はBE完了・FE UI未実装）。
