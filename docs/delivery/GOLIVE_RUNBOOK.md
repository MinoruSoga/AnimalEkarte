...
> **経路**: Cloudflare 経路で納品（PO 決定 2026-07-15。正本: [../ops/infra/_archive/migration-cloudflare.md](../../../ops/infra/_archive/migration-cloudflare.md) 「現況サマリ」節）
...
|---|---|---|---|---|
| 1 | STG Cloudflare 移行 Phase 7（NS 切替・並行稼働）完遂 | [../ops/infra/_archive/migration-cloudflare.md](../../../ops/infra/_archive/migration-cloudflare.md) P5-5〜P7 ／人間タスク正本 `todo-me.html` | staging デプロイ 2 回連続 green（P5-5）→ 画像移行（P2-4/5）→ データ投入（P3-6/7）→ NS 切替（P1-2）→ フルスモーク（P7-3） | （確定待ち: PR #186 マージ後に進行） |
...
| 6 | フロントエンド CSP の最終確認 | [../ops/infra/_archive/migration-cloudflare.md](../../../ops/infra/_archive/migration-cloudflare.md) §9 リスク登録簿 | `frontend/index.html` の CSP `connect-src` に本番 API オリジンが含まれている | （確定待ち） |
...
> NS 切替は **Terraform の管理対象外**（`infra/cloudflare/README.md` 安全ルール 4）。人手で実施し、実施前後の状態を `../ops/infra/_archive/migration-cloudflare.md` に記録すること。
...
AWS 側リソースの廃止（../ops/infra/_archive/migration-cloudflare.md Phase 8）は、本サポート期間で安定を確認した後にのみ実施する。

---

## 6. 関連ドキュメント

- [../ops/infra/_archive/migration-cloudflare.md](../../../ops/infra/_archive/migration-cloudflare.md) — 移行計画・実施記録・リスク登録簿（正本）
