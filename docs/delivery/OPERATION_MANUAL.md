...
**現行の画面制約**: 既存会計の再保存にはフロントエンドとAPIの不整合があり、エラーになる経路が残っています。締め後編集も含め、保存成功を前提に作業を進めないでください。詳細は[会計精算仕様の現行制約](../spec/screens/11-accounting-detail.md#21-精算済みデータの保護と修正)を参照します。新規会計で通信が切れた場合も、登録結果を一覧で確認してから操作してください。
