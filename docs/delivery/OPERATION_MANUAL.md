...
> **対象 Issue**: #256 ／ **リポジトリ由来 slice 同期日**: 2026-07-31 ／ **状態**: ドラフト（repo 由来ナビ完成・USER 日程と #254 依存は未 close）
...
|---|---|
| 実施日程 | （確定待ち） |
| 形式（院ごと / 合同） | （確定待ち; 目安: 院ごと or 合同で 60〜90 分 — 基本フロー §2 のデモ + 質疑） |
| 参加者範囲 | （確定待ち） |
| 実施 receipt（実施結果・opaque reference） | （確定待ち; 本文・実 identity は repo 外。台帳には結果 enum と restricted reference のみ） |

- **USER 入力待ち（U13）**: 上記表を training owner / 先方が供給後に本節へ追記する。契約責任者の U1–U12 一行入力先（[DELIVERY_PACKAGE.md](DELIVERY_PACKAGE.md)）とは分離する。
...
| ID | 項目 | 理由 | 反映先 |
|---|---|---|---|
| U13 | 操作説明会の実施日程・形式・参加者・実施 receipt | 先方との調整事項（repo 外）。**#258 とは分離**（#256 残差） | §10 |
