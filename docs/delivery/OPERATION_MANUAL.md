...
```mermaid
flowchart LR
  Rsv["予約（電話受付・LINE）"] -->|予約当日にカンバンへ自動表示| Rec["受付（当日の受付カンバン）"]
  Rec --> Med["診察（カルテ・SOAPS）"]
  Med -->|会計確認で明細連携| Acc["会計・精算"]
  Exm["検査（オーダー → 結果確定）"] -->|親カルテの会計確認が必要| Acc
  Trm["トリミング（受付 → 施術 → 完了）"] -->|完了で会計の未請求候補| Acc
  Acc --> Close["レジ締め（日次・確定後は append-only）"]
  Close --> Rep["月次集計レポート（売上・内訳・CSV）"]
  Close -.->|締め済み日の訂正| Pce["権限者の締め後編集（理由必須）"]
```
