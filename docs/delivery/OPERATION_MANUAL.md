...
> **対象 Issue**: #256 ／ **納品日**: 2026-07-18 ／ **状態**: ドラフト
...
> ⚠️ **ログイン失敗時の正しい仕様（重要）**: ログインは **同一端末（IP）につき 1 分間に 5 回まで** に制限されています（総当たり攻撃対策のレート制限）。超過すると一時的に拒否されますが、**時間経過（約 1 分）で自動的に再試行可能**になります。
> 「5 回失敗するとアカウントがロックされ、管理者による解除が必要」という記載が旧マニュアルにありますが、これは**誤り**です（アカウントロック機構は存在しません。Q&A No.25）。ログインできない場合は (1) 1〜2 分待って再試行 → (2) パスワードリセット → (3) 管理者にアカウントの「有効/無効」状態を確認、の順で対処してください。
...
- **確定済み会計の直接編集は不可**。「取消」→「新規会計登録」で作り直す（取消理由が記録される）。
- 締め済みの日の会計を直すには、先に管理者がレジ締め履歴から締めを取り消す必要がある。
...
**要点**:
- レジ締め（`/accounting/close`）で金種別に現金を数えて突合。区分は AM／PM／全日／EMG（時間帯の境界は管理者が「締め時間設定」で定義）。
- 締めの前提: 当日の未確定カルテ 0 件・「会計待ち」0 件・同日同区分の締め未実施。満たさない場合はエラー表示に従って先に処理。
...
| 症状 | 最初に確認すること |
|---|---|
| ログインが拒否される | 1〜2 分待って再試行（レート制限。§1 参照）。続くならパスワードリセット、管理者にアカウントの有効/無効を確認 |
| 会計画面に明細が出ない | カルテ・検査が「確定」済みか。同じ会計を別タブで開いていないか |
| 締め確定でエラー | 当日の未確定カルテ／「会計待ち」が残っていないか。同日同区分で締め済みでないか |
| 確定済み会計を直したい | 直接編集不可。「取消」→「新規会計登録」。締め済みなら先に管理者が締めを取消 |
...
## 10. 既知の記載差異（納品時点・修正予定）

システム内マニュアルの以下の記述は**実装と一致していません**。本書 §1 の仕様が正です（#256 受け入れ条件 3 の残作業）:

- 「ログイン 5 回失敗でアカウントロック」「スタッフマスタの『ロック解除』ボタンで解除」という記述（画面マニュアル「スタッフマスタ」、業務フロー「エラー・トラブル対応」「セキュリティと複数医院」「システム障害時の対応」に残存）。実装は **IP 単位のレート制限（5 回/分・自動回復）**であり、アカウントロック機構・ロック解除ボタンは存在しません。
- （確定待ち: 上記 4 記事の修正反映と、#254 デモ UAT で判明した紛らわしい操作の「よくある質問」追記）
...
- 形式・所要時間の目安: 院ごと or 合同で 60〜90 分（基本フロー §2 のデモ + 質疑）
- （確定待ち: 実施日程・院ごと/合同の別・参加者範囲）
