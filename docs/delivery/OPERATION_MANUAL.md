...
- ログインは **同一 IP につき 5 回 / 1 分** まで に制限されています（総当たり攻撃対策のレート制限）。超過すると一時的に拒否されますが、**時間経過（約 1 分）で自動的に再試行可能**になります。アカウント単位のロックや解除操作は存在しません。ログインできない場合は (1) 1〜2 分待って再試行 → (2) パスワードリセット → (3) 管理者にアカウントの「有効/無効」状態を確認、の順で対処してください。
