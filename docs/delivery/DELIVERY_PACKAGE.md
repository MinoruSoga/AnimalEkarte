...
- 薬剤の在庫紐付けはバックエンドにありますが、薬剤マスタ画面の紐付け入力は未実装です。物販マスタには在庫紐付け・販売時の自動減算がありません。画面から設定できる手順として案内せず、必要な運用は管理者・開発担当と確認してください（[薬剤マスタ](../spec/screens/settings/master-medicine.md#22-原価在庫管理の統合バックエンドのみ)・[物販マスタ](../spec/screens/settings/master-merchandise.md)）。
