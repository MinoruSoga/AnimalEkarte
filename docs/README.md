...
- **タスク台帳**: 実行 SoT は **Linear**（hub BRT-4）。root [`todo.md`](../todo.md) · [`todo-po.md`](../todo-po.md) · [`bug.md`](../bug.md) は入口ポインタ。時点レポートは CorpVault `evidence/2026-08-20-*` と git 履歴。`reports/` は gitignore（新規 UAT をコミットしない）。
