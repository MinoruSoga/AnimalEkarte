...
- **整合性の死守**: 全 **106 テーブル**のデータ変更（CRUD）を `audit_logs` で追跡。臨床記録の真正性を担保するための確定（Lock）フローを実装。
...
- **バックエンド**: Go 1.25 / Gin / GORM による堅牢な API（全 **88 ハンドラー**〔`backend/internal/handler/*_handler.go` ファイル数〕、**106 テーブル**）。
