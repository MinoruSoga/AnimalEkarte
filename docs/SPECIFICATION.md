...
> **最新更新**: 2026-05-21 | **ステータス**: Production Ready (Code-Synced)
...
- **整合性の死守**: 全 **92 テーブル**のデータ変更（CRUD）を `audit_logs` で追跡。臨床記録の真正性を担保するための確定（Lock）フローを実装。
...
- **入院**: ケージ稼働状況の可視化と、時系列（朝・昼・夜）でのデイリーケア計画・実施記録。
...
- **バックエンド**: Go 1.25 / Gin / GORM による堅牢な API（全 **31 ハンドラー**、**92 テーブル**）。
