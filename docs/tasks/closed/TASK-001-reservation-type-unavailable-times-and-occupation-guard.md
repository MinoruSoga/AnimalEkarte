...
| # | サブタスク | 領域 | イシュー | 依存 | 完了 |
|---|----------|------|---------|------|------|
| 1 | DBスキーマ追加 + Go モデル + make codegen | DB/BE | BE-115 | - | [x] |
| 2 | 管理API：予約不可時間 & 職種紐付け CRUD | BE | BE-116 | #1 | [x] |
| 3 | LIFF予約可否ロジック拡張 | BE | BE-117 | #1 | [x] |
| 4 | カルテ側：予約区分マスタ編集画面 UI 拡張 | FE | FE-252 | #2 | [x] |

## 受入条件（Acceptance Criteria）

- [x] AC-1: 管理画面の予約区分編集画面で「予約不可時間」を追加でき、「毎週月曜 12:00〜13:00」や「2026-05-10 09:00〜12:00」のように設定できる
- [x] AC-2: 設定した不可時間帯はLINE予約の日時選択画面に反映され、当該時間が選択できない
- [x] AC-3: 管理画面の予約区分編集画面で「対応職種」を1つ以上選択でき、複数職種を紐付けられる
- [x] AC-4: 紐付けた職種のスタッフが1人も出勤しない日はLINE予約の日付選択画面でグレーアウト（選択不可）になる
- [x] AC-5: 職種が紐付けられていない予約区分は従来通り動作する（職種ガード機能はオプション）
- [x] AC-6: 予約不可時間も紐付け職種も未設定の予約区分は従来通り動作する
