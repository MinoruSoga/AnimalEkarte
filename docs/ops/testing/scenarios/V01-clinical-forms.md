...
| 3 | (C3-1) 種別に「V01健診」（§4 で作成）+ 当日で登録し、編集導線を探す | 登録成功。実施日を診察日とするカルテが自動生成され、そのカルテの定期健診タブに本記録が表示される（C2 は生成カルテ側で確認）。独立編集ルートは無い。一覧の編集は `/medical-records/:id?tab=定期健診&checkupId=` へ遷移する |
