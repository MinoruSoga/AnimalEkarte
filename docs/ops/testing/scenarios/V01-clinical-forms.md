...
各フォームのセクションで (C1)(C2)(C3) を参照する。フィールド・境界値は各セクションの指定に従う。
...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | (C1-1) petId なしで `/medical-records/new` を直接開く | 【要実測】ペット選択画面（select-pet）へリダイレクトされる（仕様に明記なし） |
| 2 | 新規カルテで SOAP を空のまま保存し、問診タブに主訴・主訴区分・治療方針を入力して保存 → (C2) 一覧反映・再読込・再オープン | 【要実測】SOAP 空でも保存が成功する（必須の明記が仕様になし）。問診は C2-1〜C2-3 のとおり永続・初期表示される |
...
| 5 | ヘッダーの担当医・来院種別を変更（保存ボタンを押さない）→ 再読込 | 【要実測】保存ボタンを押さずに即時保存され、再読込後も永続する（仕様に明記なし） |
...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | (C1-2/C1-3) 行追加で quantity に 0 → 保存、1 → 保存。別行で unit_price に -1 → 保存 | 【要実測】0 は拒否・1 は受理、-1 は拒否・0 円は受理（quantity/price の境界は仕様に明記なし） |
...
| 4 | 数量を安全域外（自動計算値の大幅超過）へ変更して保存 | 確認ステップを経ないと保存できない（P1-7 dose hard gate — 警告への格下げ禁止） |
...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | (C1-1) 記録日時を空で保存 / (C1-2) 未来日時を指定して保存 | 空はエラーが表示され保存されない（記録日時は必須）。【要実測】未来日時の拒否有無（仕様に明記なし） |
| 2 | (C1-3) 体温に 45.0 → 保存、45.1 → 保存 | 【要実測】体温の範囲ガードの有無と境界（仕様に明記なし）。極端値が無警告で保存される場合は BUG 起票 |
| 3 | 記録日時のみ・全計測値を空で保存 | 【要実測】BE は「最低 1 フィールド」を要求 — recorded_at のみで成立するかは仕様に明記なし |
...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | (C1-1) ワクチン未選択で追加 | 保存されない（BE: vaccine_id required）。【要実測】FE のエラー提示有無（無音失敗にならないこと — 独立画面 §8 と異なりインラインエラー未確認） |
| 2 | (C3-1) ワクチンマスタ実データ由来の選択肢から接種日・ワクチン・LOT 番号（4 つまで）を入力して保存 → (C2) 再読込 | 永続・初期表示される |
| 3 | 「次回予防接種予定設定」ラジオを切り替える | 次回日付は自動では変わらない（自動算出に未配線 — 仕様書明記の既知挙動）。次回予定は種別選択または手動入力で設定 |
...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | 画像: jpeg/png/pdf をアップロード → 再読込。次に大容量ファイル（例 20MB） | 画像タブ一覧に表示され永続する（写真・PDF 資料のアップロード管理 — [06-medical-records-form.md](../../../spec/screens/06-medical-records-form.md)）。【要実測】選択可能なファイル種別の絞り込みとサイズ上限・超過時の挙動（いずれも仕様に明記なし） |
| 2 | 追記: 確定済みカルテで追記モーダルを開き、修正内容・修正理由を空で登録 | 「修正内容は必須です」「修正理由は必須です」で保存されない |
| 3 | 追記: (C1-3) 修正理由に 500 文字 → 登録、501 文字 → 登録 | 【要実測】500 は受理、501 は拒否（reason の上限は仕様に明記なし — BE 実装は 500 文字・rune 数判定） |
| 4 | 追記: 未確定カルテで追記導線を探す | 追記は finalized カルテ専用（BE が finalized 以外を拒否）。【要実測】未確定時の導線が非表示か無効化か |
| 5 | 検査取込: 何も選択せずに取込を実行 | 【要実測】未選択 submit の挙動（ボタン無効 or 何も紐付かない） |
| 6 | 検査取込: 対象ペットの既存検査を選択して取込 → 再読込 | 検査タブに紐付き表示され永続する（検査ごとに PATCH で medical_record_id 設定） |
| 7 | 見積タブ: 件名と明細を入力してメイン保存 → 再読込 | 見積が保存され永続する（必須欄なし。postSave 経由で POST/PATCH /estimates）。見積の独立画面（/estimates）は本シナリオ対象外 |
...
| 3 | 測定値（数値）を入力して保存 → (C2) 再読込・再オープン | 永続・初期表示される。基準値マスタ範囲外の値は H/L 判定（高値=赤・低値=teal）が再読込後も表示される（判定は BE 導出） |
...
| 4 | LOT 番号を 4 つ入力して保存 → (C2) 一覧反映・再読込・再オープン | 4 つとも永続・初期表示される（最大 4 並行登録）。(C3-1) ワクチン選択肢は vaccines マスタ由来の固定 2 択（混合/狂犬病） |
...
| 3 | (C3-1) 種別に「V01健診」（§4 で作成）+ 当日で登録し、編集導線を探す | 登録成功。実施日を診察日とするカルテが自動生成され、そのカルテの定期健診タブに本記録が表示される（C2 は生成カルテ側で確認）。独立編集ルートは存在しない — 編集はカルテの定期健診タブでのみ可能（仕様） |
...
| 2 | 入院タイプ・入院予定日を空のまま（ペットのみ選択）で保存 | 【要実測】日付・タイプの必須検証は実装未確認 — 保存可否と、可の場合の一覧表示の壊れなさを確認 |
| 3 | (C3-1) ケージマスタに「V01ケージ」・治療プラン項目名マスタに「V01プラン」を追加 → フォームの選択肢確認 | それぞれ選択肢に現れる（ケージは空き状況フィルタなし = 使用中でも表示される） |
| 4 | 保険適用 ON + 保険会社名/保険番号、治療プラン明細（内容/単価/数量/割引%/値引円）を入力して保存 | ON で 2 欄が表示され保存される。明細は小計が計算され保存される。行削除ボタンは削除権限者（admin）に表示される |
| 5 | (C2) 既存入院の日付・メモ・明細を編集して保存 → 一覧反映・再読込・再オープン | 永続・初期表示される |
| 6 | 検索条件「ステータス=退院済み」の入院を開いて編集保存（無ければ S05 側で確認しスキップ可） | 【要実測】エラーが表示され保存されない（退院済み入院の編集可否は仕様に明記なし） |
...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | (C1-1) 下表の必須フィールドを空にする | 保存されない。【要実測】ブロック方式（追加ボタンの非活性かエラー表示か — 仕様に明記なし・フォームにより異なる可能性） |
| 2 | 必須+任意フィールドを入力して追加 → (C2) 一覧反映・再読込 | 永続する。【要実測】時刻の保存形式（HH:mm 入力の HH:mm:ss への補完 — 仕様に明記なし） |
...
| デイリーバイタル (hospitalization-daily-vitals) | `/hospitalization/:id` デイリー記録タブ | time | 親 daily_records(hospitalization_id, date) | 体温に 50 を入力して保存 →【要実測】範囲チェックの有無（仕様に明記なし） |
...
- クロステナント隔離はスコープ外（BE isolation テスト正本）。finalized ロック・監査証跡は S06、異常値ハイライトは S02、次回予定自動計算は S03、入院サイクルは S05 へ委譲。NG 項目は `todo.md`「バグ台帳」節へ BUG-XXX として起票する（[README.md](README.md) のルール）。
...
| §10 ワクチン | 未来日接種の拒否・次回予定日の境界（接種日と同日は拒否） |
