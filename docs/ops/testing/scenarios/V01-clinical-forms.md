...
| 3 | 補助説明を入力し、「次回予防接種予定設定」ラジオを切り替え、次回予定日を手入力して保存 → 独立画面から同じ接種記録を再オープン | ラジオの切り替えだけでは次回日付は自動で変わらない。補助説明・ラジオの選択値・手入力した次回予定日が永続し、初期表示される |
...
| 6 | 検索条件「ステータス=退院済み」の入院を開いて編集保存（無ければ S05 側で確認しスキップ可） | 【要実測】現行バックエンドの通常更新には discharged ステータスを理由とする拒否ガードが見当たらないため、保存可否と退院履歴の完全性を確認する（仕様に明記なし） |
...
- クロステナント隔離はスコープ外（BE isolation テスト正本）。finalized ロック・監査証跡は S06、異常値ハイライトは S02、次回予定自動計算は S03、入院サイクルは S05 へ委譲。NG 項目は [`3-session-agent.html` の実装タスク台帳（正本）](../../../../3-session-agent.html#ledger) の `<!-- LEDGER:APPEND -->` 直前へ `<section class="task" id="BUG-XXX">` として起票する（[README.md](README.md) のルール）。
