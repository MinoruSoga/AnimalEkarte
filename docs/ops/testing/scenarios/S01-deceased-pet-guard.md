...
| 2 | 新規予約: 予約管理の予約登録フォームで患者検索し対象ペットを表示 | 一覧でグレーアウトされ、選択が物理的に無効化される（[02-reservations.md](../../../spec/screens/02-reservations.md)「死亡ペットの保護」） |
...
- 死亡登録の保存はサブダイアログ確定だけでは完結しない — 外側フォームの「更新」まで押して初めて永続する（誤認リスクは BUG-407 として起票済み・2026-07-17）。手順 1 実行時はこの点に留意。
