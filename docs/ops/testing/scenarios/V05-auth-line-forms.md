...
- ローカルの使い捨て clinic に auth/LINE/L-step 用の attached test account と合成 owner/pet/reservation fixture を作成する。パスワード変更/再設定は専用 account だけで行う。
- LIFF/line-reserve はローカル mock lane を使う。実 LINE token が必要な項目は USER 管理 lane がなければ BLOCKED。
- L-step connection test は既知の dummy credentials、`LSTEP_WRITE_API_ENABLED` off、または mocked endpoint のいずれかを必須とする。保存済み実 credential や real outbound API を使わない。
- Lステップ/LINE予約の settings/page editor は V05 が唯一の owner。V04 では実行・集計しない。
...
| 4 | 現在のパスワードを誤って入力し、妥当な新パスワードで保存 | ダイアログ内にエラーが表示され保存されない。全画面クラッシュしない（`isAxiosError` を axios 本体から import） |
...
| 5 | 手順 4 で使用済みのトークンで再度確定 | 拒否される（ワンタイムトークン・有効期限 1 時間 — [21-login.md §2](../../../spec/screens/21-login.md)。期限切れを実行できない場合は BLOCKED として run report に理由を記録） |
...
| 4 | 当日（直前）の予約でキャンセルを試行 | **キャンセル期限なし**。FE `MyReservationsPage` は confirmed なら常にキャンセル UI。BE `CancelReservation` → `CancelByID` は non-cancelled 行の status 更新のみで、当日/リードタイム/直前判定は存在しない。当日直前もキャンセル成功し得る |
...
|:--|:--|:--|:--|:--|:--|
| V05-13 | 配信優先順位（`lstep-trigger-priority`） | `/settings/integrations/lstep` 内セクション | 各トリガーの優先順位（1 以上） | 同値可（同一優先階層） | 0 を入力 → 「優先順位は1以上を指定してください」で保存されない（C1 — `TriggerPrioritySection.tsx`）。変更時のみ保存ボタン活性。同値は許可（UI 文言「同値は同一優先階層として扱われます。」） |
