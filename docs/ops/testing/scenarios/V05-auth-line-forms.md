...
|:--|:--|:--|:--|:--|:--|
| V05-13 | 配信優先順位（`lstep-trigger-priority`） | `/settings/integrations/lstep` 内セクション | 各トリガーの優先順位（1 以上） | 未確認 | 0 を入力 → 「優先順位は1以上を指定してください」で保存されない（C1 — `TriggerPrioritySection.tsx`）。変更時のみ保存ボタン活性。【要実測】同値優先順位の可否（C3(b)） |
...
| V05-15 | タグ設定（`lstep-tag-config`） | 同上（追加フォーム 2 種） | フォーム1: プレフィックス+カテゴリ / フォーム2: 疾患コード+タグ名 | 未確認 | 片方空で追加 → 「プレフィックスとカテゴリは必須です」「疾患コードとタグ名は必須です」（C1 — `LstepTagConfigSection.tsx`）。追加 → 再読込永続 → 行削除（C2）。【要実測】同一プレフィックス/疾患コードの重複追加可否（C3(b)） |
