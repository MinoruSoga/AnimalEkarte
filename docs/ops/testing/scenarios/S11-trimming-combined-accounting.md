...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | トリミング一覧から新規登録 → 対象ペットを選択し、ステータス「予約」・コース 1 件・オプション 1 件を選択して保存する（[17 §1.1](../../../spec/screens/17-trimming-form.md)・#233） | 保存成功。コース・オプションにマスタ定義の価格が表示される。record_shortcut 既定時刻は固定 10:00 ではなく一意な JST 現在時刻（秒・ミリ秒込み）。同じ担当で同日に別ペットを続けて登録しても `uk_appointment_staff_time` の 409 で止まらない |
