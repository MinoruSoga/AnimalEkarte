...
```mermaid
flowchart LR
    A["新規会計・全額精算"] -->|"POST complete（同一TX・Idempotency-Key）"| B["completed 精算済"]
    B -->|"クレジット訂正（カード系内訳のみ）<br/>理由必須・権限制御・監査同一TX<br/>締め済み期間は post_close 記録"| B
    B -->|"訂正差額 residual"| U["未納者一覧に計上"]
    W["waiting 会計 全額"] --> U
    P["部分入金"] -.->|"FE disabled / BE 400"| X["拒否"]
```
