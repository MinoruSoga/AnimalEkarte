...
| 3 | 標準時刻: 境界逆転（平日終了 < AM/PM 境界）で保存 | **拒否される**（PATCH 400。境界 14:00 + 平日終了 12:00 を例示）。BE `UpdateStandard` が `validateStandardClosingTimes` を呼び「平日締め終了時刻(…)は境界時刻(…)より後に設定してください」を返す（`closing_settings_service.go`・回帰テスト `closing_settings_update_standard_integrity_test.go`）。受理された旧観測は廃止 |
