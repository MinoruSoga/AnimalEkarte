...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | LIFF 予約アプリを開き、新規予約を開始する | トップページが表示され予約フローへ進める。二回目以降は氏名・電話番号等が自動補完される（§1） |
...
| 6 | 枠を選択 → ペット選択で飼い主に紐づくペットを選ぶ → 予約確定 | 予約が完了する（§2-4/§2-5）。飼い主 LINE へ完了メッセージ、病院へ受付通知が送信される（§5。モック時は送信確認のみ・受信確認は STG） |
