...
```mermaid
flowchart LR
    A["顧客情報（ペット選択含む）"] --> B["コース"]
    B --> C{"コース種別"}
    C -->|"診察系"| D["スタッフ"]
    C -->|"トリミング系"| T1["TrimmingCourseSelectPage"] --> T2["TrimmingOptionSelectPage"] --> D
    D --> E["日付"]
    E --> F["時間"]
    F --> G["ご要望"]
    G --> H["確認"]
    H --> I["完了"]
    H -.->|"直前で枠が埋まっていた場合"| R["インライン notice で再選択"]
    F -.-> N["空き枠は勤務時間内のみ・医院設定間隔で提示<br/>休憩・既存予約と重なる枠は出ない"]
```
