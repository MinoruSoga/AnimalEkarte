...
| 4 | 後片付け: 死亡記録の解除ボタンから `DELETE /pets/:id/death` を実行（generic PATCH status は使わない） | 解除され生存として表示される。死亡中の各種新規操作ブロックの検証は S01 が正本 |
...
| 4 | メールなし・氏名のみで accountless の新規スタッフ「V03スタッフ」を保存する。別の新規 fixture ではメールアドレスと英数字混在 8 文字以上のパスワードを指定して保存 | **メール/パスワード任意**（accountless 可）。氏名のみで保存成功。メール指定時はパスワード必須 |
...
- NG 項目はリポジトリ直下 [`todo.md#product-bugs` の確認済み製品不具合](../../../../todo.md#product-bugs) へ `### BUG-XXX` 節として起票する（ローカル連番 最大+1・[README.md](README.md) のルールに従う）。
