...
| 3 | (C1-3) 値引率に 100 → 保存、101 → 保存 | 100 は受理、101 は拒否（FE/BE とも 0〜100） |
...
| 2 | 新規「V03グループ」を作成し、説明・色・複数リソースの view/create/edit/delete をチェックして保存 → (C2) 一覧 → 再読込 → 再オープン | マトリクスのチェック状態を含め C2-1〜C2-3 のとおり永続・初期表示 |
| 3 | (C3-2) 既存グループと同名のグループを新規作成 | 保存されない（`permission_groups(clinic_id, name)` UNIQUE・deleted_at IS NULL 限定）。【要実測】エラー整形の経路（409 として表示されるか 500/白画面か）— 後者なら BUG 起票 |
...
| 4 | (C1-3) 既存医院の通常税率・軽減税率を範囲内/範囲外の値で保存 | BE は 0〜1 の小数で範囲検証する。【要実測】FE の入力形式（% 表記か小数か）と範囲外入力の拒否挙動 — 実測して仕様化 |
| 5 | 郵便番号・メールに形式違反を入力して保存 | 【要実測】clinic フォームは FE/BE とも形式チェックなしの見込み（素通し想定）。実挙動を確認し、業務影響があれば BUG 起票 |
