...
| 3 | 副作用: スタッフの所属に「V03医院」を追加（§5 手順 5 の導線）→ 医院を「V03医院」へ切替 → `/settings/permission-groups` を確認 | デフォルト権限グループ「執行」「一般」が `defaultPermissionRuleTable` 付きで自動作成されている（[master-permission-group.md §2](../../../spec/screens/settings/master-permission-group.md)。未付与ではない） |
