...
- ローカルの使い捨て clinic に、独立画面の vaccinations create/edit 権限を持つ attached account（カルテタブも確認する場合は medical-records 権限を追加） と生存犬を作成する。
...
| 7 | 同じペットにフィラリア予防を新規登録する | フィラリア等の予防も同じ予防接種機能で管理される（[14 概要](../../../spec/screens/14-vaccinations-list.md)「フィラリア・ノミダニ予防等」）。フィラリア薬選択後も次回予定候補は **3週後 / 4週後 / 1年後 / 以外（手動）**（ワクチンと同一セット。選択時の既定値はマスタ間隔に従う）。保存・再読込で対象マスタ名と次回日が保持されることを確認する |

```mermaid
flowchart TB
    V["ワクチンマスタ選択 — 間隔が設定済みなら next_schedule_type と候補が更新"] --> S{"next_schedule_type"}
    S -->|"3weeks / 4weeks / 1year"| C["calculateNextDate — 実施日 + 間隔で自動計算"]
    S -->|"other（以外・手動）"| M["カレンダーで任意日を直接指定"]
    C --> D["次回予定日 next_date を表示"]
    M --> D
    D --> G{"next_date が接種日より後"}
    G -->|"同日・前日は拒否"| X["保存されない"]
    G -->|"受理"| L["保存 → 一覧の「次回予定」列に表示"]
```
