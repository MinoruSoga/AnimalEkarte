...
業務フロー横断の S シリーズと別軸で、**全永続化フォーム（83 フォーム棚卸し済み）の入力チェック・更新チェック・DB 存在チェック**をドメイン別に検証する。各ファイル冒頭の「共通チェック手順」（C1 入力 / C2 更新永続 / C3 DB 存在・FK・一意制約）を全フォームに適用し、フォーム固有の検証は個別セクション・同型マスタは差分表で扱う。異常系は C1/C3 の共通手順に埋め込まれている（S シリーズの「## 異常系」節とは構造が異なる）。
...
| [V04](V04-settings-master-forms.md) | /settings 配下のデータ駆動マスタ全数 | マスタ | 29 |
...
- **AI 実行**: browser-test スキルで実行可能な粒度で書かれている。NG 項目はリポジトリ直下 `todo.md` の「バグ台帳」節（正本・旧 bug.md）へ BUG-XXX として起票する（バグではない対応事項は todo.md の他節または q&a.html（USER 操作・決裁））。
