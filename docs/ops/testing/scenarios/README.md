...
- **AI 実行**: browser-test スキルで実行可能な粒度で書かれている。NG 項目はリポジトリ直下 [`bug.md` の受入テストバグ台帳（正本）](../../../../bug.md) の末尾へ `## BUG-XXX:` 節として起票する（ローカル連番 最大+1。バグではない対応事項は [`todo.md`](../../../../todo.md) の `### TASK-XXX:` 節、USER 操作・決裁は q&a.html）。
