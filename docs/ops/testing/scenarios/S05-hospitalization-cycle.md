...
| # | 操作 | 期待結果 |
|:--|:--|:--|
| 1 | 入院管理から新規入院登録 → ペット選択 → 入院タイプ「入院」・予定期間・空きケージ・主訴（および任意で担当医）を設定し、治療プランに給餌指示と投薬指示を各 1 件登録して保存（[09 §1/§2](../../../spec/screens/09-hospitalization-form.md)） | 保存成功。入院一覧の「入院中」タブに表示される。リストビューでは主訴・担当医列に入力値が載る（[07 §1/§2](../../../spec/screens/07-hospitalization-list.md)） |
...
- runtime 2026-08-07: **PARTIAL** — auth OK; Playwright `hospitalization-flow` 4/4 PASS (list/select-pet/status tabs). Discharge-with-billing atomic fail (A2) **not executed** (needs dedicated data).
