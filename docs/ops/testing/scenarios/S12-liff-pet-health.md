...
| 3 | 連携済み UI で発行導線を探す / 解除後または API で再連携を試す | **連携済み UI には発行無し**。409 は BE linked 拒否経路で確認 |
...
| 6 | ペットカードの表示内容を確認する | ペットごとに、ペット名・種/品種・最終来院日（記録がない場合は「記録なし」）・ワクチン記録テーブル（ワクチン名/接種日/次回予定日、予定なしは「—」）が表示される。表示項目は実装確認済みの上記のみ — これ以外の健康情報（予防状況サマリ等）の表示有無は【要実測】。**runtime 2026-08-01 BLOCKED**: `/liff/` → `GET /api/liff/1/profile` **401**（カード未描画） |
| 7 | 別の飼主（前提条件の 2 人目）の LINE アカウント／トークンで同様に連携し、ペットヘルスを開く | その飼主自身のペットのみが表示され、手順 6 の飼主のペットは一切表示されない — 飼主間隔離の実機証明 |
| 8 | `clinic_id` クエリなしの URL でペットヘルスを開く | エラー表示（resolveFetchError で文言正規化され得る）。他テナントデータは出ない |
