...
手順はリポジトリの schema と認証の所属参照経路に静的照合した。2026-09-10 に migrate 済みの disposable `auth_d1_db` で、success / 既存 admin 競合 / email 競合 / audit rollback の 4 ケースを実行した。この証拠は disposable 環境に限られる。対象環境は未定であり、本番への実付与・実環境での存在確認・対象環境での SQL 実行・本人の通常 login・メール経路確認は **未実施**。対象環境・担当者・資格情報の受領と実行は別承認が必要である。
