...
- **管理**: 調査用途、作成者、削除予定日をリポジトリ直下 [`3-session-agent.html` の実装タスク台帳（正本）](../../../3-session-agent.html#ledger) の該当タスク節に記録
...
記録内容（[`3-session-agent.html` の実装タスク台帳（正本）](../../../3-session-agent.html#ledger) の該当タスク節または `audit_logs`）:
- 削除日時（JST）
- 削除した レコード種別 + ID + 件数
...
| **4** | 残置理由が文書化されている | [`3-session-agent.html` の実装タスク台帳（正本）](../../../3-session-agent.html#ledger) の該当タスク節に記載 | ✅ |

**許容例**:
- 調査結果と API cleanup 手順を [`3-session-agent.html` の実装タスク台帳（正本）](../../../3-session-agent.html#ledger) の該当タスク節に記録し、「2026-07-24 18:00 JST までに削除」と明記
