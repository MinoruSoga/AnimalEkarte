...
Demo login account は `002_master` には含まれず、許可された `APP_ENV` の migrate フェーズ3（`internal/seedlogin`）で合成アカウントを upsert する（医院ごとに一般9。執行の林 文明は1人（catalog 4医院所属））。設定操作用アカウントは [STAFF_ACCOUNT_PROVISIONING.md](./STAFF_ACCOUNT_PROVISIONING.md) の承認済み運用で別途払い出す。
