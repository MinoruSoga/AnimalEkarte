...
| `REHEARSAL_ONLY` / `PARTIAL` の暫定21表bundle | `backend/migrations/seeds/_old_db_handoff/<clinic>/`（`make old-db-handoff-stage`） | このworktreeだけのローカル保管。`cmd/migrate` / `make seed` からは読まれない。手順は [OLD_DB_HANDOFF_LOCAL.md](./OLD_DB_HANDOFF_LOCAL.md) |
