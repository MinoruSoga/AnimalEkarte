...
```mermaid
flowchart TB
    subgraph Vol["月間配信ボリュームの試算カテゴリ"]
        A["高優先度・必須配信<br/>再診リマインド / ワクチン期限案内"]
        B["関係構築・CRM<br/>初診お礼 / 誕生日祝い"]
        C["健診抽出・キャンペーン<br/>手動バルク（実施月のみ）"]
    end

    Vol --> Cost["配信コスト"]
    Cost --> M["LINE 公式アカウント（Messaging API）<br/>月額固定 ＋ 無料枠超過分の従量課金"]
    Cost --> L["Lステップ（拡張ツール）<br/>プランに応じた月額固定費"]
```
