...
```mermaid
flowchart TB
    subgraph Screen["この画面の構成"]
        Company["法人情報セクション<br>インボイス登録番号・単一"]
        List["医院一覧"]
        Panel["詳細編集サイドパネル<br>拠点ごとの登録番号・基本情報"]
        List --> Panel
    end
    Company -->|保存すると即座に反映| Doc["全拠点の会計伝票・領収書"]
    Panel --> Clinic["拠点レコード"]
    Clinic -->|所属の選択肢| Staff["スタッフの所属"]
    Clinic -->|テナント分離の源泉| Tenant["診療・会計データの clinic_id 境界"]
```
