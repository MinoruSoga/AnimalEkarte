...
### 2. デフォルトグループの提供
新規院開設時に「執行」と「一般」の 2 グループが、**デフォルトの権限ルール付きで**自動作成されます（`clinic_service.go` の `defaultPermissionRuleTable`。SD-9 対応・2026-07-17）。ルールの内容は seed 003_demo の既存パターンを踏襲し、おおむね「執行 = 全業務リソースのフル権限（削除含む）／一般 = 閲覧＋主要業務の作成・編集（削除・会計確定・設定系は不可）」。開設直後から system_admin 以外もログイン運用でき、細部は本画面で調整します。
