...
```mermaid
flowchart TB
    subgraph master["トリミングマスタ（本画面）"]
        course["コース<br/>コース種別・対象サイズ・所要時間・単価"]
        option["オプション<br/>組合せ可否・所要時間・単価"]
    end
    course --> sel["LINE 予約 LIFF / 院内予約画面<br/>の選択肢（有効化したもの）"]
    option --> sel
    course --> rec["トリミング記録画面"]
    option --> rec
    rec --> bill["会計明細<br/>単価分岐は対象サイズ区分のみ"]
```
