...
### 直接ナビゲーション
- **飼主詳細へ**: 飼主単位・月次繰越表示で行をクリックすると、連絡先や全履歴を確認できる `/owners/:id` へ遷移します。
- **会計詳細へ**: 会計単位表示で行をクリックすると、精算処理を行える `/accounting/:id` へ直接遷移します。
...
### パフォーマンスと拡張性
- **URLパラメータ同期**: 期間（`start_date`/`end_date`）・対象月（`month`）・表示単位（`group_by`）の設定は URL と同期され、共有やブックマークが可能です。
