...
```mermaid
flowchart TB
    A["Path A カルテ詳細の定期健診タブ"] -->|診療コンテキストに紐付けて保存| S["健診レコード"]
    B["Path B 一覧の新規登録"] --> Pet["ペット選択"] --> F["健診フォーム"]
    F -->|実施日を診察日とするカルテを自動生成して紐付け| S
    C["Path C 一覧からの編集"] -->|カルテの定期健診タブへ遷移| E["対象行を編集状態で表示"]
```
