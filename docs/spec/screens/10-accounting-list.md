...
```mermaid
flowchart TB
    A["/accounting（UnifiedTabs）"]
    A --> T1["会計一覧タブ<br/>本仕様書の一覧・検索ビュー"]
    A --> T2["当日会計タブ ?tab=daily<br/>指定日の精算済内訳・日計集計・印刷"]
    A --> T3["未納者一覧タブ ?tab=unpaid<br/>売掛金のある顧客を抽出"]
    T1 --> D["/accounting/:id<br/>詳細確認・支払い処理・キャンセル"]
    T1 --> N["新規会計登録<br/>診療外の物販等"]
```
...
### 3. 当日会計タブ（日次集計テーブル）
- **画面上の日次集計テーブル**: `領収No` → （複数拠点選択時のみ `拠点`）→ `診療` → `外科` → `RV` → `フード` → `トリミング` → `ホテル` → `用品他` → `売上合計` → `支払方法` → **`飼主名` → `ペット名`**。飼主名・ペット名はテーブル最右端の2列（この順）で、支払方法はその左隣。フッターの合計行はカテゴリ別合計と売上合計を表示する。
- **印刷面（日次集計一覧表・A4 landscape）**: `領収No` → `診療` → `外科` → `RV` → `フード` → `トリミング` → `ホテル` → `用品他` → `支払方法` → `合計` → **`飼主名` → `ペット名`**。画面と同様に最右端2列（この順）。フッターには病院合計・トリミング合計・全体合計の3行を出力する。
