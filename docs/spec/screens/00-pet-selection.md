...
- **危険マーク（EMR-173、共有 `DangerBadge`）**: ペット危険度はペット名列に表示 — `danger_level` 高=赤 `⚠ 危険`（Popover で `危険理由` = `danger_reason`、空は `理由未登録`）、中=黄 `⚠ 注意`（Popover で `注意理由`）、低・未設定は非表示。飼主 `is_dangerous` は飼主名列の氏名横に `⚠ 危険人物`（ペット危険度とは別概念・理由 Popover なし）。いずれもスタッフ向け表示で、LIFF・owner 向け契約には出さない。
