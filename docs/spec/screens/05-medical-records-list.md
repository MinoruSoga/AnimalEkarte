...
stateDiagram-v2
    state "作成中" as draft
    state "確定済 (finalized)" as fin

    [*] --> draft : 予約確定・受付済みで自動作成 (best-effort)
    draft --> fin : 確定
    fin --> fin : 訂正追記 (addendum)
    note left of draft : トリミング・入院/ホテル予約は自動作成対象外
    note right of fin : 更新・削除はサービス層が拒否
```
