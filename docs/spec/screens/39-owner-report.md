...
```mermaid
flowchart TB
    A["別ウィンドウで /owners/:id/report?petId= を開く"] --> G{"ページ権限: medical-records:view と owners:view の両方が必要"}
    G -->|未認証| L["ログインへ遷移"]
    G -->|不足| NG["キャッシュ済みの飼主・ペット情報も描画しない"]
    G -->|両方あり| R["固定ヘッダーと全領域を同じビューポートに描画"]
    R --> P{"各リソースの view 権限を確認"}
    P -->|あり| S["領域に情報を表示"]
    P -->|なし| D["その情報だけ「閲覧権限なし」に縮退"]
```
