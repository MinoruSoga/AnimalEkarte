...
- 治療系の診療日は medical_records.date 由来（treatments.created_at ではない）で、`toJSTWallDate` により JST 壁日付で整形する。
- 各セクションのヘッダー右に件数バッジを表示（閲覧可・取得済み・非エラー時のみ）。
- 履歴系 API の取得上限は `HISTORY_FETCH_LIMIT`（100 件）。それより古い履歴はページングなしのため表示されない（予防接種のみ limit 未指定）。
...
### 2.3 印刷・共有導線
専用の印刷ボタン・印刷用スタイル・共有機能は現状存在しない。印刷はブラウザ標準機能に委ねる運用となる（lg 以上はパネル内部スクロールの固定ビューポートのため、長い履歴は印刷で切れる制約がある）。
