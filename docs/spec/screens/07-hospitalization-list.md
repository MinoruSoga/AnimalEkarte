...
```mermaid
flowchart TB
    U["画面操作"] --> P{"hospitalization 権限"}
    P -->|"view"| V["閲覧のみ<br>・ボード/リスト表示<br>・入院Noリンクで詳細 (死亡時はリンクなし)<br>・カード open / DnD / 操作ボタンは無効"]
    P -->|"edit (canEdit)"| E["操作可能<br>・ボードカード click で詳細 (在室・非死亡時)<br>・DnD でケージ移動 (使用中はスワップ)<br>・リストの操作列ボタン"]
```
