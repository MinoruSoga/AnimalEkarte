...
**既存会計の再保存に関する現行制約**: 画面の既存会計保存処理は `completed_at` を PATCH に含めるが、バックエンドはその指定を400で拒否する。精算済み会計の再保存にもこの不整合があり、確認モーダルが表示されることを修正成功の証明にしない。新規会計の専用確定経路とは区別する。根拠は `use-accounting-completion-action.ts`、`update-accounting.ts`、`backend/internal/billing/accounting_service_core.go`。実環境での再現は未確認。
...
- 別ドメインのAPIへ接続する環境では、このヘッダーのCORS許可も必要。現行allowlistの不足は [Vercel STG検証手順](../../ops/deploy/VERCEL-FRONTEND-STAGING-TEST.md) を参照する。
