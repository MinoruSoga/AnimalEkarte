...
flowchart TB
    S["保存操作"] --> Q{"新規会計の決済確定か"}
    Q -->|はい| C["POST /accountings/complete<br/>Idempotency-Key（UUID）必須<br/>明細・支払・監査を原子確定"]
    Q -->|いいえ| P["PATCH /accountings/:id<br/>既存会計の更新・精算済データの修正<br/>completed_at は送らず最初の確定日時を維持<br/>未確定会計の決済確定は不可"]
    C --> K{"同一医院・同一キーで再送"}
    K -->|同一内容| R1["既存会計を返す<br/>支払・監査は再作成しない（初回201 / 再送200）"]
