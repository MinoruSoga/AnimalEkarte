...
stateDiagram-v2
    direction LR
    state "予約 (reserved)" as R
    state "入院中 (admitted)" as A
    state "退院済 (discharged)" as D

    R --> A : チェックイン (edit 権限・確認ダイアログなし・死亡ペットは拒否)
    A --> D : 退院処理 (edit 権限)
    note right of D
        「退院後、そのまま会計画面へ進む」チェック時は
        ケアプラン明細を取り込んだ会計を作成し会計詳細へ遷移。
        未チェックはステータス更新のみで入院一覧へ戻る。
        退院済からの再チェックインは不可。
    end note
```
