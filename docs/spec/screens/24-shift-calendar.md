...
```mermaid
flowchart TB
    Grid["シフトグリッド<br>スタッフ行 × 日付列"]
    Grid -->|セルをクリック| Form["シフト編集フォーム<br>種別・時刻・複数休憩"]
    Grid -->|日付ヘッダーをクリック<br>権限保有時のみ| Holiday["医院休診日の設定・解除"]
    Form -->|保存| Shifts["シフトデータ"]
    Shifts -.->|次回計算時に参照| Slot["LINE予約の空き枠 API"]
```
