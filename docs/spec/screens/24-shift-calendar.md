...
### 2. 医院休診日管理
カレンダーの日付ヘッダーをクリックすることで、医院全体の休診設定を行えます（`create` 権限保有時のみクリック可能）。
...
- **複数休憩時間**: 1日のシフトの中に、複数の休憩時間（例：午前休憩、午後休憩）を登録可能です。休日選択時は時刻・休憩入力が非活性になります。
- **有休（paid_leave）の時刻は任意入力**: 休日と異なり時刻入力欄は非活性化しません。時刻を空欄のまま保存すると終日休扱い、時刻を入力すると半休（時間単位有休）として扱われる仕様です（2026-07-17 決裁）。
