...
### 2.1 危険個体の視覚的警告
`danger_level` が設定されたペットは、各一覧・ヘッダーにおいて、即座に目立つ警告バッジが表示され、スタッフの安全確保を促します（EMR-173、共有 `DangerBadge`）。
- **表示ルール**: 「高」= 赤 `⚠ 危険`、「中」= 黄 `⚠ 注意`、`低`・未設定 = バッジ非表示。色だけに依存せず ⚠ 記号と文言で段階を識別します。
- **理由の開示**: 危険度バッジは、クリック / タップ / Enter / Space のいずれか1操作で開くアクセシブルな Popover とし、`danger_reason` を表示します（高は `危険理由`、中は `注意理由` 見出し）。理由が空の既存データでは `理由未登録` を表示します。
- **飼主の危険人物マーク**: `owners.is_dangerous` が true の飼主は氏名横に `⚠ 危険人物` を表示します（理由 Popover なし・ペット危険度とは別概念）。
...
```mermaid
flowchart LR
    O["飼主 (owners)"] -->|"is_dangerous"| OD["危険人物<br>人への対応上の注意"]
    P["ペット (pets)"] -->|"danger_level + danger_reason"| PD["動物取扱注意<br>高のみ理由必須<br>owner-facing 応答には含めない"]
    OD -. "別概念・必要なら併存" .- PD
```
