...
### 2.2 臨床連携
診察室（カルテ）の見積タブ (`MedicalRecordEstimate`) は、本画面（`/estimates/new` `/estimates/:id/edit`）とは別実装のフォームです。品目名・単価・数量を自由入力する明細テーブルを持ち、消費税は一律 10% で自動計算されますが、診療項目マスタからの選択機能はありません。
