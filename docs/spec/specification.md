...
- **整合性の死守**: 全 **110 テーブル**のデータ変更（CRUD）を `audit_logs` で追跡。臨床記録の真正性を担保するための確定（Lock）フローを実装（カルテ画面の確定ボタン＋確認ダイアログによる明示操作、確定後は編集 UI を無効化。確定の解除は不可・訂正は追記のみ。詳細: [screens/06-medical-records-form.md §2.3](screens/06-medical-records-form.md)）。
...
- **バックエンド**: Go 1.25 / Gin / GORM による堅牢な API（**110 テーブル**。ハンドラー実装は ADR-006 に基づき domain package へ移行中のため、ファイル数指標は廃止）。
