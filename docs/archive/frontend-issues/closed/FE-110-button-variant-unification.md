...
- **変更ファイル**:
  - `frontend/src/components/ui/button.tsx` — `variant="primary"` 追加（bg-[#2EAADC]）
  - `frontend/src/components/shared/FormDialog/FormDialog.tsx` — 保存ボタンを variant="primary" に変更
  - `frontend/src/features/hospitalization/components/CarePlan/CarePlanSection.tsx` — プラン追加ボタンを variant="primary" に変更
  - `frontend/src/features/hospitalization/components/DailyRecord/TimingSection.tsx` — 実施するボタンを variant="primary" に変更
  - `frontend/src/features/hospitalization/components/DailyRecord/SimpleNoteForm.tsx` — 記録ボタンを variant="primary" に変更
- **残存例外**（意図的）:
  - StaffSelectionModal.tsx: 選択状態トグルの条件クラス（静的 variant に変換不可）
  - HospitalizationBoard.tsx: D&D エリアのリング・背景色（ボタンではない）
