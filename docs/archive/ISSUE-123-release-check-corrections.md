...
> 訂正の要約コメントは Issue #123 に投稿済み（§6.1）。Issue 本文（チェックリスト原文）の編集は別途承認が必要なため未実施（§6.2）。本書は**ローカル訂正**と**ローカル Docker による代替検証結果**を集約する。
>
...
## 6. GitHub Issue #123 への反映

GitHub への反映は「訂正コメントの投稿」と「Issue 本文（チェックリスト原文）の編集」の2段階に分かれる。現状は前者のみ実施済みで、後者は未実施。

### 6.1 訂正コメント投稿 — ✅ 実施済み

§1 の2件（P1-3 / P1-5）を要約した訂正コメントを Issue #123 に投稿済み（2026-06-22 JST）。テスターには「コメント記載を正として実施せよ」と周知済み。

- 参照: https://github.com/MinoruSoga/AnimalEkarte/issues/123#issuecomment-4762897829

### 6.2 Issue 本文（チェックリスト原文）の編集 — ⏳ 未実施（要承認）

Issue 本文・原チェックリストの該当箇所は**まだ原文のまま**であり、誤記述（`status = 'unpaid'` 等）が残っている。本文編集は外部書き込みであり明示承認が必要なため未実施。コメント（6.1）で運用上はカバー済みだが、本文を正にしたい場合の推奨編集テキスト:
