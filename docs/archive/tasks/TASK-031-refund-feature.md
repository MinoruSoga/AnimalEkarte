...
| # | サブタスク | 領域 | イシュー | 依存 | 完了 |
|---|----------|------|---------|------|------|
| 1 | billing_refunds テーブル + BillingRefund モデル + refund API (GET/POST) | BE | BE-062 | - | [x] |
| 2 | 会計一覧「返金あり」バッジ + 会計精算画面 返金フォーム・履歴・残額表示 | FE | FE-126 | #1 | [x] |

## 受入条件（Acceptance Criteria）

- [x] AC-1: 会計精算済み（completed）の会計詳細画面に「返金する」ボタンが表示される
- [x] AC-2: 返金フォームに返金額（必須）・返金理由（任意）を入力して送信すると `POST /accountings/:id/refunds` が呼ばれ、返金レコードが保存される
- [x] AC-3: 返金後、返金履歴に追加された返金が即座に表示される
- [x] AC-4: 返金可能残額 = total_amount - Σrefunds.amount が正しく表示される
- [x] AC-5: 返金可能残額が ¥0 になると「返金する」ボタンが無効化される
- [x] AC-6: 部分返金を複数回実施できる（例: ¥5,000 の請求に ¥2,000 → ¥3,000 の順で返金可能）
- [x] AC-7: 会計一覧で返金が1件以上ある会計行に「返金あり」バッジが表示される
- [x] AC-8: waiting / cancelled 状態の会計には返金ボタンが表示されない
- [x] AC-9: 返金額が返金可能残額を超える場合、BE がエラーを返し FE にトーストで表示される
