...
| clinic_id保持済 | `user_clinic_memberships`, `permission_groups` のみ現時点で保持（`user_permissions` は廃止済み） |
...
**clinic_id を保持するテーブル（全体）:** `user_clinic_memberships`, `permission_groups` + 上記25テーブル（計27テーブル）。
