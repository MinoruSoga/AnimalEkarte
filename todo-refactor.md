...
更新日: 2026-09-02（90–99行の公開関数抽出まで完了）

| 項目 | 値 |
|------|-----|
| **範囲** | backend の **production Go**（`*_test.go` と `cmd/_archive/` は対象外） |
| **目的** | 挙動を変えずに、プロジェクトの Go 規約へ寄せる |
| **ブランチ** | `main`。第1弾 `739207a1f`（ファイル分割）。第2弾 `8fa232d09`（150行超関数）。第3弾 `e4f47fbba`（140行超関数）。第4弾 `b94f1dd7e`（130行超関数）。第5弾 `56df7c429`（120行超関数）。第6弾 `994055a95`（110–119行の公開関数）。第7弾 `69b56c06a`（残110行と100–109行の公開関数）。第8弾は続くコミット（90–99行の公開関数） |
...
| `ChangePassword` のファイル外抽出 | source-scan 契約が `account_service.go` 内の WithTx → CAS → token delete → audit 順を固定している |
| middleware `auth` / `LiffAuth` | 認証ミドルウェア。分割すると fail-closed 契約が読みにくくなる |
| 90 行未満まで機械分割 | 規約ゲートは 150。50 行ルールは個人コーディングスタイルであり台帳ゲートではない |
...
- 第8弾の抽出例: バイタル Create tx、current access identity、staff CreateWithAccount tx、薬剤 Delete tx、カルテ subrecord upsert、検査 Create tx、返金 Create tx、LIFF 空き日 slot filter
...
- compile-only / `go vet`: auth, staff, billing, reservation, medicalrecord
- `./internal/auth` `./internal/staff` `./internal/billing` `./internal/reservation`
- `./internal/medicalrecord -skip TestLabDeviceConnectivityDocsMatchRuntime`
...
- 90 行超の公開関数は直前スライスの tx helper・ChangePassword・middleware・quota/SQL/route 表を除きゼロ
