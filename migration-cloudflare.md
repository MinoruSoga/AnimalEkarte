...
- [ ] **P7-2** 並行稼働期間 — 開始（2026-07-17〜）。旧 ECS/ALB/RDS は温存・夜間停止スケジュールも従来どおり
  ...
  - **観測 #2 の残課題（地雷・要対応）**: 所有権移転は失敗確定 — 失効済み一時 role の ADMIN を保有する role が存在せず `GRANT`/`REASSIGN OWNED` とも permission denied（109 オブジェクトが `pscale_api_dm6y13euvxzj` 所有のまま）。**既存テーブルへの ALTER TABLE 系 migration が来た時点で権限エラーになる**。解消手段は 2 択: (a) PlanetScale サポートに REASSIGN を依頼（推奨・無停止） (b) `pscale role reset-default` で得た恒久 role 資格情報を使い DROP SCHEMA → フルデモ再投入をやり直す（所有者が恒久 role になる。wrangler secrets の DB_USER/DB_PASSWORD 更新が必須・約 40 分）。直近の既知 migration（#249 P2 = CREATE TABLE）は非該当のため即時性はないが、放置期間が長いほど踏む確率が上がる
