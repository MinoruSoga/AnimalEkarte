...
| **旧本文** | git 履歴（統合前の `todo_1.md` 全文を含む） |
...
- STG 実データ運用テストの作業台帳 → 本ファイルの該当節（旧 `todo_1.md` を 2026-08-26 に統合）
...
更新日: 2026-08-26（Lane 2 コードは `4703cf3e9` で main 入り。M-3 の canonical runbook 追記は未適用）。旧 `todo_1.md` を本節へ統合。手順の変数・backup・失敗時は末尾の runbook。行値・パスワードを本書に書かない。エージェントは pscale / STG apply / `DROP SCHEMA` を実行しない。

old_db 側の producer 作業は sibling `old_db/todo.md`。
...
| レーン | 状態 | メモ |
|--------|------|------|
| Lane 0 入力 | 進行中 | H0-1・H0-3a/H0-4 城東・H0-5 ローカルログイン済み。未: 八王子 H0-2 |
| Lane 2 コード | **完了** | `4703cf3e9`。SKELETON / IMPORT / STAFF / Make の DB_HOST・SSL・sentinel 転送 |
| Lane 1 ローカル証明 | 城東 rehearsal 済み | H1-1a write-0 PASS。H1-1b: 2026-08-25 と 2026-08-26 とも USER `make reset` apply+verify PASS（`jouto-intake-20260822-01`）。ローカル rehearsal 経路であり STG UAT `stg-uat-csv-import` apply ではない。八王子未 |
...
`claim/AE-STG-UAT-SKELETON` · `IMPORT` · `STAFF` · `OPS-SHEET` · `MAKE-REMOTE` · `H0-JOU-CHECK` · `H1-JOU-PREFLIGHT` · `H1-LAND` · `JOU-PAY-SNAP` · `JOU-PAY-GRAPH` · `H0-5-ROSTER` · `STAFF-ATTACH-LOCAL` · `LANE3-PREP` · `AE-STG-UAT-REMAINING-TODO` · `AE-SEED-RESTORE-LFS` · old_db `AE-OLD-DB-PAY-SNAP-STAGE` / `PAY-GRAPH-STAGE`
...
| その後 | 本番移行日を決める。その日に旧データを出力し、新システムの本番へ移行する |
...
| STG 並行運用 | 第1段階。本番 cutover とは別レーン |
| 旧会計の異常値 | 補正せずそのまま載せる（本ファイル AE-MIG-NEG-*） |
...
### 3. いまの在庫（件数のみ。行値は書かない）

正本の状態は old_db の医院別ドキュメント。ここは AnimalEkarte 側の要約。

| 医院 | producer 21 CSV | 資格 | AnimalEkarte 側 |
|------|-----------------|------|-----------------|
| 城東 `jouto` | `jouto-intake-20260822-01` あり。飼主約 4.3 万 / ペット約 7.0 万 / カルテ約 114 万 / 会計明細約 498 万 / 検査結果約 797 万 / 支払約 88 万 / 分割約 89 万（header-only **ではない**）。スタッフ 414 行は全員 `is_active=false`。CSV 合計およそ 1.8GB | `REHEARSAL_ONLY` / `UNVERIFIED`。TRUSTED は JOU-CSV-1 で BLOCKED | `_old_db_handoff/jouto/` にローカル rehearsal あり。manifest SHA `7bbda50f06f7d0acac6711d1a73b78ca68b835ee9be5df6cd04f3e6a5094a405`。STG 未投入 |
...
Class A（gitignored、次の `old-db-handoff-stage` で消える）: payments snapshot、completed+nonzero 欠 graph を pending、同一 `medical_record_id` の余剰 192 リンク解除。durable は old_db producer。**stage 再実行しない。**
...
- `old-db-handoff-stage` 再実行（Class A が消える）

### 5. 確定した判断（D1–D7、2026-08-25）

Q2=B と Q5=A は衝突したため、**城東で第1段階を開始**し、八王子 21 表は後追い必須（HAC-CSV-1。identity 無しの 7 CSV は使わない）と読む。

| ID | 判断 | 決定 |
|---|---|---|
| D1 | 城東 rehearsal を STG に載せるか | **載せる。** STG 専用フラグを新設。本番 F6 は緩めない。`--allow-local-rehearsal` は流用しない |
| D2 | 第1段階の開始 | **B. 城東単独で開始。** 現場へ「城東のみ。八王子はまだ旧システムのまま」と明示。八王子 apply は H3-7 maintenance window |
| D3 | ログイン対象 | **各医院の現行スタッフほぼ全員**を移行 `staffs.id` に後付け。CSV の履歴・退職行は対象外。414 行の一括発行はしない。誰が現行かは名簿で渡す |
| D4 | email 方針 | **デモ専用。SMTP には使わない。** 形式 `stg-staff-{id}@example.test`。実メールは使わない |
| D5 | 権限グループ | 現行役割ごとに **明示 ID**。role 名からの推論はしない。ローカル roster は clinic-2 一般 id=4（`003_demo/permission_groups.csv`）。USER がグループを設定済み |
| D6 | 必須業務 | 検索・受付・カルテ・会計（preflight が通る範囲）。未来日予約・LIFF・機器・画像は必須にしない |
| D7 | 並行期間と破棄 | **開始:** 城東投入後。八王子は後追い。**終了:** 必須 4 業務を対象医院で連続 5 営業日 STG 再現、かつ現場が切替可と判断。上限 8 週。切替印は両院投入後。破棄日は第2段階の前に書く |
...
- [x] **H0-1** D1–D7 確定（2026-08-25）。D5 の権限 ID は名簿（ローカルは clinic-2 一般 id=4）
...
- [x] **H0-4** 城東 manifest SHA-256（`backend/migrations/seeds/_old_db_handoff/jouto/manifest.json`）: `7bbda50f06f7d0acac6711d1a73b78ca68b835ee9be5df6cd04f3e6a5094a405`。Class A: payments snapshot、completed+nonzero 欠 graph を pending 再分類、同一 medical_record_id の余剰 billing 192 行のリンク解除。billings SHA `c04d05a014d7ae58cc9103e347ffe42100e7c84bede9a72c4bee0a1f3ca780be`。八王子は H0-2 後に別途転記
...
- [x] **H1-1a** 城東 preflight（ローカル `db` / write-0）: `make stg-uat-csv-import-preflight`（`CLINIC_CODE=jouto` / ordinal 2 / clinic id 2 / `STG_UAT_CSV_IMPORT_ALLOW_REHEARSAL=YES_I_UNDERSTAND`）。途中 fail-closed（payment snapshot 不一致、completed+nonzero 欠 graph）を Class A で解消し **PASS**。支払行は捏造していない。apply 未実行
- [x] **H1-1b** 城東 apply → verify（ローカル rehearsal）: USER `make reset`（`--allow-local-rehearsal` / `cmd/csv-import`）。**2026-08-25** と **2026-08-26** とも apply PASS + verify PASS（`jouto-intake-20260822-01`）。2 回目は reset 後の再 import（先行 staff-attach は wipe）。STG UAT `make stg-uat-csv-import` apply は未実行（このローカル DB は band 占有済み）
...
#### Lane 2 — AnimalEkarte 実装（完了 `4703cf3e9`）

残るのは名簿と USER の投入（Lane 1 → Lane 3）。PHI をテストフィクスチャに入れない。ローカル go / npm は使わない。検証は Docker のスコープ限定。`make db` / STG 破壊操作は USER 手動。
...
**実装済み:** `backend/cmd/stg-uat-skeleton` + `make stg-uat-skeleton`。投入は USER。エージェントは実行しない。

- 既定はローカル `DB_HOST=db` / `DB_SSL_MODE=disable`。Compose の `up` は引き続き `DB_HOST:db` 固定で、`docker compose run -e` だけが上書きする
- Make は `-e STG_UAT_SKELETON_ALLOW_REMOTE` と `-e DB_HOST` / `DB_PORT` / `DB_SSL_MODE` を転送する。リモート STG は USER が `DB_HOST` / `DB_PORT` / `DB_SSL_MODE`（`require`|`verify-ca`|`verify-full`）と `STG_UAT_SKELETON_ALLOW_REMOTE=YES_I_UNDERSTAND` をセットしてから `make stg-uat-skeleton`
- `pscale connect` で `localhost` に見せかけて通す経路は禁止
...
**実装済み:** `backend/cmd/csv-import-stg-uat` + `make stg-uat-csv-import-preflight` / `make stg-uat-csv-import` / `make stg-uat-csv-import-verify`。投入は USER。エージェントは実行しない。
...
- Make は `DB_HOST` / `DB_PORT` / `DB_SSL_MODE` を転送し、apply の `--confirm-target-host` は `STG_UAT_CSV_IMPORT_CONFIRM_HOST`（既定 `db`）経由。`DB_HOST` への自動 alias はしない。リモート STG は USER が `DB_HOST` / `DB_PORT` / `DB_SSL_MODE` と一致する `STG_UAT_CSV_IMPORT_CONFIRM_HOST`、および `STG_UAT_CSV_IMPORT_ALLOW_REHEARSAL=YES_I_UNDERSTAND` をセットしてから `make stg-uat-csv-import-*`
- Compose の通常 `up` は `DB_HOST:db` のまま。`pscale connect` → `localhost` 偽装は禁止
...
**実装済み:** `backend/cmd/stg-uat-staff-attach` + `make stg-uat-staff-attach-preflight` / `make stg-uat-staff-attach`。**2026-08-26 ローカル apply PASS**（414）。STG リモート attach は USER / Lane 3。
...
- ローカルは `DB_HOST=db` / SSL `disable` 既定。リモート STG は USER が `DB_HOST` / `DB_PORT` / `DB_SSL_MODE`（`require`|`verify-ca`|`verify-full`）と `STG_UAT_STAFF_ATTACH_ALLOW_REMOTE=YES_I_UNDERSTAND` をセットしてから `make stg-uat-staff-attach*`。ローカル専用ガードを本番 F6 と共用して緩めない
...
[STG_PLANETSCALE_SEED_RUNBOOK.md](docs/ops/deploy/STG_PLANETSCALE_SEED_RUNBOOK.md) の破壊境界に従う。各 CSV は 512MiB 上限。城東の大きい表は約 370MB（exam_results / billing_items）。八王子は H0-2 で上限確認する。**城東先行。**

- [ ] **H3-1** 他エンジニアが STG を使っていないことを確認。並行運用の開始を医院へ伝える（現場へ「城東のみ」）
- [ ] **H3-2** 検証済み full backup。復元担当を作業票に書く。現行データの破棄は USER 承認
- [ ] **H3-3** TTL 付き `pscale role` で直結。必要なら `DROP SCHEMA public CASCADE` と `CREATE SCHEMA public` を **対で**実行。`pscale connect`→localhost 禁止
- [ ] **H3-4** `gh workflow run backend-deploy.yml --ref staging`。migrate 成功。この時点は `002_master` のみ
- [ ] **H3-5** `make stg-uat-skeleton`（clinic 1=八王子 / 2=城東、6 seed、権限グループ）。**21 表に行を作っていないこと。** ID を作業票へ転記（PHI なし）。Make は `STG_UAT_SKELETON_ALLOW_REMOTE` と `DB_*` を転送する。ローカル既定は `DB_HOST=db` / SSL `disable`。リモートは USER が `DB_HOST` / `DB_PORT` / `DB_SSL_MODE`（`require`|`verify-ca`|`verify-full`）と `STG_UAT_SKELETON_ALLOW_REMOTE=YES_I_UNDERSTAND` をセットしてから `make`
...
- [ ] **H3-9** `make stg-uat-staff-attach-preflight` → `make stg-uat-staff-attach`。`STG_UAT_STAFF_ATTACH_ROSTER` / `STG_UAT_STAFF_ATTACH_SECRETS`（repo 外 0600）必須。Make は `STG_UAT_STAFF_ATTACH_ALLOW_REMOTE` と `DB_*` を転送する。リモートは USER が `DB_HOST` / `DB_PORT` / `DB_SSL_MODE` と `STG_UAT_STAFF_ATTACH_ALLOW_REMOTE=YES_I_UNDERSTAND` をセットしてから `make`。初期パスワードは別経路
...
|----|------|----------------|--------------|
| [x] AE-OLD-DB-PAY-SNAP | `payments.total_amount` = 親 billing 合計 | 済み `482c29f` | 同じ SQL |
| [x] AE-OLD-DB-PAY-GRAPH | 支払なし completed+nonzero は pending。支払捏造しない | 済み `8187d9b7` | 同じ SQL |
| [ ] **AE-OLD-DB-MR-UNIQ** | 非 NULL `medical_record_id` は 1 カルテ 1 billing。余剰はリンク解除 | **残** | 同じ |
...
次: 城東ローカル attach 済み。H3 / STG は USER。八王子は H0-2 待ち。
    follow-up: AE-OLD-DB-MR-UNIQ（1 カルテ 1 billing）。ログインする人は set_active / is_active を画面で有効化
Lane 3: AE-STG-UAT-JOU → 第1段階開始（現場へ「城東のみ」）  ← USER。未着手。patched 城東束。stage 再実行しない
...
1. **AE-OLD-DB-MR-UNIQ**（次の stage の前に推奨。1 カルテ 1 billing）
2. **AE-STG-UAT-LANE3** USER（patched 城東束。`stg-uat-csv-import`。stage 再実行しない。H3 は未チェックのまま）
3. **HAC-CSV-1** 並行。投入は城東のあと、maintenance window
4. Lane 4 → 第2段階（本番 F6。STG 並行分は入力にしない）

任意: シフト、A4/失敗リハーサル、M-3 runbook 1 行（明示時のみ）、AE-SEED-RETIRE-DEMO。

claim 削除は USER（上記 claim 一覧）。
...
| [docs/ops/deploy/STG-DEMO-DATA-LIFECYCLE.md](docs/ops/deploy/STG-DEMO-DATA-LIFECYCLE.md) | STG は master-only。本節の実データ UAT は例外（canonical 側への 1 行追記は M-3、未適用） |
