...
- **U5 完了（2026-07-26・第2走）**: `model.AllItemCategories()` を12定数の正準完全集合として追加し、締め集計の raw category 集約と `GetPreview` category×支払方法按分を同集合で fail-closed 化した。unknown 値は alias・`other` fallback・黙殺を行わず error 伝播し、純関数 test が12値/validator self-consistency・unknown 拒否・既存の整数按分を固定する。既存 category fixture は `"診察"`→`ItemCategoryExamination`、`"検査"`→`ItemCategoryTest` へ現代化した。
  - 変更file: `backend/internal/model/accounting.go`、`backend/internal/billing/accounting_repository_reports_close.go`、`backend/internal/billing/cash_register_service.go`、`backend/internal/billing/{cash_register_service_test.go,accounting_service_test.go,accounting_report_service_test.go,item_category_close_contract_test.go}`。recovery contract により同クラスを検出した `cash_register_response_test.go` と `accounting_report_handler_test.go` も category field/map key だけ限定更新。
  - TDD: `docker compose exec backend go test ./internal/billing/ -run '^TestCloseCategoryContract$' -v -count=1` は RED（未実装4 symbol、exit 1）→ GREEN（独立reviewで追加した source/helper/validator 完全集合比較を含む4 subtest PASS、`ok ... 0.005s`）。`-list '^TestCloseCategoryContract$'` は test 名を1件出力。
