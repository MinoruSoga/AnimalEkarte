...
| `.claude/skills/be8-package-refactor/SKILL.md` | **BE 第8期計画の正本（2026-07-17 新設 → 同日 skill へ移設）**: backend/ 全体のパッケージ構成統一。全 15 internal パッケージ+cmd+worker+migrations を実測評価済み。着手は Go-live 後・BE8-0（lint 網羅性固定）が必須ゲート。`.agents/skills/` 側は生成ミラー |

> 旧 `BE_todo.md` / `FE-refactor.md` は本ファイルへ吸収済み（削除）。`BE-refactor.md` は第7期版を吸収・削除後、2026-07-17 に第8期計画として新設 → 同日 `.claude/skills/be8-package-refactor/SKILL.md` へ移設（リポジトリ直下からは削除）。旧 `docs/tasks/`・`docs/archive/` は 2026-07-16 に廃止（詳細は git 履歴）。
