...
認証の残検証と開始条件は [todo-fix-auth.md](todo-fix-auth.md) を正本とする。disposable 実DBでの D2 並行・D3 全178経路返却分離・D1 合成 SQL・D5 2プロセス即時失効は PASS。D1 本番付与・対象環境メール・Linear ライブ更新は未完了。DB/環境操作は個別の実行条件に従う。
