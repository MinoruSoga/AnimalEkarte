...
| SCEN-OPS-CLAIM-001 | `claim/SCENARIOS-ALIGN-REMEDIATE-WAVE1` 解放 | **ops-only**（下記）— USER が merge 後に `git branch -D`。エージェントは削除しない |
...
| ARCH-DONE | architecture docs Mode 3 同期（ERD `TABLE_COUNT=114` + identity 4、`check-docs-symbol-drift` 3a 正本、FE `RESOURCE_LABELS` 35=`AllResources`、narrative 12 ファイル） | **closed / done** — 再実装 TASK にしない。path-scoped 純 docs commit は未作成で内容は mixed preserve/merge 履歴（例: `1d8d2e59d`）に混入。索引フッタ 110 残は ARCH-R4 / TASK-017 |
| ARCH-R1 | ADR-003 References の歴史的 `internal/service/...` パス | **TASK-016**（Low / docs） |
| ARCH-R2 | architecture fix-all 第1回: empty-diff / worktree 未 merge で COMPLETE 宣言 | **ops-only**（R7 拡張）— 製品 TASK なし |
| ARCH-R3 | architecture が mixed preserve commit に混入。screens-drift land 時の foreign 定義 | **ops note** + **TASK-004 文言 amend**（現状 tree / committed 状態に合わせる） |
| ARCH-R4 | docs-symbol-drift が README/screens 等フッタを 3a 対象外にした経緯。110→114 手動同期の残と gate 再対象化の方針 | **TASK-017**（Low / 任意） |
...
- **ARCH-R2（R7 拡張）**: architecture docs fix-all 第1回で、マルチエージェントが empty-diff または worktree 未 merge のまま COMPLETE を宣言した事例あり。受け入れ判定は **`git diff -- <path>` / `git status` で実 diff を必須**とし、宣言文面だけで PASS にしない（harness/ops 側の規律。製品コード TASK にしない）。
- **ARCH-R3**: architecture 同期成果は `1d8d2e59d chore: preserve current main work` 等の **mixed preserve commit** に混入（path-scoped pure-docs commit ではない）。将来 screens-drift land（TASK-004）時は **その時点の working tree / `git diff` を再計測**して intentional / foreign を定義し直す。architecture を「未コミット foreign WIP」前提に固定しない（現行は local main に commit 済み。dirty は主に screens 系などの別レーン）。
- **SCEN-OPS-CLAIM-001**: claim ブランチ `claim/SCENARIOS-ALIGN-REMEDIATE-WAVE1` の解放は **USER only**（merge 後 `git branch -D claim/SCENARIOS-ALIGN-REMEDIATE-WAVE1`）。エージェントは claim を削除しない（Agents.md packet claim プロトコル）。
...
1. TASK-004 path-scoped stage → 再 TASK-005 → ユーザー承認後 commit で screens-drift land（**foreign 定義は ARCH-R3 / 現状 diff に合わせて更新済み**）  
2. ~~TASK-006 / 007 / 008~~（docs residual 完了）  
3. ~~ARCH-DONE~~（architecture Mode 3 同期完了・再開禁止）  
4. TASK-001-BE → TASK-001-FE（原子性、臨床 integrity）  
5. TASK-002 / TASK-003（PO 判断待ち）  
6. **ops**: SCEN-OPS-TREE-001（隔離）を維持 → merge 後 SCEN-OPS-CLAIM-001（USER）  
...
13. **TASK-016 / TASK-017** architecture 残債（Low・任意順; ARCH-DONE の再オープンではない）
...
- **根拠（2026-07-31 amend / ARCH-R3）**: architecture docs（ERD 114、narrative、symbol-drift 等）は local main に **既に commit 済み**（mixed 例: `1d8d2e59d chore: preserve current main work` ほか。path-scoped pure-docs commit ではない）。したがって architecture を「未コミット foreign WIP」として前提にしない。intentional 候補は引き続き `docs/spec/screens/*`（`40-identity-links.md` 含む）、Pack C hospitalization form/write、`route-inventory.test.tsx`、`master-settings-index-model(.test).ts` 等。foreign は **その時点で dirty な意図外パス**（例: billing/csvimport/examinations/scenarios/package/symbol-drift scripts/codex-security 等）。identity-links FE / ExamTypeFieldsEditor は ambiguous（既定 OUT）。**land 直前に再計測**し、architecture が clean なら foreign から外す。
...
- **状態**: Medium/ops。**stage plan 準備済み（未 stage）**。commit はユーザー承認境界。architecture 未コミット前提のブロッカー文言は撤回。
...
- **問題**: テーブル数の正本は ERD / specification で **114** だが、一部索引フッタが **110** のまま。`check-docs-symbol-drift.sh` 3a は ERD+specification のみ強制し、README/screens/overview 系フッタを意図的に対象外にしているため、フッタ drift がゲートをすり抜ける。
- **根拠**: residual **ARCH-R4**。`scripts/check-docs-symbol-drift.sh` L192 コメント「docs/README.md / screens/README.md / overview の索引フッタは別 unit で追随」。実測: `docs/README.md:40`・`docs/spec/screens/README.md:89`・`docs/ops/deploy/DEPLOYMENT_CHECKLIST.md:39` が 110。ERD `TABLE_COUNT=114` は ARCH-DONE。Mode 3 の「フッタ 114 手動同期」は **未完了がディスク証拠**。
- **修正方針**: 案A（推奨最小）: 上記 3 ファイルの 110→114 のみ同期（Resources 35 は維持）。案B: 3a の `check_number` 対象に該当フッタを戻し機械強制。案C: フッタを out-of-scope のまま accepted とし台帳/コメントで固定（同期は任意）。handler 3c の dead branch は本 TASK で触らない。
- **受け入れ条件**: 案A なら ① 3 ファイルが 114 ② ERD/specification と矛盾なし ③ 既存 3a PASS。案B なら フッタ不一致で FAIL。案C なら out-of-scope が 1 箇所に文書固定。製品コード・migration 変更なし。
- **状態**: open / Low・任意。ARCH-DONE（ERD/gate/FE labels/narrative）の再実装ではない。
