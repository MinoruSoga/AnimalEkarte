...
加えて **SCENARIOS-ALIGN-REMEDIATE-WAVE1**（scenarios doc↔code 監査・修正 wave + Mode 3）後の residual を同一ファイルに併記する（索引 ID は `SCEN-*`。**新規 TASK は 009 以降**。既存 TASK へ merge する residual は例外）。screens-spec 節（R1–R8 / TASK-001–008）は保持する。

さらに **docs/spec top-level audit→fix→Mode3 連鎖**（SPEC-TOP residual）の未カバー follow-up を 2026-07-31 に併記する（索引 ID は `SPEC-TOP-*`。**新規 TASK は 018 以降**。既存 TASK へ merge する residual は例外 — 例: SPEC-TOP-FOOTER-115 → TASK-017）。G1–G12 機能クローズと ERD/spec **115** 同期は **再実装 TASK にしない**（coverage matrix / ARCH-DONE のみ）。

> **ID namespace 注記（2026-07-31 実測）**: 本ファイルの `TASK-001`–`TASK-021` は **このファイル内ローカル**の連番であり、正本台帳 `3-session-agent.html#ledger` の体系外である。正本は接頭辞横断の単一連番（実測: BUG-421/430/431/433/442/446/449、TASK-251/444/603–610）。現時点で数値衝突は **なし**だが、`/implement` および `task-create` skill は ID を `3-session-agent.html#ledger` からのみ解決するため **本ファイルの ID は標準ツールから参照できない**。また task-create は採番時に本ファイルを見ないため将来衝突しうる。正本への統合か namespace 明示かは **USER 判断（未決）**。
...
| R4 | screens-drift 意図変更のコミット隔離 | **TASK-004**（open・**手順のみ**。intentional / foreign は land 直前の `git status` 実測で定義する — 本台帳に dirty 一覧を書かない） |
| R5 | コミット前の closed pack 回帰ゲート | **TASK-005**（手順。symbol-drift は 2026-07-31 実測 green。vitest 分は**証拠未記録** — land 前に実行） |
...
| SCEN-OPS-CLAIM-001 | `claim/*` 解放（旧 `claim/SCENARIOS-ALIGN-REMEDIATE-WAVE1` 含む） | **ops-only**（下記）— USER only。**2026-07-31 実測: live `claim/*` は 7 本存在**（`claim/SEC-CS2-*`）。6 本は HEAD の ancestor = 削除可。`claim/SEC-CS2-F01-R1-SCOPE` のみ **未統合**（main に無いファイルを保持）→ **要 USER 判断** |
...
- **POST-PULL（migrations 005/006）≡ SPEC-TOP-MIGRATE-006**: `005_line_webhook_bot_user_id.sql`（`line_reservation_settings` への `ALTER TABLE` + `uq_line_reservation_settings_line_bot_user_id` UNIQUE INDEX。**新規テーブルなし**）および `006_medical_record_image_upload_quota.sql`（`medical_record_image_upload_quota` 1 テーブル）を pull した開発者は、更新アプリ利用前に **USER が手動で `make migrate`**。エージェントは migration を auto-apply しない（Agents.md / CLAUDE.md）。
- **SCEN-OPS-CLAIM-001 ≡ SPEC-TOP-CLAIM-RELEASE**: claim 解放は **USER only**。エージェントは claim を削除しない（Agents.md packet claim プロトコル）。**2026-07-31 実測** `git branch --list 'claim/*'` = **7 本**:
  - **統合済み 6 本**（HEAD の ancestor。merge 後 `git branch -D` 可）: `claim/SEC-CS2-F01-STG-ADMIN` / `F02-LIFF-LINK` / `F03-TF-STATE` / `F04-F05-F07-POLICY` / `F06-AUTH-FAIL-CLOSED` / `F08-LSTEP-REDACT`
  - **未統合 1 本**: `claim/SEC-CS2-F01-R1-SCOPE`（HEAD の ancestor でない）。6 commit / 42 files (+1154 −725) を保持し、**main に存在しないファイル**を含む — `scripts/check-agent-security-policy.sh`、`scripts/check-agent-security-policy.test.mjs`、`scripts/check-workflow-remote-exec-policy.sh`。他に `infra/cloudflare/hyperdrive.tf` / `variables.tf`、`show-demo-accounts.ts` の差分。`check-agent-security-policy.sh` の全 ref 履歴は `e149c02ba` のみで **main には一度も現れず revert 記録もない**。統合漏れか意図的除外かは git だけでは判定不能 → **削除前に USER が判定必須**（安易な `-D` で security gate 作業を失う）。
  - 旧 `claim/SCENARIOS-ALIGN-REMEDIATE-WAVE1` は live に不在（解放済み）。
...
1. TASK-004: intentional dirty が **screens-drift land 用**に再出現したとき path-scoped stage → 再 TASK-005 → ユーザー承認後 commit（**foreign 定義は ARCH-R3 / その時点の `git status` 実測**）  
...
- **状態**: **done**（2026-07-31 orchestration `TODO-MD-ALL-OPEN-ORCHESTRATED-20260731`）。案A: `POST /v1/hospitalizations` に optional `treatment_plans` を載せ、同一 `WithTx` 内で plan repo 直書き。discount:create は handler で guard。検証: `docker compose exec backend go test ./internal/medicalrecord/ -run Hospitalization` PASS。残差: 実 DB 付き orphan 0 件 integration は任意 follow-up（unit は error 伝播+同一 clinic を固定）。claim: `claim/TASK-001-BE`（USER が integrate 後に解放）。
...
- **状態**: **done**（2026-07-31）。create は `buildCreateHospitalizationRequest(..., treatmentPlans)` で nested 同梱 1 リクエスト。`createTreatmentPlanForHospitalization` loop 削除。hook/model テスト更新。claim: `claim/TASK-001-FE`。
...
- **状態**: **PO待ち**（2026-07-31）。意思決定パケット: `reports/2026-07-31-todo-po-decisions.md`（推奨 B/C。コード変更なし）。
...
- **状態**: **PO待ち**（2026-07-31）。パケット: `reports/2026-07-31-todo-po-decisions.md`（推奨 B 表示専用）。コード変更なし。
...
- **根拠（2026-07-31 amend / ARCH-R3）**: architecture docs は local main に commit 済み。ERD/specification の **115** 同期は path-scoped `07e2fb945`（`erd.md` + `specification.md` のみ — isolation の好例）。Mode 3 本体は mixed 例 `1d8d2e59d` にも混入し得る。`40-identity-links.md` は PRESENT/tracked（例: `6f83aae9f`）。identity-links FE / ExamTypeFieldsEditor は ambiguous（既定 OUT）。
- **⚠ 本台帳に dirty ファイル一覧を書かない**: 台帳が自身の working tree 状態を記述すると、その台帳を commit した瞬間に記述が偽になる（2026-07-31 に実害: 「許容 dirty は `todo.md` のみ」が `f4f2be716` commit 後に偽化）。intentional / foreign の集合は **land 直前の `git status --porcelain` / `git diff` 実測のみ**を正本とする。
- **修正方針**: land 直前に `git status --porcelain` を実測し intentional 集合を確定する。dirty が無ければ screens stage は no-op。dirty があるときのみ path-scoped `git add`（`git add -A` 禁止）。foreign は触らない・捨てない。ledger（`todo.md`）単独 commit は screens-drift land と混ぜない。
...
- **状態**: **ops 手順実測済み / land 未実行**（2026-07-31）。`reports/2026-07-31-task-004-005-land-proc.md`。commit/push は USER 承認境界。foreign WIP 保持。
...
- **状態**: **ops ゲート部分 PASS / land 前 vitest は reports 参照**（2026-07-31）。symbol-drift: `OK ... 535 件` exit 0。hospitalization 関連 unit は BE/FE で PASS（詳細 `reports/2026-07-31-task-004-005-land-proc.md`）。commit は USER。
...
- **状態**: **設計 done / 適用は USER**（2026-07-31）。設計: `reports/2026-07-31-task-009-seed-design.md`（header-only 29 CSV・最小 fixture・手動 apply 手順）。エージェントは migrate/seed 未適用。claim: `claim/TASK-009`。
...
- **状態**: **BLOCKED（env）**（2026-07-31）。`reports/2026-07-31-task-010-020-runtime-blocked.md`。【要実測】78 件。browser-test 未実行。
...
- **状態**: **done**（2026-07-31）。S11 手順2 操作文を受付済-only open に整合（診療中は結果）。claim: `claim/TASK-011`。
...
- **状態**: **PO待ち**（2026-07-31）。`reports/2026-07-31-todo-po-decisions.md`（推奨 B）。コード変更なし。
...
- **状態**: **PO待ち**（2026-07-31）。`reports/2026-07-31-todo-po-decisions.md`（推奨 B append-only）。コード変更なし。
...
- **状態**: **PO待ち**（2026-07-31）。`reports/2026-07-31-todo-po-decisions.md`（推奨 A 運用方針文書化）。コード変更なし。
...
- **状態**: **done**（2026-07-31）。`reports/2026-07-31-task-015-audit-medium.md`（orphan MEDIUM 残差なし・78 要実測は TASK-010）。claim: `claim/TASK-015`。
...
- **状態**: **done**（2026-07-31）。References を `internal/billing/*` へ更新（BE9 以前は historical 注記）。claim: `claim/TASK-016`。
...
- **状態**: **done**（2026-07-31・案A）。3 フッタ 110→115。`bash scripts/check-docs-symbol-drift.sh` OK exit 0。claim: `claim/TASK-017`。
...
- **状態**: **監査記録 done / full page-by-page は follow-up**（2026-07-31）。`reports/2026-07-31-task-018-screens-audit.md`（41 vs 84 vs 66）。claim: `claim/TASK-018`。
...
- **状態**: **監査記録 partial**（2026-07-31）。`reports/2026-07-31-task-019-line-audit.md`。deep pass は follow-up。claim: `claim/TASK-019`。
...
- **状態**: **BLOCKED（env）**（2026-07-31）。`reports/2026-07-31-task-010-020-runtime-blocked.md`。Playwright runtime 未実行。
...
- **状態**: **PO待ち**（2026-07-31）。`reports/2026-07-31-todo-po-decisions.md`（推奨 B→段階 A）。コード変更なし。
