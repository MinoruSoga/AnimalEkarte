...
| H0-2 / HAC-CSV-1 | 不足票: [SLACK-HAC-IMPORT](docs/work/remaining-campaign-20260920/SLACK-HAC-IMPORT.md)（送付報告≠正式bundle） | 現在の正式入力受領と検証。未接続Linearの昔の本文で受領済みにしない |
...
| H3-9 | 不足票: [SLACK-ACCESS](docs/work/remaining-campaign-20260920/SLACK-ACCESS.md) | repo外入力と対象host/database、preflight、必要分のattach承認 |
...
| P1 / SEC-SECRETS-5 | 不足表: [LINMIG-239](docs/work/linmig-campaign-20260919/LINMIG-239.md) | 各系統の担当・変更対象・復旧・明示承認 |
| P2 / PROD-SETUP | 差分表: [LINMIG-232](docs/work/linmig-campaign-20260919/LINMIG-232.md) | setup §1–6、保護された実行経路、環境・配備承認 |
| P3 / PROD-DATA-MIGRATION | 当日票: [LINMIG-234](docs/work/linmig-campaign-20260919/LINMIG-234.md) | rehearsal/復旧成功、本番window、入力停止・最終import承認 |
| P5 / STAFF-PROVISION | remote設計: [LINMIG-230](docs/work/linmig-campaign-20260919/LINMIG-230.md) | roster等の入力、対象/承認/rollback、remote方式の実装・検証 |
| P6 / DELIVERY | 供給者別未記入: [P6](docs/work/remaining-campaign-20260920/P6.md) | 現行の事実・P1/P2等のreceiptと契約責任者の承認 |
| P7 / TRAINING | U13実施票: [P7](docs/work/remaining-campaign-20260920/P7.md) | 医院担当・対象版・開催条件、案内送信の承認 |
...
3. 同一build・DB適用receiptを [競合検証](todo-verification.md#code-followup-20260921) へ渡し、2セッション/実DB並行と残ケースを確認する。適用未確認、制約不適合、不明なcommit結果がある場合は停止する。
...
| ID / 段階 | 現在の判定 | 次の一手 |
|---|---|---|
| [H0-2 / HAC-CSV-1](#h0-2--hac-csv-1) | UNKNOWN（producer の追加状況未照合）。照合枠は [SLACK-HAC-IMPORT](docs/work/remaining-campaign-20260920/SLACK-HAC-IMPORT.md) | 前回の HAC-INPUT-2 は完全 KNJO 未受領。現在の受領・producer 結果を確認し、未解消なら完全入力を待つ |
...
| ID | 状態 | 残作業・完了条件 |
|---|---|---|
| [P1 / SEC-SECRETS-5 / #89 / #97](#p1--sec-secrets-5) | 準備票済 / receipt 未照合 | [LINMIG-239](docs/work/linmig-campaign-20260919/LINMIG-239.md)。4系統の発行・投入・deploy・health・旧値 revoke / 拒否の非機密 receipt。不足時は明示承認後に実施 |
| [P2 / #253 / U12 PROD-SETUP](#p2--prod-setup) | 準備票済 / 現行環境 UNKNOWN | [LINMIG-232](docs/work/linmig-campaign-20260919/LINMIG-232.md)。Cloudflare 本番、reviewer / workflow、rollback、backup rehearsal、URL/CI receipt |
| [P3 / #250 PROD-DATA-MIGRATION](#p3--prod-data-migration) | 準備票済 / 前提・実施証拠待ち | [LINMIG-234](docs/work/linmig-campaign-20260919/LINMIG-234.md)。rehearsal、入力停止、backup/rollback、最終 import、件数・医院・金額突合 |
| [P5 / #255 STAFF-PROVISION](#p5--staff-provision) | 準備票済 / 入力・実施証拠待ち | [LINMIG-230](docs/work/linmig-campaign-20260919/LINMIG-230.md)。roster・email 方針・clinic・role・操作者・環境承認を非機密 receipt で確認 |
...
ローカル照合枠は [LINMIG-239](docs/work/linmig-campaign-20260919/LINMIG-239.md)。4系統は **PlanetScale DB / Cloudflare API・Worker secrets / LINE channel secret・access token / JWT・INTEGRATION_ENCRYPTION_KEY**。値は取得・転記せず、[既存の不足表とreceipt欄](docs/ops/deploy/runbooks/BUG_MD_EXTERNAL_OPS_PENDING_APPROVAL.md) を使う。発行済み・投入済み・旧値拒否の証拠は別セルにし、一部成功で全系統を閉じない。
...
ローカル切替準備は [LINMIG-234](docs/work/linmig-campaign-20260919/LINMIG-234.md)。件数突合の集計仕様（値なし）は [LINMIG-219](docs/work/linmig-campaign-20260919/LINMIG-219.md)。環境別 migrate の適用要否は [LINMIG-209](docs/work/linmig-campaign-20260919/LINMIG-209.md)。
...
remote 方式の設計票は [LINMIG-230](docs/work/linmig-campaign-20260919/LINMIG-230.md) に作成済み。方式の実装・apply実行・receiptの代替ではない。[スタッフ発行手順](docs/ops/deploy/STAFF_ACCOUNT_PROVISIONING.md) は remote apply の実行方式を未定義としている。`STAFF_PROVISION_ALLOW_REMOTE` の許可値だけで共有STG/PROD applyへ進まない。secretのread-only供給、対象を固定した実行入口、承認、rollback、非機密receiptを設計し、実装・検証が揃ってから実行する。H3-9 の `stg-uat-staff-attach` は既存staffへのattach用で、P5の方式が完成した根拠にはしない。
...
供給者別の未記入表は [P6](docs/work/remaining-campaign-20260920/P6.md)。未記入欄は [納品パッケージ U1–U12](docs/delivery/DELIVERY_PACKAGE.md) に集約する。U1–U4=クラウド/DB/Vercel/GitHubの契約・請求・移管/権限、U5–U6=LINE/LSTEPの本番設定参照（秘密は別管理）、U7–U8=支援窓口/通知先、U9–U11=DB復旧・R2保全・監査保持の方針と実測、U12=本番構築/疎通の証拠。公開repoから契約名義や保持年数を推定せず、確認依頼の下書きにする。
...
実施票の空欄は [P7](docs/work/remaining-campaign-20260920/P7.md)。日程・参加者は UNKNOWN のまま。
