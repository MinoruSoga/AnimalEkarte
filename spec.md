...
### データ永続化
バックエンド（Go/Gin/GORM）とPostgreSQLで構築されたRESTful APIを使用しています。
フロントエンドはTanStack Query（React Query）を使用してAPIと通信し、データの永続化を実現しています。
...
1. **在庫管理 (Inventory)**:
    ...
    - 医薬品・フードの在庫数管理とマスタ連動
2. **シフト管理**: スタッフの勤務シフト作成機能
3. **認証機能**: ログイン/ログアウト、ロールベースアクセス制御 (RBAC)
4. **印刷機能**: 帳票（明細書、処方箋、証明書）の正式な印刷対応
5. **処方機能**: カルテ画面での薬剤選択・用量指定の実装（UIは存在するが機能未実装）
6. **画像管理**: 検査画像のアップロード・表示・管理機能の実装
