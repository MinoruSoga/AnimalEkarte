...
- **L⑥ 残置解消（完遂・2026-07-22・core `849c27524` / final composition `962ce70e3` / cleanup `8238395e2` / fixture `bd3f53987`）**:
...
1. **Historical allocation rule (closed / DO NOT APPLY)**: 2026-07-23当時は、大規模domainのために先行移行済みの小規模dependencyを除き、clean local `main`の同一immutable baseから作るready frontierで、残存production行数、file数、facade/call-site削減量が最大の候補をSession Aへ、次に大きく相互非競合な候補をSession Bへ割り当て、最大2 domain laneを同時実装していた。Session Aはその`main`の唯一writer、Session Bは同baseから作る専用branch/worktreeのwriterとしていた。pairはfile/hunk、symbol/caller、business fact、table/write owner、consumer/provider interface、DI、transaction/lock、route/RBAC/OpenAPI、lint/gate、migration、protected WIPの全軸で競合0を証明していた。安全な2件目がなければBは別domainの非競合characterization/test-only blocker-preparation batchへ縮退させ、並行数のためにready条件を緩めなかった。behavior/security blocker removalはcharacterization、contract/ADR、clinic-isolation runtime testを先に固定した独立batchとして全競合軸0を証明できる場合だけ割り当て、それ以外はBをread-onlyへ縮退させていた。これは閉じた履歴であり、現行の作業割当には使用しない。
...
- 2026-07-23夜の再計測でremote reconciliationは完了済み — infra統合merge `854fc6447` 時点でlocal `main`=`origin/main`（0/0）、その後のlocal commit `e14421cef` でahead 1 / behind 0。divergenceは解消済みのため、B0 packetのremote方針は「同期済み・未push 1件（push=USER専権）」の記録で足りていた。当時は各waveのbase packetで両tipとahead/behindを再計測し、remoteを先にreconcileするか、local-only authorityとして進めremote integrationをHOLDするかを明記していた。自動pull/rebaseや履歴書換えで解消しない方針だった。
...
**Historical role record (closed / DO NOT APPLY)**: 当時はSession Aをcanonical local `main`の唯一writer兼integration owner、Session Bを同じimmutable baseから作る専用branch/worktreeのdomain laneとしていた。A用・integration用の追加worktreeは作らず、並行化するのは相互非競合なdomain-local実装期間だけとして、central/shared surface、共有DB gate、Session B candidateのmain反映はSession Aが直列化していた。独立検証は固定commit/treeを読む一時的なread-only reviewer agentsへ分離し、長寿命のSession C/Dは置かなかった。taskのscope・完了条件は各BE9節、当時の進捗・残債務は「[現在地と着手前ゲート](#be9-current-state)」、package/file分類はclassification manifestとboundary mapを正としていた。

- **Historical Session A（closed: `main` writer / domain lane A / integration owner）**: 当時はcleanかつquiescentなlocal `main`でlargest-ready候補1のdomain-local production/test実装を行い、共有file、candidate統合順、文書reconciliation、global DB test queue、最終結果を所有していた。別sessionや別taskが同じ`main` worktreeを同時に編集してはならない規定だった。
- **Historical Session B（closed: isolated domain lane B / historical assignment = inventory（移行完了済み・2026-07-24））**: Wave 2aで確定したrank #2のinventoryを担当していた。Session Aと同じcleanかつquiescentな`B0`から専用branch/worktreeを1つだけ作り、INV-SEC P1を先行させてからinventoryをdomain-localに縦移動する規定だった。P1ではinventory-owned fileに加えて、signature追随に必要なmedicalrecord view/call site/mockだけを開始時allowlistへ明記していた。共有master-FK lint行、composition、route/OpenAPI等のcentral/shared surfaceは編集せず、path・symbol・受け入れ条件をhandoffのchange requestとしてAへ渡していた。Aはexact B candidateを反映した直後にlint理由文をA-authored central commitで同期し、combined exact treeのgateとreviewがgreenになるまでlanding unitを完了扱いにしなかった。Aのauth landing後は旧candidateをそのままmergeせず、new main tipから再census・再構成・再freezeしていた。INV-SEC P1を開始できない場合の縮退先はBUG-421〜425（代替=BUG-426〜428）で、csvimport cutoverとは同時実行しない規定だった。
- **Independent reviewers**: A/B各candidateとcombined exact treeを編集せずにreviewする。最低でもgeneral、Go、clinic-isolation、securityを使い、DB/transaction変更にはdatabase、臨床データ変更にはhealthcare reviewを追加する。findingはauthor laneへ返し、treeが変われば旧verdictを失効させる。必要なreviewer agentを利用できずcombined treeの独立reviewを成立させられない場合、A/Bの相互reviewだけで完了扱いにせず、当該waveをHOLDするかA=executor/B=verifierの直列モデルへ縮退する。

以下の全waveは完了済みの履歴であり、現行の実行計画ではない（DO NOT APPLY）。

| wave | Session A（clean `main`） | Session B（専用worktree） | 直列barrier |
|---|---|---|---|
| 2a: six-domain recensus（完了） | 6 domain全体を統合し、authをready #1として確定 | inventoryをready #2として独立countercheckし、A/B全競合軸0を確認 | 2026-07-23の`B0=44bd0fabb` / tree=`5295021`でpair matrixをreconcileし、A=auth / B=inventoryを確定。これは実装開始baseではなくrecensus証跡 |
| 2b: auth + inventory（completed） | AUTH-PREP後にauthをdomain-localに縦移動。global authorizerとshared surfaceは凍結し、central cutoverへ分離 | INV-SEC P1後にinventoryをdomain-localに縦移動。central追随はchange requestとしてhandoff | 新しいclean/quiescent `B0`で開始し、A landing後にB candidateをnew main tipから再構成・再freezeする。Aだけがexact B candidateとB用central gate同期をmainへ反映する |
| 3: BE9-2F | lane A所有domainとshared consumerのfacade/call-siteをmainで収束する | lane B所有domainの非競合なconsumer 0候補を作る | shared caller/mock/aggregator、route/RBAC/OpenAPI、write ownerを隠し得る削除はAがmainで直列実行する |
| 4–5: BE9-3 / BE9-4 | lane A resourceの監査・修正、cross-domain integration、文書完了化を行う | 非競合resourceの監査・修正を専用worktreeで行う | combined exact-tree gateとfixed-tree reviewersのPASS後だけBE9を完了化する |

`staff`、`auth`、`clinic`、`pet`、`owner`、`inventory`のproduction file/LOC、manifest/live、changed blocker path、ready条件、全競合軸、rankingはWave 2aで再計測・reconcile済みであり、結果は「[現在地と着手前ゲート](#be9-current-state)」を正とする。Wave 2aの`B0`はrecensus証跡であってWave 2bの実装baseには再利用しなかった。当時は各landing後に残frontierをnew main tipで再計測し、後続pairを再選定していた。

当時、第2 full-domainがreadyでない場合、Session Bは別domainの非競合characterization/test-only blocker-preparation batchを担当していた。behavior/security blocker removalは独立contract/testと全競合軸0を先に証明できる場合だけ別landingとして扱っていた。それも成立しないwaveだけは一時的にBをread-only review/readiness laneへ縮退していた。これは安全条件による例外であり、当時の通常契約はA=`main`とB=worktreeの同時writerであった。第三のproduction writer laneは設けなかった。

**開始・物理分離gate（Historical / DO NOT APPLY）**:

- 当時は各wave開始時にlocal `main`のclean commit/tree `B0`を固定していた。tracked unstaged、staged、untracked、active Git state、別writerのいずれかが存在する間は開始しない規定だった。Session Aは`main`を直接使用し、Session B用`WT-B`だけを`B0`から作る規定で、`WT-A`や`WT-I`は作らなかった。
- `B0` packetにはlocal `main`、`origin/main`、merge-base、ahead/behindを含める。behindがある場合は、remote reconciliationを先行するか、local-only authorityとして進めpush/PR/remote integrationをHOLDするかを明記する。Session Aは他ownerのWIPやremote divergenceを代行してstash、clean、restore、pull、rebase、history rewrite、bulk commitで解消しない。
- frontend、infra、別backend taskを含むSession A以外のwriterは、AがBE9を`main`で実行している間、それぞれ別branch/worktreeを使う。pathname overlapが0でも同じ`main` worktreeを共有しない。
- dirty `main`を検出した場合は、Session Aが`HEAD`、tree、status、central file hash、staged/unstaged patch、untracked path/hashを固定し、各WIP ownerが自身のbranch/worktreeへ正規にlandingまたは隔離するまでHOLDする。
- Session B worktreeはcandidate landingまたはabandon後にfreezeし、証跡取得後に退役させる。同じSession Bが複数のactive writer worktreeを同時に持たない。

**並行化・統合規則（Historical / DO NOT APPLY）**:

1. L③a〜L⑤・BE9-2E-0の混在差分はcentral single-writer barrierとして処理済み。当時の後続waveでもA/Bはlive dirty treeではなく同じclean `B0`から開始することとしていた。
2. Session Aだけがlocal `main`、共有file、docs、およびfinal integrationのwriterである。Session Bは専用branch/worktreeの開始時allowlistだけを編集し、Aや既存WIPの変更を修正・restore・revertしない。
3. BE9-2Eの`largest-ready`は、ready frontier内の最大候補をA、次に大きい相互非競合候補をBへ割り当てると解釈する。固定domain順を正本化せず、各landing後に再計測する。file/hunk・symbol/caller・business fact・table/write owner・consumer/provider interface・DI edge・transaction/lock・route/RBAC/OpenAPI・lint/gate・migration・protected WIPのいずれかが不明または非0なら同時pairにしない。別fileでも同じtable、parent row、advisory/row lock、audit transactionを触る候補は競合として直列化する。
4. domain-local batchはtarget package、対応する旧domain file/test、期限付きfacadeだけを変更し、現行central compositionのままcompile可能な状態でhandoffする。structural move、behavior/security hardening、central integrationを別landing単位にする。
5. `backend/cmd/api/main.go`、`backend/docs/api.yaml`、`backend/internal/{handler/handler.go,handler/master_routes.go,service/service.go,repository/repositories.go}`、旧route snapshot、OpenAPI drift gate、lint inventory、classification manifest、ADR-006、boundary map、`BE-refactor.md`は共有integration面であり、Session A以外のwriterを置かない。`middleware`、`httpapi`、`sharedkernel`、`repository/repohelpers`、共有mock carrier、migration runner/checksum等、複数domain consumerを持つcross-cutting面は列挙外でもA専有とする。
6. authのpermission middlewareとdiscount permission helperは全route/domainへfan-inするglobal barrierである。auth coreを先にdomain-local移行しても、middleware/callerの一括cutoverは関連するroute batchが着地した後にSession Aが単独実行する。
7. 共有DBを使うDB-backed testはSession Aが単一global test queueを所有し、A/Bとreviewer agentsの実行者は同じglobal leaseをatomicかつfail-closedに取得する。lease evidenceにはholder、candidate commit/tree、Compose project、container/source hash、DB/port/volume identity、acquire/release時刻、command/resultを記録する。既存holder、identity不一致、stale holderの生存確認不能、異常終了後のsafe release未確認のいずれかでは実行しない。`-p 1`は単一process内の直列化にすぎず、session間のlease代替にならない。別Git worktreeだけではDocker Compose project、DB、port、bind mount/sourceを隔離したことにならない。Docker経由の全checkは、containerがcandidate treeまたは対象file hashと一致するsourceをmountしていることを証明する。証明できない結果はcandidateの合格根拠にしない。DB-backed testを並行実行できるのは、sourceに加えてCompose project、DB、port、volumeもsession単位で分離できた場合だけとし、それ以外はglobal leaseで直列化する。container/DBを使わないstatic/docs checkは並行実行できる。
8. 各candidateはauthor以外のfixed-tree reviewer agentsが確認し、findingはauthor laneが修正する。A/Bは相互のpath/blob/symbol/business-fact差分をcountercheckするが自己承認しない。candidate treeが変わった場合は旧reviewを失効させ、combined exact treeでも独立reviewを再実行する。
9. Session BからAへのhandoffは`B0`、candidate commit/tree hash、path/hunk allowlist、structural/behavior区分、central change request、scoped検証結果、未解消事項を必須情報とする。Aのrank #1 landing後、Bはnew main tipから専用worktreeを作り直し、rank #2をre-census・reconstruct・再freezeする。Aはそのexact B candidateをmainへ反映し、Bのcentral change requestをA-authored別commitとして適用する。candidateと必須central同期は連続する1 landing unitとし、combined exact treeのgate/reviewが完了するまで中間commitだけを完了・統合済みと報告しない。AがBのdomain hunkを変更した場合、そのhunkはA-authoredとなりBとfixed-tree reviewersの再reviewを必須とする。blind merge/rebaseはせず、path/blob/mode/deletion、symbol、business fact、write owner、gateに未説明差分があればB candidateをHOLDしてpairを再選定する。
10. L⑤のfresh DB実migration適用は承認を要する現行release gateである。Session A/B体制とは無関係に、明示承認なしに実行しない。
...
**Historical snapshot — 当時のdomain = trimming BE9-2E移行・qualification COMPLETE / BE9-2F central cutover待ち**: reconstruction `297a23fc7`はmainへ統合済みで、exact-source DB-backed test、race、coverage 91.6%、scoped build/vet/target lint、独立reviewまでPASSしている。13件のcompatibility surfaceとcentral route/composition/tygo consumerはBE9-2Fで収束する。残6 domainのWave 2a recensusは完了し（2026-07-23・B0=`44bd0fabb`）、production pair = **A=auth / B=inventory** を確定した（詳細は「現在の実行状態」節）。現在は全domain移行・BE9-2F完了済みであり、このA/B pairを現行割当として再利用しない。
