...
- **B① マスタ群**: payment_method_master + insurance + campaign(+discount) + subpackage 3個（paymentmethod/insurance/campaign）roll-up。master-CRUD先行（R①同型）。
- **B② estimate + billing_confirmation**: 見積・請求確定（sharedkernel.LockDraftMedicalRecord consumer=service側delegate解消先・X-11隣接）。
- **B③ billing本体 + billing_item + refund**: 金銭中核（refund tx原子性=#211 fail-closed・refund/ subpackage roll-up。treatment facade（repository.TreatmentRepository=④b残置billing_item read用）の解消先）。
- **B④ accounting core**: accounting_service_core/builders/correction + accounting_repository(+unpaid/ltv)。
- **B⑤ accounting reports + cash_register**: レポート群（reports_daily/monthly/close/shared）+ cash_register(+close)。estimate_audit_diff.go（⑦分離コピー）の帰属先。
- **B⑥ 残置解消**: validators_accounting・Services field棚卸し・transitional alias/carrier解消。
