...
### BE8-4: repository 残り約 107 ファイルの段階分割 — **18バッチ実施済み（2026-07-18・commit ebe845de9〜c4f70f134）、残多数**
- **実施済みバッチ**: ①`reservationtypegroup`②`reservationtypeavailableslot`+`reservationtypeunavailabletime`③`manualarticle`（以上 batch1-3）④`inquiry_template`⑤`checkup_type`⑥`daily_record`⑦`prescription`⑧`refund`（batch4-8）⑨`campaign`⑩`clinic_settings`⑪`clinic_holiday`⑫`shift_template`⑬`shift_entry`⑭`ltv`（**スキップ** — 下記）⑮`company`⑯`shared_file`⑰`checkup_sync`⑱`inventory`⑲`staff_clinic_assignment`（batch9-19・Workflowツールで凍結ゲート再確認→investigation subagent 3体並行調査→sequential実装→独立監査の4フェーズ構成で実施。commit: bf381c56d/5f04895f9/d7261eb4a/e3158ca1d/e5c6d0f84/9e3278ec8/f6398736/47364268/03266f8d4/c4f70f134）。
- **手順テンプレ確定**: ① 新規サブパッケージへ実装+テストを新規作成（`Repository`/`repository`/`New` の非stutter命名）② repohelpers.X 直接呼び出し（cage/vaccine等の主流規約、paymentmethod のローカル scope.go alias 規約とは非統一のまま — 別途 BE8-1 追補が必要）③ **旧flatファイルは型名維持のfacade化**（`type XxxRepository = <domain>.Repository`）で service/handler呼び出し側を無変更に保つ ④ 3lint該当エントリの手動更新（該当は今のところ全てdbortx_inventory_lint_test.goのみ・preload/audit_txはbatch9-19対象10ドメインで該当ゼロ）⑤ `go test -p 1 ./internal/repository/... `（**`-p 1` 必須**）。
- **batch9-19の追加知見**: ①`ltv`はスキップ — `ltv_repository.go` が package-private な `kana_normalize.go`（kanaSourceChars/escapeLike等、owner/pet/medical_recordと共有）に依存し、切り出すとrepository↔ltvの真のimport cycleになる（consumer側ローカルinterfaceでは解決不可な非behavioral依存）。unblock策 = `kana_normalize.go` を `repohelpers` へ先出しhoist（3ファイル外部影響・medical_recordクラスタに触れるため別バッチで判断）。②`permission_group`はinvestigation段階で不採用（実装1+テスト4=5ファイルで閾値超・`clinic_permission_group_tx_atomicity_test.go`がclinicと合成テストしているが本番importサイクルではない）。③`shift_template`/`shift_entry`分割時、共有`junction_helpers.go`のgeneric helperを`repohelpers.ReplaceChildRowsByParentID`へ直接呼び出しへ切替（shift_entry側でreplaceChildRowsByParentID自体が無参照になったため削除）。④`docker compose exec`は`--no-deps`/`--entrypoint`非対応、`docker compose run --rm --no-deps --entrypoint golangci-lint backend run ...`が正しい呼び出し（execではなくrun）。
- **残**: repository配下フラット`*_repository.go`実測93本（うちbatch5-8/paymentmethod分のfacade5本は実装済み扱い）。5〜10ファイル/バッチ換算で20バッチ超が未着手（`accounting`クラスタ8ファイル・`reservation`/`medical_record`/`accounting`/`LSTEP`各クラスタはCLAUDE.mdの「Forbidden in drive-by tasks」により境界マップ確定まで着手不可・`appointment_admin`(392行テストファイルが不釣合い)と`lab_import`(3コンストラクタ)・`permission_group`(ファイル数超過)・`ltv`(kana_normalize hoist待ち)は次候補から意図的除外・要個別検討）。
- **facade剥がし方針**: 未着手・申し送り継続（フラット側を最終的に空にする段階。呼び出し側全変更のchurnを要するため責任者判断）。
- **検証**: `docker compose exec backend go test -p 1 ./internal/repository/<domain>/ -count=1` + 3lint実行数確認（`-run` は関数名prefix、`Lint`は不可）+ golangci-lint scoped（`docker compose run --rm --no-deps --entrypoint golangci-lint backend run ./internal/repository/...`、フレッシュキャッシュ必須）。
