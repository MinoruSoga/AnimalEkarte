...
> **進捗 (2026-07-20)**: BE9-0 → BE9-2A → BE9-1 → BE9-2B（pilot=manualarticle・`d00c72a93`）→ **sub-batch①（master-CRUD 4エンティティ・`538cdb34`）→ sub-batch②（checkup/vaccine/prescription/inquiry・`14f00f6c`）→ sub-batch③（lab_import/lab_report saga・本コミット）完遂**。checkup_sync系はlstep batchへ先送り（論点#7）。medicalrecord domainの残 = **④a（vital/clinical_plan/medical_record_image）完遂（本コミット）→④b（treatment・tx機構refactor後に移動・checkpoint要・未着手）**→共有カーネル昇格batch→⑤（hospitalization/discharge-with-billing）。次 = ④b（要ユーザーgo-ahead）。分割根拠と着手前ゲートは「現在地」節参照。
