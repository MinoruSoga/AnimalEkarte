...
- **Session D post-integration qualification = BLOCKED**: code landing後のexact mainは`853137184`（backendは`297a23fc7`と同一）だが、Session Dはこのtreeを独立監査していない。central composition、tygo source、classification manifest、ADR-006、boundary map、route/OpenAPI gateは未cutoverで、canonical worktreeにはunrelated dirty WIPが残る。Session A/Session Bのruntime evidenceだけをSession D APPROVEへ読み替えない。
- post-integration qualification前に、mainへlanding済みのCSV cutover `e4e74d1fb`より後に残るcsvimport・password-reset・ADRのdirty WIPをlandingまたは隔離し、main由来のnew tipで全競合軸を再censusする。central cutover時はtarget直接登録、tygo source、classification manifest、ADR-006、boundary map、本書を同期する。
