...
- **状態（2026-07-04 実装・検証待ち）**: 「非 Tx の `auditSvc.LogEntry(` が 6箇所残存」は陳腐化（実測では service/handler 層に残る非 Tx `.LogEntry(` は permission_group_handler.go(4)・manual_article_handler.go(2)・lab_audit_logger.go(1) の計7箇所のみで、いずれも tx を持たない経路＝allowlist 対象外）。**唯一の実対象は audit-tx inventory lint が pending-migration と分類していた exam_results だった**。調査の結果、`examinationService.ReplaceItems` には監査呼び出し自体が一件も存在しない無監査ギャップ（`auditSvc AuditService` フィールドが代入されるのみで未使用のデッドコード）と判明。checkup_field_result（fe04b460）と同型で新規実装: `examination_repository.go` の `ReplaceItemsByExamID`/`FindAllItemsByExamID` を `dbOrTx` 化し `deletedCount` を返すよう変更、`examinationService` に `transactor`/`auditTx AuditTxLogger` を追加して `WithTx` 内でスナップショット→置換→`deletedCount>0` 時のみ `LogEntryTx` を実行（監査失敗で tx rollback・fail-closed）。`AuditActionExamResultReplace`/`AuditResourceExamResult` 新設。DB-backed atomicity テスト `examination_repository_tx_atomicity_test.go`（RollsBack/CommitsWithinAmbientTx）+ service 層 mock テスト（audit失敗→rollback、deletedCount=0→無監査）を追加。`audit_tx_inventory_lint_test.go` の allowlist を `pending-migration`→`audited-tx-internal` に更新（`TestClinicalResultAuditTxInventory_StatusesAreLive` も追随）。ハンドラは `optionalStaffID(c)` を actorID として配線。**状態（2026-07-04 Docker検証・PASS）**: `go build ./internal/...` / `go vet ./internal/...` 共に exit 0。`go test ./internal/repository/... ./internal/service/... ./internal/handler/... -run 'Examination|AuditTx'` 全 PASS（`TestExaminationRepository_ReplaceItemsByExamID_RollsBackWhenAmbientTxFails`/`_CommitsWithinAmbientTx`・`TestClinicalResultAuditTxInventory_*`・service 層 fail-closed rollback テスト含む）。scoped lint（`--max-same-issues 0 --max-issues-per-linter 0`、フレッシュキャッシュ）0 issues。
...
- **状態（2026-07-04 実装・検証待ち）**: internal/model には clinic_id 列を持つ struct が70件超あり（Pet/Owner/Billing 等の中核業務データも含む）、純粋な struct 形状だけから「master か否か」を自動判別するのは既存コードが明示する通り意味論的判断（Account/LineCustomer は clinic_id を持つが意図的に master 扱いしない、という既存コメントが根拠）であり信頼できない。そのため「後者」（突合チェック）を採用: 新規 `preload_master_model_reconciliation_test.go` で (1) internal/model を実ファイルシステムから AST 走査し ClinicID 列を持つ struct 集合を取得（go:embed は `..` を含むパターンを許可しないため、`go test` がテスト対象パッケージのディレクトリを cwd にする仕様を利用した `os.Getwd()` ベースの相対パス解決を採用）、(2) internal/service の write 側 `clinicScopedMasterFKField`（`master_fk_write_inventory_lint_test.go`）を同様に実ファイルから抽出、(3) read 側 `clinicScopedMasterAssoc` は同一パッケージ内なのでそのまま参照、の3者を双方向突合。model に存在しない read 側エントリ（stale）・model に存在し write 側に登録済みだが read 側未登録のエントリ（漏れ、`masterModelReadWriteExemptions` 未登録なら fail）・read 側に登録済みなのに exemptions に残る stale exemption、の3種を検出する。「意図的に1マッピングを欠落させて検出することを証明するフィクスチャ」（harness improvement 指定）を `TestMasterModelReconciliation_GateDetectsOmission` として追加。現行 master 群（ChiefComplaintType/HospitalizationPlan/Inventory含む）は model 層のフィールド定義を個別確認済みで GREEN になる設計。**状態（2026-07-04 Docker検証・PASS）**: `go test ./internal/repository/... -run 'PreloadClinicScope|MasterModelReconciliation'` 全 PASS。`TestMasterModelReconciliation_RealSourceIsConsistent`（floor guard含む実データ突合）・`TestMasterModelReconciliation_ExemptionsAreLive`・harness-improvement フィクスチャ `TestMasterModelReconciliation_GateDetectsOmission`（意図的欠落マッピングの検出を6ケースで実証）すべて GREEN。scoped build/vet/lint も同時に PASS 済み（R1-2 状態記述参照）。
...
- **状態（2026-07-04 arm・fail ゲート化完了）**: `backend/cmd/coverage-ratchet` ツール自体（Phase 1 判定ロジック）は既に実装済みだったが、`.coverage-baseline` が `0`（未 arm）・CI ステップが `-warn-only` + `continue-on-error: true` で実質 warn オンリーのまま放置されていた。GitHub Actions run 28655388836（main push, commit 80e0648a）の `backend-coverage` artifact から `coverage-summary.txt` 実測値 **89.9%** を取得し `.coverage-baseline` に転記（推測値は使わない、というツール自身のコメント方針どおり）。`.github/workflows/ci.yml` の「Coverage ratchet」ステップから `-warn-only` フラグと `continue-on-error: true` を削除し、tolerance（既定0.5pp）超の低下でステップ自体が exit 1 しジョブを fail させる状態にした。`docs/coverage-policy.md` の Phase 1 節・ベースライン記録表も実装と整合するよう更新。**状態（2026-07-04 Docker検証・PASS）**: `cmd/coverage-ratchet` の単体テスト（`TestParseTotalCoverage`/`TestEvaluateRatchet`）全 PASS。加えて実バイナリを合成 func-file 3パターンで armed baseline（89.9）に対して実行し、①同値 89.9%→`OK`・exit 0、②floor 境界 89.4%→`OK`・exit 0、③大幅低下 85.0%→`FAIL`・exit 1、を実測確認（fail ゲートが実際に機能することの直接証拠）。`.github/workflows/ci.yml` の Coverage ratchet ステップに `-warn-only`/`continue-on-error` が残っていないことも確認済み（隣接する「Coverage summary (non-gating)」ステップの `continue-on-error: true` は可視化専用の別ステップであり ratchet ステップとは無関係）。
