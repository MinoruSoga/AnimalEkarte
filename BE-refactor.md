...
- **Session B（isolated domain lane B / active assignment = inventory）**: Wave 2aで確定したrank #2のinventoryを担当する。Session Aと同じcleanかつquiescentな`B0`から専用branch/worktreeを1つだけ作り、INV-SEC P1を先行させてからinventoryをdomain-localに縦移動する。P1ではinventory-owned fileに加えて、signature追随に必要なmedicalrecord view/call site/mockだけを開始時allowlistへ明記する。共有master-FK lint行、composition、route/OpenAPI等のcentral/shared surfaceは編集せず、path・symbol・受け入れ条件をhandoffのchange requestとしてAへ渡す。Aはexact B candidateを反映した直後にlint理由文をA-authored central commitで同期し、combined exact treeのgateとreviewがgreenになるまでlanding unitを完了扱いにしない。Aのauth landing後は旧candidateをそのままmergeせず、new main tipから再census・再構成・再freezeする。INV-SEC P1を開始できない場合の縮退先はBUG-421〜425（代替=BUG-426〜428）で、csvimport cutoverとは同時実行しない。
...
`staff`、`auth`、`clinic`、`pet`、`owner`、`inventory`のproduction file/LOC、manifest/live、changed blocker path、ready条件、全競合軸、rankingはWave 2aで再計測・reconcile済みであり、結果は「[現在地と着手前ゲート](#be9-current-state)」を正とする。Wave 2aの`B0`はrecensus証跡であってWave 2bの実装baseには再利用しない。各landing後は残frontierをnew main tipで再計測し、次pairを再選定する。
