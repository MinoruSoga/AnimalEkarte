...
> **進捗 (2026-07-20)**: BE9-0 → BE9-2A → BE9-1 → BE9-2B（pilot=manualarticle・`d00c72a93`）→ **sub-batch①（master-CRUD 4エンティティ・`538cdb34`）→ ②（checkup/vaccine/prescription/inquiry・`14f00f6c`）→ ③（lab_import/lab_report saga・`75c55c48`）→ ④a（vital/clinical_plan/medical_record_image・`e3eb253e`）完遂**。checkup_sync系はlstep batchへ先送り（論点#7）。medicalrecord domainの残 = **④b（treatment・tx機構refactor後に移動・checkpoint要・未着手／2026-07-20ユーザー判断で本セッションは④aまでで区切り）**→共有カーネル昇格batch→⑤（hospitalization/discharge-with-billing）。次 = ④b（要ユーザーgo-ahead）。分割根拠と着手前ゲートは「現在地」節参照。push未実施。
...
|---|---|---:|---|
| 1 | medicalrecord | 185 | 最大domain・**進行中**。①master CRUD=済（`538cdb34`）→②非確定処理=済（`14f00f6c`・checkup_syncは論点#7で除外）→③lab(saga)=済（`75c55c48`）→④a（vital/clinical_plan/medical_record_image）=済（`e3eb253e`）→**④b treatment（tx機構refactor後移動・checkpoint要・未着手＝次）**→⑤hospitalization/billing接続の順（詳細定義 = 本doc「BE9-2D: medicalrecord sub-batch定義」） |
