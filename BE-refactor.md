...
5. **新規発見（BE8-4 申し送り事項）**: preload/audit_tx は bare basename キー、dbortx は相対パスキーと**3 lint 間でキー方式が不統一**。ドメイン分割が進むと同名ファイル（`repository.go` 等）の衝突で preload/audit_tx 側が false negative を起こしうる（現状は非衝突で潜在的・未発火）。§5-1-3 に記録。対処は本 run のスコープ外（BE8-4/5 着手前の判断事項として申し送り）。
