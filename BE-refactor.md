...
- **進捗**: 対応済 2 / 22。残存 BE7-0, BE7-3〜BE7-21（部分対応: BE7-4 / BE7-15）。棚卸し基準 HEAD `e424de5c`（2026-07-14）。各項目完了時に見出しの `[ ]` を `[x]` に更新してよい（同一コミットに本書を含めてよい）。
...
#### BE7-3 [ ] fix: Lステップ疎通確認の復号エラーが無言で握りつぶされる
...
#### BE7-5 [ ] fix: panic recovery 漏れの goroutine 2箇所
...
#### BE7-6 [ ] refactor: repository ページネーション定型文（約18ファイル）を共有 Scope に集約
...
#### BE7-8 [ ] refactor: シフト休憩置換の19行重複をヘルパ統合
...
#### BE7-9 [ ] refactor: 日本語曜日配列の重複を timeutil へ集約
...
#### BE7-10 [ ] refactor: 割引率バリデーションのインライン再実装2箇所を共有ヘルパへ
...
#### BE7-12 [ ] refactor: LINE / Lステップ API URL の重複定義を一元化
